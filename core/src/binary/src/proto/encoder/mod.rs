...
mod test {
	...
	fn encodes_supported() {
		...
		size +=
			encoder.encode((&18, EncoderLit::SInt32Vec(&vec![-10i32, 10i32])), &mut dst).unwrap();
		size += encoder.encode((&19, EncoderLit::SInt64(&-10)), &mut dst).unwrap();
		size +=
			encoder.encode((&20, EncoderLit::SInt64Vec(&vec![-10i64, 10i64])), &mut dst).unwrap();
		...
		size +=
			encoder.encode((&26, EncoderLit::SFixed32Vec(&vec![-10i32, 10i32])), &mut dst).unwrap();
		size += encoder.encode((&27, EncoderLit::SFixed64(&-10)), &mut dst).unwrap();
		size +=
			encoder.encode((&28, EncoderLit::SFixed64Vec(&vec![-10i64, 10i64])), &mut dst).unwrap();
		assert_eq!(
			dst,
			vec![
				10, 3, 102, 111, 111, 16, 1, 26, 2, 0, 1, 32, 1, 42, 11, 156, 255, 255, 255, 255,
				255, 255, 255, 255, 1, 100, 48, 1, 58, 11, 156, 255, 255, 255, 255, 255, 255, 255,
				255, 1, 100, 64, 1, 74, 2, 1, 2, 80, 1, 90, 2, 1, 2, 101, 0, 0, 128, 63, 106, 8, 0,
				0, 128, 63, 0, 0, 0, 64, 113, 0, 0, 0, 0, 0, 0, 240, 63, 122, 16, 0, 0, 0, 0, 0, 0,
				240, 63, 0, 0, 0, 0, 0, 0, 0, 64, 130, 1, 3, 102, 111, 111, 136, 1, 19, 146, 1, 2,
				19, 20, 152, 1, 19, 162, 1, 2, 19, 20, 173, 1, 10, 0, 0, 0, 178, 1, 8, 1, 0, 0, 0,
				2, 0, 0, 0, 185, 1, 10, 0, 0, 0, 0, 0, 0, 0, 194, 1, 16, 1, 0, 0, 0, 0, 0, 0, 0, 2,
				0, 0, 0, 0, 0, 0, 0, 205, 1, 246, 255, 255, 255, 210, 1, 8, 246, 255, 255, 255, 10,
				0, 0, 0, 217, 1, 246, 255, 255, 255, 255, 255, 255, 255, 226, 1, 16, 246, 255, 255,
				255, 255, 255, 255, 255, 10, 0, 0, 0, 0, 0, 0, 0,
			]
		);
		assert_eq!(size, 209);
	}
