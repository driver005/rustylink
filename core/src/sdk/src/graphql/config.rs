...
impl Default for RendererConfig {
	fn default() -> Self {
		Self {
			using: None,
			data_source_fetch_method: Self::default_data_source_fetch_method_from_ctx(),
			...
			resolver_type: None,
			additional_attributes: Some(Self::additional_attributes()),
			phases: vec![],
...
impl RendererConfig {
	...
	fn additional_attributes() -> String {
			...
			Some(hidden_field) => {
				if hidden_field.is_empty() {
					...
					for each_hidden_field in hidden_field.iter() {
						...
						hidden_field.field_defs.push(each_hidden_field.field_def.clone());
