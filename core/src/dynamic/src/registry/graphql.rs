...
use crate::{InputType, Value, graphql::Context};
use async_graphql::{Any, CacheControl, IntrospectionMode, OutputType, SubscriptionType};
...
pub struct MetaField {
	...
	pub args: BTreeMap<String, MetaInputValue>,
...
pub enum MetaType {
	...
	Interface {
		...
		possible_types: BTreeSet<String>,
	...
	Enum {
		...
		enum_values: BTreeMap<String, MetaEnumValue>,
	...
	InputObject {
		...
		input_fields: BTreeMap<String, MetaInputValue>,
...
impl MetaType {
	...
	#[inline]
	pub fn fields(&self) -> Option<&BTreeMap<String, MetaField>> {
		match self {
			MetaType::Object {
				fields,
				..
	...
	#[inline]
	pub fn possible_types(&self) -> Option<&BTreeSet<String>> {
		match self {
			MetaType::Interface {
				possible_types,
				..
...
pub struct GraphQLRegistry {
	...
	pub implements: HashMap<String, BTreeSet<String>>,
...
impl GraphQLRegistry {
	...
	where
		T: InputType,
		F: FnMut(&mut GraphQLRegistry) -> MetaType,
	{
	...
	where
		...
		F: FnMut(&mut GraphQLRegistry) -> MetaType,
	{
		self.create_type(&mut f, &T::type_name(), std::any::type_name::<T>(), MetaTypeId::Object);
		T::qualified_type_name()
	}

	fn create_type<F: FnMut(&mut GraphQLRegistry) -> MetaType>(
		&mut self,
		f: &mut F,
		name: &str,
		...
		match self.types.get(name) {
			...
			None => {
				...
				// exist.
				self.types.insert(name.to_string(), type_id.create_fake_type(rust_typename));
				let ty = f(self);
	...
	pub fn create_fake_output_type<T: OutputType>(&mut self) -> MetaType {
		...
		self.types.get(&*T::type_name()).cloned().expect("You definitely encountered a bug!")
	}
	...
	fn create_entity_type_and_root_field(&mut self) {
		let possible_types: BTreeSet<String> = self
			.types
			.values()
		...
		if let MetaType::Object {
			fields,
			..
		} = self.types.get_mut(&self.query_type).expect("missing query type")
		{
			fields.insert(
				"_service".to_string(),
				MetaField {
	...
	pub fn create_introspection_types(&mut self) {
		...
		if let Some(MetaType::Object {
			fields,
			..
		}) = self.types.get_mut(&self.query_type)
		{
			fields.insert(
				"__schema".to_string(),
				MetaField {
	...
	pub fn remove_unused_types(&mut self) {
		...
		fn traverse_field<'a>(
			...
			traverse_type(types, used_types, MetaTypeName::concrete_typename(&field.ty));
		...
		fn traverse_input_value<'a>(
			...
			traverse_type(types, used_types, MetaTypeName::concrete_typename(&input_value.ty));
		}
	...
	pub fn find_visible_types(&self, ctx: &Context<'_>) -> HashSet<&str> {
		...
		fn traverse_field<'a>(
			...
			traverse_type(ctx, types, visible_types, MetaTypeName::concrete_typename(&field.ty));
			for arg in field.args.values() {
