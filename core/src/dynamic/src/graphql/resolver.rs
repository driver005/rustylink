...
use async_graphql::{
	Error, IntrospectionMode, Name, OutputType, Positioned, SDLExportOptions, ServerError,
...
use crate::{FieldFuture, FieldValue, FieldValueInner, ObjectAccessor, ResolverContext, Value};

use super::{Context, ContextSelectionSet, Field, Object, Schema, Type};
			...
			let resolve_fut = async {
				...
				let field_value = match field_future {
					...
					FieldFuture::Future(future) => {
						future.await.map_err(|err| err.into_server_error(field.pos))?
					}
				};
	...
	async move {
		match (type_ref, value) {
			...
			(
				...
				let values = values.iter().cloned().map(FieldValue::value).collect::<Vec<_>>();
				resolve_list(schema, ctx, type_ref, &values).await
			}
...
async fn resolve_list<'a>(
	...
	for (idx, value) in values.iter().enumerate() {
		...
		futures.push(async move {
			...
			let res_value =
				ctx_item.query_env.extensions.resolve(resolve_info, &mut resolve_fut).await?;
		...
		(Type::Enum(e), FieldValueInner::Value(Value::Enum(name))) => {
			if !e.enum_values.contains_key(name.as_str()) {
				return Err(ctx.set_error_path(
					Error::new(format!("internal: invalid item for enum \"{}\"", e.type_name()))
						.into_server_error(ctx.item.pos),
				));
			}
