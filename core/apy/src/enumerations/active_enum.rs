use dynamic::prelude::{GraphQLEnum, ProtoEnum};
...
impl ActiveEnumBuilder {
	...
	pub fn enumeration_graphql<A: ActiveEnum>(&self) -> GraphQLEnum {
		let enum_name = self.type_name::<A>();

		A::values().into_iter().fold(GraphQLEnum::new(&enum_name), |enumeration, variant| {
	...
	pub fn enumeration_proto<A: ActiveEnum>(&self) -> ProtoEnum {
		let enum_name = self.type_name::<A>();

		A::values().into_iter().enumerate().fold(
			ProtoEnum::new(&enum_name),
			|enumeration, (index, variant)| {
				let variant: Value = variant.into();
				let variant: String = variant.to_string();
				enumeration.item((self.variant_name(&enum_name, &variant), index as u32))
			},
		)
	}
}
