...
use dynamic::prelude::{ListAccessor, ObjectAccessor, ValueAccessor, ValueAccessors};
...
pub fn prepare_enumeration_condition<'a, T, O>(
	filter: &'a O,
	column: &T::Column,
	condition: Condition,
) -> SeaResult<Condition>
	...
	O: ObjectAccessor<'a>,
{
	...
	let extract_condition = |data: &O::ValueAccessor| -> Vec<String> {
		match data.get_accessor() {
			ValueAccessors::GraphQL(val) => val
				.list()
				.unwrap()
				.to_iter()
				.map(|item| extract_variant(&item.enum_name().unwrap().to_string()))
				.collect::<Vec<String>>(),
			ValueAccessors::Proto(val) => val
				.list()
				.unwrap()
				.to_iter()
				.map(|item| extract_variant(&item.enum_name().unwrap().to_string()))
