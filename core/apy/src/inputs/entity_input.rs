...
use dynamic::prelude::{
	GraphQLInputObject, GraphQLInputValue, ObjectAccessor, ProtoField, ProtoMessage,
};
...
impl EntityInputBuilder {
	...
	fn input_object<T>(&self, is_insert: bool) -> GraphQLInputObject
	where
		T: EntityTrait,
		<T as EntityTrait>::Model: Sync,
	{
		...
		T::Column::iter().fold(GraphQLInputObject::new(name), |object, column| {
			...
			object.field(GraphQLInputValue::new(column_name, graphql_type))
		})
	}

	/// used to produce the SeaORM entity insert input object
	pub fn insert_input_object<T>(&self) -> GraphQLInputObject
	where
		T: EntityTrait,
		<T as EntityTrait>::Model: Sync,
	{
	...
	pub fn update_input_object<T>(&self) -> GraphQLInputObject
	where
		T: EntityTrait,
		<T as EntityTrait>::Model: Sync,
	{
	...
	/// used to produce the SeaORM entity input object
	fn message<T>(&self, is_insert: bool) -> ProtoMessage
	where
		T: EntityTrait,
		<T as EntityTrait>::Model: Sync,
	{
		...
		T::Column::iter().enumerate().fold(ProtoMessage::new(name), |object, (index, column)| {
			...
			let graphql_type = match types_map_helper.sea_orm_column_type_to_proto_type(
				column_def.get_column_type(),
				!column_def.is_null() && is_insert,
			) {
				Some(type_name) => type_name,
			...
			object.field(ProtoField::input(column_name, index.add(1) as u32, graphql_type))
		})
	}

	/// used to produce the SeaORM entity insert input message
	pub fn insert_message<T>(&self) -> ProtoMessage
	where
		T: EntityTrait,
		<T as EntityTrait>::Model: Sync,
	{
	...
	/// used to produce the SeaORM entity update input message
	pub fn update_message<T>(&self) -> ProtoMessage
	where
		T: EntityTrait,
		<T as EntityTrait>::Model: Sync,
	{
		self.message::<T>(false)
	}

	pub fn parse_object<'a, T, O>(
		&self,
		object: &'a O,
		...
		O: ObjectAccessor<'a>,
	{
		...
		for column in T::Column::iter() {
			...
			let result =
				types_map_helper.value_to_sea_orm_value::<T, O::ValueAccessor>(&column, &value)?;
