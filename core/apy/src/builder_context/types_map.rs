...
use dynamic::{
	prelude::{
		GraphQLTypeRef, GraphQLValue, GraphQLValueAccessor, ProtoTypeRef, ProtoValue,
		ProtoValueAccessor, ValueAccessor,
	},
	ValueAccessors,
};
...
pub type FnInputTypeConversionGraphQl =
	Box<dyn Fn(&GraphQLValueAccessor) -> SeaResult<sea_orm::Value> + Send + Sync>;
pub type FnInputTypeConversionProto =
	Box<dyn Fn(&ProtoValueAccessor) -> SeaResult<sea_orm::Value> + Send + Sync>;
pub type FnOutputTypeConversionGraphQL =
	Box<dyn Fn(&sea_orm::sea_query::Value) -> SeaResult<GraphQLValue> + Send + Sync>;
pub type FnOutputTypeConversionProto =
	Box<dyn Fn(&sea_orm::sea_query::Value) -> SeaResult<ProtoValue> + Send + Sync>;
...
pub struct TypesMapConfig {
	...
	pub input_conversions_graphql: BTreeMap<String, FnInputTypeConversionGraphQl>,
	/// used to map entity_name.column_name input to a custom parser
	pub input_conversions_proto: BTreeMap<String, FnInputTypeConversionProto>,
	/// used to map entity_name.column_name output to a custom formatter
	pub output_conversions_graphql: BTreeMap<String, FnOutputTypeConversionGraphQL>,
	/// used to map entity_name.column_name output to a custom formatter
	pub output_conversions_proto: BTreeMap<String, FnOutputTypeConversionProto>,
...
impl std::default::Default for TypesMapConfig {
	fn default() -> Self {
		Self {
			...
			input_conversions_graphql: BTreeMap::new(),
			input_conversions_proto: BTreeMap::new(),
			output_conversions_graphql: BTreeMap::new(),
			output_conversions_proto: BTreeMap::new(),
...
impl TypesMapHelper {
	...
	pub fn value_to_sea_orm_value<'a, T, V>(
		&self,
		column: &T::Column,
		value: &V,
		...
		V: ValueAccessor<'a>,
	{
		...
		match value.get_accessor() {
			ValueAccessors::GraphQL(graphql) => {
				if let Some(parser) = self
					.context
					.types
					.input_conversions_graphql
					.get(&format!("{entity_name}.{column_name}"))
				{
					return parser.as_ref()(&graphql);
				}
			...
			ValueAccessors::Proto(proto) => {
				if let Some(parser) = self
					.context
					.types
					.input_conversions_proto
					.get(&format!("{entity_name}.{column_name}"))
				{
					return parser.as_ref()(&proto);
				}
	...
	pub fn sea_orm_column_type_to_graphql_type(
		...
		match ty {
			ColumnType::Char(_) | ColumnType::String(_) | ColumnType::Text => {
				Some(GraphQLTypeRef::named(GraphQLTypeRef::STRING))
			}
			...
			| ColumnType::Unsigned
			| ColumnType::BigUnsigned => Some(GraphQLTypeRef::named(GraphQLTypeRef::INT)),
			ColumnType::Float | ColumnType::Double => {
				Some(GraphQLTypeRef::named(GraphQLTypeRef::FLOAT))
			}
			...
			| ColumnType::Time
			| ColumnType::Date => Some(GraphQLTypeRef::named(GraphQLTypeRef::STRING)),
			ColumnType::Year => Some(GraphQLTypeRef::named(GraphQLTypeRef::INT)),
			ColumnType::Interval(_, _) => Some(GraphQLTypeRef::named(GraphQLTypeRef::STRING)),
			ColumnType::Binary(_)
			| ColumnType::VarBinary(_)
			| ColumnType::Bit(_)
			| ColumnType::VarBit(_)
			| ColumnType::Blob => Some(GraphQLTypeRef::named(GraphQLTypeRef::STRING)),
			ColumnType::Boolean => Some(GraphQLTypeRef::named(GraphQLTypeRef::BOOLEAN)),
			// FIXME: support json type
			ColumnType::Json | ColumnType::JsonBinary => None,
			ColumnType::Uuid => Some(GraphQLTypeRef::named(GraphQLTypeRef::STRING)),
			ColumnType::Enum {
				name: enum_name,
				variants: _,
			} => Some(GraphQLTypeRef::named(active_enum_builder.type_name_from_iden(enum_name))),
			...
			ColumnType::Array(iden) => {
				...
				iden_type.map(|it| GraphQLTypeRef::List(Box::new(it)))
			}
			ColumnType::Custom(_iden) => Some(GraphQLTypeRef::named(GraphQLTypeRef::STRING)),
			_ => None,
		}
		.map(|ty| {
			if not_null {
				GraphQLTypeRef::NonNull(Box::new(ty))
			} else {
				ty
			}
		})
	...
	/// None indicates that we do not support the type
	pub fn sea_orm_column_type_to_proto_type(
		&self,
		ty: &ColumnType,
		not_null: bool,
	) -> Option<ProtoTypeRef> {
		...
		match ty {
			ColumnType::Char(_) | ColumnType::String(_) | ColumnType::Text => {
				Some(ProtoTypeRef::named(ProtoTypeRef::STRING))
			}
			ColumnType::TinyInteger => Some(ProtoTypeRef::named(ProtoTypeRef::INT32)),
			ColumnType::SmallInteger => Some(ProtoTypeRef::named(ProtoTypeRef::INT32)),
			ColumnType::Integer => Some(ProtoTypeRef::named(ProtoTypeRef::INT32)),
			ColumnType::BigInteger => Some(ProtoTypeRef::named(ProtoTypeRef::INT64)),
			ColumnType::TinyUnsigned => Some(ProtoTypeRef::named(ProtoTypeRef::UINT32)),
			ColumnType::SmallUnsigned => Some(ProtoTypeRef::named(ProtoTypeRef::UINT32)),
			ColumnType::Unsigned => Some(ProtoTypeRef::named(ProtoTypeRef::UINT32)),
			ColumnType::BigUnsigned => Some(ProtoTypeRef::named(ProtoTypeRef::UINT64)),
			ColumnType::Float => Some(ProtoTypeRef::named(ProtoTypeRef::FLOAT)),
			ColumnType::Double => Some(ProtoTypeRef::named(ProtoTypeRef::DOUBLE)),
			ColumnType::Decimal(_) | ColumnType::Money(_) => {
				Some(ProtoTypeRef::named(ProtoTypeRef::STRING))
			}
			ColumnType::DateTime
			...
			| ColumnType::TimestampWithTimeZone
			| ColumnType::Time
			| ColumnType::Date => Some(ProtoTypeRef::named(ProtoTypeRef::STRING)),
			ColumnType::Year => Some(ProtoTypeRef::named(ProtoTypeRef::INT32)),
			ColumnType::Interval(_, _) => Some(ProtoTypeRef::named(ProtoTypeRef::STRING)),
			ColumnType::Binary(_)
			| ColumnType::VarBinary(_)
			| ColumnType::Bit(_)
			| ColumnType::VarBit(_)
			| ColumnType::Blob => Some(ProtoTypeRef::named(ProtoTypeRef::BYTES)),
			ColumnType::Boolean => Some(ProtoTypeRef::named(ProtoTypeRef::BOOL)),
			// FIXME: support json type
			ColumnType::Json | ColumnType::JsonBinary => None,
			ColumnType::Uuid => Some(ProtoTypeRef::named(ProtoTypeRef::STRING)),
			ColumnType::Enum {
				name: enum_name,
				variants: _,
			} => Some(ProtoTypeRef::named(active_enum_builder.type_name_from_iden(enum_name))),
			ColumnType::Cidr | ColumnType::Inet | ColumnType::MacAddr => {
				Some(ProtoTypeRef::named(ProtoTypeRef::STRING))
			}
			#[cfg(not(feature = "with-postgres-array"))]
			...
			ColumnType::Array(iden) => {
				...
				//   conts: bad for inserts
				let iden_type = self.sea_orm_column_type_to_graphql_type(iden.as_ref(), true);
				iden_type.map(|it| ProtoTypeRef::List(Box::new(it)))
			}
			ColumnType::Custom(_iden) => Some(ProtoTypeRef::named(ProtoTypeRef::STRING)),
			_ => None,
		}
		.map(|ty| {
			if not_null {
				ProtoTypeRef::NonNull(Box::new(ty))
			} else {
				ty
			}
		})
...
pub fn converted_value_to_sea_orm_value<'a, V>(
	...
	V: ValueAccessor<'a>,
{
	Ok(match column_type {
		...
		ConvertedType::Json => {
			...
			let value = sea_orm::entity::prelude::Json::from_str(input_string_map_error(
				value,
				entity_name,
				column_name,
			)?)
		...
		ConvertedType::ChronoDateTimeUtc => {
			...
			let value = sea_orm::entity::prelude::ChronoDateTimeUtc::from_str(
				input_string_map_error(value, entity_name, column_name)?,
			)
			.map_err(|e| {
				crate::SeaographyError::TypeConversionError(
		...
		ConvertedType::ChronoDateTimeLocal => {
			...
			let value = sea_orm::entity::prelude::ChronoDateTimeLocal::from_str(
				input_string_map_error(value, entity_name, column_name)?,
			)
			.map_err(|e| {
				crate::SeaographyError::TypeConversionError(
		...
		ConvertedType::Uuid => {
			...
			let value = sea_orm::entity::prelude::Uuid::from_str(input_string_map_error(
				value,
				entity_name,
				column_name,
			)?)
		...
		ConvertedType::Decimal => {
			...
			let value = sea_orm::entity::prelude::Decimal::from_str(input_string_map_error(
				value,
				entity_name,
				column_name,
			)?)
		...
		ConvertedType::BigDecimal => {
			...
			let value = sea_orm::entity::prelude::BigDecimal::from_str(input_string_map_error(
				value,
				entity_name,
				column_name,
			)?)
