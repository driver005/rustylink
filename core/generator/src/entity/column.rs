use crate::{types::Operation, util::escape_rust_keyword, DateTimeCrate};
...
impl Column {
	...
	pub fn get_rs_type(
		...
		fn write_rs_type(col_type: &ColumnType, date_time_crate: &DateTimeCrate) -> String {
			...
			match col_type {
				...
				ColumnType::Timestamp => match date_time_crate {
					...
					// ColumnType::Timpestamp(_) => PrimitiveDateTime: https://docs.rs/sqlx/0.3.5/sqlx/postgres/types/index.html#time
					DateTimeCrate::Time => "TimeDateTime".to_owned(),
				},
		...
		let ident: TokenStream = write_rs_type(&self.col_type, date_time_crate).parse().unwrap();

		let option = match operation {
			Some(op) => {
				if *op == Operation::Update || *op == Operation::List || *op == Operation::Get {
					false
				} else {
					self.not_null
				}
	...
	pub fn get_std_rs_type(
		...
		fn write_rs_type(col_type: &ColumnType, date_time_crate: &DateTimeCrate) -> String {
			...
			match col_type {
				...
				ColumnType::Float => "f32".to_owned(),
				ColumnType::Double => "f64".to_owned(),
				ColumnType::Json | ColumnType::JsonBinary => "String".to_owned(),
				ColumnType::Date => "String".to_owned(),
				ColumnType::Time => "String".to_owned(),
				ColumnType::DateTime => "String".to_owned(),
				ColumnType::Timestamp => "String".to_owned(),
				ColumnType::TimestampWithTimeZone => "String".to_owned(),
				ColumnType::Decimal(_) | ColumnType::Money(_) => "f32".to_owned(),
	...
	pub fn get_col_type_entity_attrs(&self) -> Option<TokenStream> {
	...
	pub fn get_col_type_grpc_attrs(&self) -> Option<TokenStream> {
		fn write_col_grpc_attrs(col_type: &ColumnType) -> Option<TokenStream> {
			match col_type {
				ColumnType::String(_) | ColumnType::Char(_) | ColumnType::Text => {
					Some(quote! { string })
				}
				...
				ColumnType::BigInteger => Some(quote! { int64 }),
				ColumnType::TinyUnsigned | ColumnType::SmallUnsigned | ColumnType::Unsigned => {
					Some(quote! { uint32 })
				}
				ColumnType::BigUnsigned => Some(quote! { uint64 }),
				ColumnType::Float => Some(quote! { float }),
				ColumnType::Double => Some(quote! { double }),
				ColumnType::Decimal(_) => Some(quote! { float }),
				ColumnType::DateTime
				...
				| ColumnType::Date
				| ColumnType::Year
				| ColumnType::Interval(..) => Some(quote! { string }),
				ColumnType::Bit(_) | ColumnType::VarBit(_) => Some(quote! { bytes }),
				ColumnType::Boolean => Some(quote! { bool }),
				ColumnType::Money(_) => Some(quote! { float }),
				ColumnType::Json | ColumnType::JsonBinary => Some(quote! { string }),
				ColumnType::Uuid => Some(quote! { string }),
				ColumnType::Custom(_) => Some(quote! { string }),
				ColumnType::Enum {
					name,
					..
				} => {
					let enum_name = name.to_string().to_upper_camel_case();
					Some(quote! { enumeration=#enum_name })
				}
				ColumnType::Array(column_type) => {
					let inner_type = write_col_grpc_attrs(column_type.as_ref());
					match inner_type {
						Some(inner_type) => Some(quote! { #inner_type }),
						None => Some(quote! { repeated }),
					}
				}
				ColumnType::Cidr | ColumnType::Inet | ColumnType::MacAddr | ColumnType::LTree => {
					Some(quote! { string })
				}
				_ => None,
			}
		}
		write_col_grpc_attrs(&self.col_type)
	}

	pub fn get_col_type_proto_attrs(&self) -> Option<TokenStream> {
		fn write_col_grpc_attrs(col_type: &ColumnType) -> Option<TokenStream> {
			match col_type {
				ColumnType::String(_) | ColumnType::Char(_) | ColumnType::Text => {
					Some(quote! { string })
				...
				ColumnType::Array(column_type) => {
					let inner_type = write_col_grpc_attrs(column_type.as_ref());
					match inner_type {
						Some(inner_type) => Some(quote! { repeated #inner_type }),
						None => Some(quote! { repeated }),
					}
				}
				ColumnType::Cidr | ColumnType::Inet | ColumnType::MacAddr | ColumnType::LTree => {
	...
	pub fn get_info(&self, date_time_crate: &DateTimeCrate) -> String {
		...
		let type_info = self.get_rs_type(date_time_crate, None).to_string().replace(' ', "");
		let col_info = self.col_info();
		write!(&mut info, "Column `{}`: {}{}", self.name, type_info, col_info).unwrap();
		info
	}
...
impl From<&ColumnDef> for Column {
	fn from(col_def: &ColumnDef) -> Self {
		...
		let auto_increment =
			col_def.get_column_spec().iter().any(|spec| matches!(spec, ColumnSpec::AutoIncrement));
		let not_null =
			col_def.get_column_spec().iter().any(|spec| matches!(spec, ColumnSpec::NotNull));
		let unique =
			col_def.get_column_spec().iter().any(|spec| matches!(spec, ColumnSpec::UniqueKey));
		Self {
			name,
			col_type,
			auto_increment,
...
#[cfg(test)]
mod tests {
	use crate::{entity::Column, DateTimeCrate};
	...
	fn setup() -> Vec<Column> {
		...
		vec![
			...
			make_col!("cake_id", ColumnType::Custom(SeaRc::new(Alias::new("cus_col")))),
	...
	fn test_get_rs_type_with_chrono() {
		...
		for (mut col, rs_type) in columns.into_iter().zip(rs_types) {
			...
			col.not_null = true;
			assert_eq!(
				col.get_rs_type(&chrono_crate, None).to_string(),
	...
	fn test_get_rs_type_with_time() {
		...
		for (mut col, rs_type) in columns.into_iter().zip(rs_types) {
			...
			col.not_null = true;
			assert_eq!(
				col.get_rs_type(&time_crate, None).to_string(),
				quote!(#rs_type).to_string()
			);
	...
	fn test_get_info() {
		...
		let column: Column = ColumnDef::new(Alias::new("id")).string().not_null().to_owned().into();
		assert_eq!(
		...
		let column: Column =
			ColumnDef::new(Alias::new("id")).string().not_null().unique_key().to_owned().into();
		assert_eq!(
			column.get_info(&DateTimeCrate::Chrono).as_str(),
		...
		let column: Column =
			ColumnDef::new(Alias::new("date_field")).date().not_null().to_owned().into();
		assert_eq!(
			column.get_info(&DateTimeCrate::Chrono).as_str(),
		...
		let column: Column =
			ColumnDef::new(Alias::new("time_field")).time().not_null().to_owned().into();
		assert_eq!(
			column.get_info(&DateTimeCrate::Chrono).as_str(),
		...
		let column: Column =
			ColumnDef::new(Alias::new("date_time_field")).date_time().not_null().to_owned().into();
		assert_eq!(
			column.get_info(&DateTimeCrate::Chrono).as_str(),
		...
		let column: Column =
			ColumnDef::new(Alias::new("timestamp_field")).timestamp().not_null().to_owned().into();
		assert_eq!(
			column.get_info(&DateTimeCrate::Chrono).as_str(),
	...
	fn test_from_column_def() {
		...
		let column: Column =
			ColumnDef::new(Alias::new("id")).string().unique_key().not_null().to_owned().into();
