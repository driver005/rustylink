...
use crate::{
	entity::Column, entity::ConjunctRelation, entity::PrimaryKey, entity::Relation,
	util::escape_rust_keyword, DateTimeCrate,
};
...
impl Entity {
	...
	pub fn get_column_names_snake_case(&self) -> Vec<Ident> {
		self.columns.iter().map(|col| col.get_name_snake_case()).collect()
	}

	pub fn get_column_names_camel_case(&self) -> Vec<Ident> {
		self.columns.iter().map(|col| col.get_name_camel_case()).collect()
	}
	...
	pub fn get_column_rs_types(
		...
		self.columns
			.clone()
			.into_iter()
			.map(|col| col.get_rs_type(date_time_crate, operation))
			.collect()
	}

	pub fn get_column_std_rs_types(
		&self,
		date_time_crate: &DateTimeCrate,
		operation: Option<&Operation>,
	) -> Vec<TokenStream> {
		self.columns
			.clone()
			.into_iter()
			.map(|col| col.get_std_rs_type(date_time_crate, operation))
			.collect()
	}

	pub fn get_column_defs(&self) -> Vec<TokenStream> {
		self.columns.clone().into_iter().map(|col| col.get_def()).collect()
	}

	pub fn get_primary_key_names_snake_case(&self) -> Vec<Ident> {
		self.primary_keys.iter().map(|pk| pk.get_name_snake_case()).collect()
	}

	pub fn get_primary_key_names_camel_case(&self) -> Vec<Ident> {
		self.primary_keys.iter().map(|pk| pk.get_name_camel_case()).collect()
	}

	pub fn get_relation_module_name(&self) -> Vec<Option<Ident>> {
		self.relations.iter().map(|rel| rel.get_module_name()).collect()
	}

	pub fn get_relation_enum_name(&self) -> Vec<Ident> {
		self.relations.iter().map(|rel| rel.get_enum_name()).collect()
	}
	...
	pub fn get_related_entity_enum_name(&self) -> Vec<Ident> {
		...
		let self_relations_reverse: Vec<Ident> = self
			.relations
			.iter()
		...
		let types = self
			...
			.map(|primary_key| {
				self.columns
					...
					.unwrap()
					.get_rs_type(date_time_crate, operation)
					.to_string()
			})
			.collect::<Vec<_>>();
		if !types.is_empty() {
	...
	pub fn get_conjunct_relations_via_snake_case(&self) -> Vec<Ident> {
		self.conjunct_relations.iter().map(|con_rel| con_rel.get_via_snake_case()).collect()
	}

	pub fn get_conjunct_relations_to_snake_case(&self) -> Vec<Ident> {
		self.conjunct_relations.iter().map(|con_rel| con_rel.get_to_snake_case()).collect()
	}

	pub fn get_conjunct_relations_to_upper_camel_case(&self) -> Vec<Ident> {
		self.conjunct_relations.iter().map(|con_rel| con_rel.get_to_upper_camel_case()).collect()
	}
...
mod tests {
	...
	use crate::{
		entity::Column, entity::Entity, entity::PrimaryKey, entity::Relation, entity::RelationType,
		DateTimeCrate,
	};
	...
	fn test_get_table_name_snake_case() {
		...
		assert_eq!(entity.get_table_name_snake_case(), "special_cake".to_owned());
	}
	...
	fn test_get_table_name_snake_case_ident() {
		...
		assert_eq!(entity.get_table_name_snake_case_ident(), format_ident!("{}", "special_cake"));
	}
	...
	fn test_get_table_name_camel_case_ident() {
		...
		assert_eq!(entity.get_table_name_camel_case_ident(), format_ident!("{}", "SpecialCake"));
	}
	...
	fn test_get_column_rs_types() {
		...
		for (i, elem) in
			entity.get_column_rs_types(&DateTimeCrate::Chrono, None).into_iter().enumerate()
		{
			assert_eq!(
				elem.to_string(),
				entity.columns[i].get_rs_type(&DateTimeCrate::Chrono, None).to_string()
			);
		}
	}
	...
	fn test_get_primary_key_names_snake_case() {
		...
		for (i, elem) in entity.get_primary_key_names_snake_case().into_iter().enumerate() {
	...
	fn test_get_primary_key_names_camel_case() {
		...
		for (i, elem) in entity.get_primary_key_names_camel_case().into_iter().enumerate() {
	...
	fn test_get_relation_attrs() {
		...
		for (i, elem) in entity.get_relation_attrs().into_iter().enumerate() {
			assert_eq!(elem.to_string(), entity.relations[i].get_attrs().to_string());
		}
	}
	...
	fn test_get_primary_key_auto_increment() {
		...
		assert_eq!(entity.get_primary_key_auto_increment(), format_ident!("{}", false));

		entity.columns[0].auto_increment = true;
		assert_eq!(entity.get_primary_key_auto_increment(), format_ident!("{}", true));
	}
	...
	fn test_get_primary_key_rs_type() {
		...
		assert_eq!(
			entity.get_primary_key_rs_type(&DateTimeCrate::Chrono, None).to_string(),
			entity.columns[0].get_rs_type(&DateTimeCrate::Chrono, None).to_string()
		);
	}
	...
	fn test_get_conjunct_relations_via_snake_case() {
		...
		for (i, elem) in entity.get_conjunct_relations_via_snake_case().into_iter().enumerate() {
	...
	fn test_get_conjunct_relations_to_snake_case() {
		...
		for (i, elem) in entity.get_conjunct_relations_to_snake_case().into_iter().enumerate() {
	...
	fn test_get_conjunct_relations_to_upper_camel_case() {
		...
		for (i, elem) in entity.get_conjunct_relations_to_upper_camel_case().into_iter().enumerate()
		{
