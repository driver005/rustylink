use crate::{
	entity::ActiveEnum, entity::Column, entity::ConjunctRelation, entity::Entity,
	entity::PrimaryKey, entity::Relation, entity::RelationType, error::Error,
	util::unpack_table_ref, writer::Writer,
};
...
impl EntityTransformer {
	pub fn transform(table_create_stmts: Vec<TableCreateStatement>) -> Result<Writer, Error> {
		...
		for table_create in table_create_stmts.into_iter() {
			...
			let relations: Vec<Relation> = table_create
				...
				.collect();
			primary_keys.extend(
				table_create.get_indexes().iter().filter(|index| index.is_primary_key()).flat_map(
					|index| {
						index
							.get_index_spec()
							.get_column_names()
		...
		for (tbl_name, relations) in inverse_relations.into_iter() {
			if let Some(entity) = entities.get_mut(&tbl_name) {
				for relation in relations.into_iter() {
					let duplicate_relation =
						entity.relations.iter().any(|rel| rel.ref_table == relation.ref_table);
...
mod tests {
	...
	fn duplicated_many_to_many_paths() -> Result<(), Box<dyn Error>> {
		...
		validate_compact_entities(
			...
			vec![
				("bills", include_str!("../tests_cfg/duplicated_many_to_many_paths/bills.rs")),
				("users", include_str!("../tests_cfg/duplicated_many_to_many_paths/users.rs")),
				(
					"users_saved_bills",
	...
	fn many_to_many() -> Result<(), Box<dyn Error>> {
		...
		validate_compact_entities(
			...
			vec![
				...
				("users_votes", include_str!("../tests_cfg/many_to_many/users_votes.rs")),
			],
		)
	}
	...
	fn many_to_many_multiple() -> Result<(), Box<dyn Error>> {
		...
		validate_compact_entities(
			...
			vec![
				("bills", include_str!("../tests_cfg/many_to_many_multiple/bills.rs")),
				("users", include_str!("../tests_cfg/many_to_many_multiple/users.rs")),
				("users_votes", include_str!("../tests_cfg/many_to_many_multiple/users_votes.rs")),
			],
		)
	}
	...
	fn self_referencing() -> Result<(), Box<dyn Error>> {
		...
		validate_compact_entities(
			...
			vec![
				("bills", include_str!("../tests_cfg/self_referencing/bills.rs")),
				("users", include_str!("../tests_cfg/self_referencing/users.rs")),
			],
		)
	}
	...
	fn validate_compact_entities(
		...
		for (entity_name, file_content) in files {
			let entity = entities.get(entity_name).expect("Forget to add entity to the list");
