...
impl Relation {
	...
	pub fn get_module_name(&self) -> Option<Ident> {
		if self.self_referencing {
			...
			Some(format_ident!("{}", escape_rust_keyword(self.ref_table.to_snake_case())))
		}
	}
	...
	pub fn get_column_camel_case(&self) -> Vec<Ident> {
		self.columns.iter().map(|col| format_ident!("{}", col.to_upper_camel_case())).collect()
	}

	pub fn get_ref_column_camel_case(&self) -> Vec<Ident> {
		self.ref_columns.iter().map(|col| format_ident!("{}", col.to_upper_camel_case())).collect()
	}
	...
	{
		let from: I =
			self.get_column_camel_case().iter().fold(I::default(), |mut acc, src_column| {
				acc.extend([map_src_column(src_column)]);
				acc
			});
		let to: I =
			self.get_ref_column_camel_case().iter().fold(I::default(), |mut acc, ref_column| {
...
#[cfg(test)]
mod tests {
	use crate::{entity::Relation, entity::RelationType};
