...
#[sea_orm(table_name = "json_transform")]
pub struct Model {
	#[sea_orm(ignore)]
...
impl TaskMapper for Model {
	...
	async fn map_task(&self, context: &Context, task: &mut TaskModel) -> Result<()> {
		...
		task.task_type = TaskType::JsonJqTransform;
...
impl TryFrom<Arc<TaskConfig>> for Model {
	...
	fn try_from(value: Arc<TaskConfig>) -> Result<Self, Self::Error> {
		...
		Ok(Self {
			task_configuration,
			id: Uuid::new_v4(),
			query_expression: owned.get_input_parameter_required("query_expression")?.to_string(),
			task_model_id: None,
		})
	}
}

pub type JsonTransform = Model;
