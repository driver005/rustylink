...
#[sea_orm(table_name = "buissness_rule")]
pub struct Model {
	#[sea_orm(ignore)]
	...
	pub input_column: serde_json::Value,
...
impl TaskMapper for Model {
	...
	fn map_task(&self, _context: &Context, task: &mut TaskModel) -> Result<()> {
		task.task_type = TaskType::BuissnessRule;
	...
	async fn execute(&self, context: &mut Context) -> Result<TaskModel> {
		...
		task_model.buissness_rule_id = Some(self.id);
...
impl TryFrom<Arc<TaskConfig>> for Model {
	...
	fn try_from(value: Arc<TaskConfig>) -> Result<Self, Self::Error> {
		...
		Ok(Self {
			task_configuration,
			id: Uuid::new_v4(),
			rule_file_location: owned
				.get_input_parameter_required("rule_file_location")?
				.to_string(),
			execution_strategy: owned
				.get_input_parameter_required("execution_strategy")?
				.to_string(),
			input_column: owned
				.get_input_parameter_optinal("input_column")
				.and_then(|v| v.as_object())
				.map(|v| v.clone().into_iter().collect())
				.ok_or_else(|| Error::IllegalArgument("input_column is missing".to_string()))?,
			output_column: match owned.get_input_parameter_required("output_column")?.as_array() {
				Some(v) => v.iter().map(|v| v.to_string()).collect(),
				None => return Err(Error::IllegalArgument("output_column is missing".to_string())),
			},
		})
	}
}

pub type BuissnessRule = Model;
