...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/batch_job")
			.route("/", web::get().to(GetBatchJob::route))
			.route("/", web::post().to(CreateBatchJob::route))
			.route("/list", web::get().to(ListBatchJob::route))
			.route("/{id}", web::put().to(UpdateBatchJob::route))
			.route("/{id}", web::get().to(DeleteBatchJob::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetBatchJob {
...
impl GetBatchJob {
	async fn route(
		...
		json_data: Option<web::Json<GetBatchJob>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetBatchJob::default(),
		};
		let mut query = BatchJob::find();
		if let Some(id) = &model.id {
			query = query.filter(BatchJobColumn::Id.eq(id.clone()));
		}
		if let Some(r#type) = &model.r#type {
			query = query.filter(BatchJobColumn::Type.eq(r#type.clone()));
		}
		if let Some(created_by) = &model.created_by {
			query = query.filter(BatchJobColumn::CreatedBy.eq(created_by.clone()));
		}
		if let Some(context) = &model.context {
			query = query.filter(BatchJobColumn::Context.eq(context.clone()));
		}
		if let Some(result) = &model.result {
			query = query.filter(BatchJobColumn::Result.eq(result.clone()));
		}
		if let Some(dry_run) = &model.dry_run {
			query = query.filter(BatchJobColumn::DryRun.eq(dry_run.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(BatchJobColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(pre_processed_at) = &model.pre_processed_at {
			query = query.filter(BatchJobColumn::PreProcessedAt.eq(pre_processed_at.clone()));
		}
		if let Some(confirmed_at) = &model.confirmed_at {
			query = query.filter(BatchJobColumn::ConfirmedAt.eq(confirmed_at.clone()));
		}
		if let Some(processing_at) = &model.processing_at {
			query = query.filter(BatchJobColumn::ProcessingAt.eq(processing_at.clone()));
		}
		if let Some(completed_at) = &model.completed_at {
			query = query.filter(BatchJobColumn::CompletedAt.eq(completed_at.clone()));
		}
		if let Some(failed_at) = &model.failed_at {
			query = query.filter(BatchJobColumn::FailedAt.eq(failed_at.clone()));
		}
		if let Some(canceled_at) = &model.canceled_at {
			query = query.filter(BatchJobColumn::CanceledAt.eq(canceled_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(BatchJobColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(BatchJobColumn::DeletedAt.eq(deleted_at.clone()));
		}
...
impl ListBatchJob {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListBatchJob>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListBatchJob::default(),
		};
		let mut query = BatchJob::find();
		if let Some(id) = &model.id {
			query = query.filter(BatchJobColumn::Id.eq(id.clone()));
...
impl UpdateBatchJob {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<BatchJobParmns>,
		json_data: Option<web::Json<UpdateBatchJob>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = BatchJob::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: BatchJobActiveModel = table.into();
					if let Some(r#type) = &model.r#type {
						model_active.r#type = Set(r#type.clone());
					}
					model_active.created_by = Set(model.created_by);
					model_active.context = Set(model.context);
					model_active.result = Set(model.result);
					if let Some(dry_run) = &model.dry_run {
						model_active.dry_run = Set(dry_run.clone());
					}
					if let Some(created_at) = &model.created_at {
						model_active.created_at = Set(created_at.clone());
					}
					model_active.pre_processed_at = Set(model.pre_processed_at);
					model_active.confirmed_at = Set(model.confirmed_at);
					model_active.processing_at = Set(model.processing_at);
					model_active.completed_at = Set(model.completed_at);
					model_active.failed_at = Set(model.failed_at);
...
impl DeleteBatchJob {
	async fn route(
		...
		let res = BatchJob::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
