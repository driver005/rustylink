...
#[derive(Deserialize)]
pub struct CartGiftCardsParmns {
	cart_id: String,
	gift_card_id: String,
}
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/cart_gift_cards")
			.route("/", web::get().to(GetCartGiftCards::route))
			.route("/", web::post().to(CreateCartGiftCards::route))
			.route("/list", web::get().to(ListCartGiftCards::route))
			.route("/{id}", web::put().to(UpdateCartGiftCards::route))
			.route("/{id}", web::get().to(DeleteCartGiftCards::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetCartGiftCards {
	pub cart_id: Option<String>,
	pub gift_card_id: Option<String>,
}

impl GetCartGiftCards {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetCartGiftCards>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetCartGiftCards::default(),
		};
		let mut query = CartGiftCards::find();
		if let Some(cart_id) = &model.cart_id {
			query = query.filter(CartGiftCardsColumn::CartId.eq(cart_id.clone()));
		}
		if let Some(gift_card_id) = &model.gift_card_id {
			query = query.filter(CartGiftCardsColumn::GiftCardId.eq(gift_card_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListCartGiftCards {
	pub cart_id: Option<String>,
	pub gift_card_id: Option<String>,
}

impl ListCartGiftCards {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListCartGiftCards>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListCartGiftCards::default(),
		};
		let mut query = CartGiftCards::find();
		if let Some(cart_id) = &model.cart_id {
			query = query.filter(CartGiftCardsColumn::CartId.eq(cart_id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateCartGiftCards {}

impl CreateCartGiftCards {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateCartGiftCards>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = CartGiftCardsActiveModel {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateCartGiftCards {}

impl UpdateCartGiftCards {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<CartGiftCardsParmns>,
		json_data: Option<web::Json<UpdateCartGiftCards>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		...
		let old_table =
			CartGiftCards::find_by_id((path.cart_id.clone(), path.gift_card_id.clone()))
				.one(db.get_ref())
				.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: CartGiftCardsActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteCartGiftCards {
	pub cart_id: String,
	pub gift_card_id: String,
}

impl DeleteCartGiftCards {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<CartGiftCardsParmns>,
	) -> impl Responder {
		let res = CartGiftCards::delete_by_id((path.cart_id.clone(), path.gift_card_id.clone()))
			.exec(db.get_ref())
			.await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseCartGiftCards {
	pub cart_id: String,
	pub gift_card_id: String,
}
