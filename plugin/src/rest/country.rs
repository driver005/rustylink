...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/country")
			.route("/", web::get().to(GetCountry::route))
			.route("/", web::post().to(CreateCountry::route))
			.route("/list", web::get().to(ListCountry::route))
			.route("/{id}", web::put().to(UpdateCountry::route))
			.route("/{id}", web::get().to(DeleteCountry::route)),
	);
}
...
impl GetCountry {
	async fn route(
		...
		json_data: Option<web::Json<GetCountry>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetCountry::default(),
		};
		let mut query = Country::find();
		if let Some(id) = &model.id {
			query = query.filter(CountryColumn::Id.eq(id.clone()));
		}
		if let Some(iso_2) = &model.iso_2 {
			query = query.filter(CountryColumn::Iso2.eq(iso_2.clone()));
		}
		if let Some(iso_3) = &model.iso_3 {
			query = query.filter(CountryColumn::Iso3.eq(iso_3.clone()));
		}
		if let Some(num_code) = &model.num_code {
			query = query.filter(CountryColumn::NumCode.eq(num_code.clone()));
		}
		if let Some(name) = &model.name {
			query = query.filter(CountryColumn::Name.eq(name.clone()));
		}
		if let Some(display_name) = &model.display_name {
			query = query.filter(CountryColumn::DisplayName.eq(display_name.clone()));
		}
		if let Some(region_id) = &model.region_id {
			query = query.filter(CountryColumn::RegionId.eq(region_id.clone()));
		}
...
impl ListCountry {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListCountry>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListCountry::default(),
		};
		let mut query = Country::find();
		if let Some(id) = &model.id {
			query = query.filter(CountryColumn::Id.eq(id.clone()));
...
impl CreateCountry {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateCountry>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = CountryActiveModel {
			id: Set("id".to_string()),
			iso_2: Set(model.iso_2.clone()),
			iso_3: Set(model.iso_3.clone()),
			num_code: Set(model.num_code.clone()),
...
impl UpdateCountry {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<CountryParmns>,
		json_data: Option<web::Json<UpdateCountry>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Country::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: CountryActiveModel = table.into();
					if let Some(iso_2) = &model.iso_2 {
						model_active.iso_2 = Set(iso_2.clone());
					}
					if let Some(iso_3) = &model.iso_3 {
						model_active.iso_3 = Set(iso_3.clone());
					}
					if let Some(num_code) = &model.num_code {
						model_active.num_code = Set(num_code.clone());
					}
					if let Some(name) = &model.name {
						model_active.name = Set(name.clone());
					}
					if let Some(display_name) = &model.display_name {
						model_active.display_name = Set(display_name.clone());
					}
...
impl DeleteCountry {
	async fn route(
		...
		let res = Country::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
