...
#[derive(Deserialize)]
pub struct IdempotencyKeyParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/idempotency_key")
			.route("/", web::get().to(GetIdempotencyKey::route))
			.route("/", web::post().to(CreateIdempotencyKey::route))
			.route("/list", web::get().to(ListIdempotencyKey::route))
			.route("/{id}", web::put().to(UpdateIdempotencyKey::route))
			.route("/{id}", web::get().to(DeleteIdempotencyKey::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetIdempotencyKey {
...
impl GetIdempotencyKey {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetIdempotencyKey>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetIdempotencyKey::default(),
		};
		let mut query = IdempotencyKey::find();
		if let Some(id) = &model.id {
			query = query.filter(IdempotencyKeyColumn::Id.eq(id.clone()));
		}
		if let Some(idempotency_key) = &model.idempotency_key {
			query = query.filter(IdempotencyKeyColumn::IdempotencyKey.eq(idempotency_key.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(IdempotencyKeyColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(locked_at) = &model.locked_at {
			query = query.filter(IdempotencyKeyColumn::LockedAt.eq(locked_at.clone()));
		}
		if let Some(request_method) = &model.request_method {
			query = query.filter(IdempotencyKeyColumn::RequestMethod.eq(request_method.clone()));
		}
		if let Some(request_params) = &model.request_params {
			query = query.filter(IdempotencyKeyColumn::RequestParams.eq(request_params.clone()));
		}
		if let Some(request_path) = &model.request_path {
			query = query.filter(IdempotencyKeyColumn::RequestPath.eq(request_path.clone()));
		}
		if let Some(response_code) = &model.response_code {
			query = query.filter(IdempotencyKeyColumn::ResponseCode.eq(response_code.clone()));
		}
		if let Some(response_body) = &model.response_body {
			query = query.filter(IdempotencyKeyColumn::ResponseBody.eq(response_body.clone()));
		}
		if let Some(recovery_point) = &model.recovery_point {
			query = query.filter(IdempotencyKeyColumn::RecoveryPoint.eq(recovery_point.clone()));
		}
...
impl ListIdempotencyKey {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListIdempotencyKey>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListIdempotencyKey::default(),
		};
		let mut query = IdempotencyKey::find();
		if let Some(id) = &model.id {
			query = query.filter(IdempotencyKeyColumn::Id.eq(id.clone()));
...
impl CreateIdempotencyKey {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateIdempotencyKey>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = IdempotencyKeyActiveModel {
			id: Set("id".to_string()),
			idempotency_key: Set(model.idempotency_key.clone()),
			created_at: Set(model.created_at.clone()),
			locked_at: Set(model.locked_at.clone()),
			request_method: Set(model.request_method.clone()),
			request_params: Set(model.request_params.clone()),
			request_path: Set(model.request_path.clone()),
			response_code: Set(model.response_code.clone()),
			response_body: Set(model.response_body.clone()),
			recovery_point: Set(model.recovery_point.clone()),
		};
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateIdempotencyKey {
	pub idempotency_key: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub locked_at: Option<DateTimeWithTimeZone>,
	pub request_method: Option<String>,
...
impl UpdateIdempotencyKey {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<IdempotencyKeyParmns>,
		json_data: Option<web::Json<UpdateIdempotencyKey>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = IdempotencyKey::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: IdempotencyKeyActiveModel = table.into();
					if let Some(idempotency_key) = &model.idempotency_key {
						model_active.idempotency_key = Set(idempotency_key.clone());
					}
					if let Some(created_at) = &model.created_at {
						model_active.created_at = Set(created_at.clone());
					}
					model_active.locked_at = Set(model.locked_at);
					model_active.request_method = Set(model.request_method);
					model_active.request_params = Set(model.request_params);
					model_active.request_path = Set(model.request_path);
					model_active.response_code = Set(model.response_code);
					model_active.response_body = Set(model.response_body);
					if let Some(recovery_point) = &model.recovery_point {
						model_active.recovery_point = Set(recovery_point.clone());
					}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteIdempotencyKey {
	pub id: String,
}

impl DeleteIdempotencyKey {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<IdempotencyKeyParmns>,
	) -> impl Responder {
		let res = IdempotencyKey::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseIdempotencyKey {
	pub id: String,
	pub idempotency_key: String,
	pub created_at: DateTimeWithTimeZone,
	pub locked_at: Option<DateTimeWithTimeZone>,
