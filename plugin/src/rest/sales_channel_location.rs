...
#[derive(Deserialize)]
pub struct SalesChannelLocationParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/sales_channel_location")
			.route("/", web::get().to(GetSalesChannelLocation::route))
			.route("/", web::post().to(CreateSalesChannelLocation::route))
			.route("/list", web::get().to(ListSalesChannelLocation::route))
			.route("/{id}", web::put().to(UpdateSalesChannelLocation::route))
			.route("/{id}", web::get().to(DeleteSalesChannelLocation::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetSalesChannelLocation {
...
impl GetSalesChannelLocation {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetSalesChannelLocation>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetSalesChannelLocation::default(),
		};
		let mut query = SalesChannelLocation::find();
		if let Some(id) = &model.id {
			query = query.filter(SalesChannelLocationColumn::Id.eq(id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(SalesChannelLocationColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(SalesChannelLocationColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(sales_channel_id) = &model.sales_channel_id {
			query = query
				.filter(SalesChannelLocationColumn::SalesChannelId.eq(sales_channel_id.clone()));
		}
		if let Some(location_id) = &model.location_id {
			query = query.filter(SalesChannelLocationColumn::LocationId.eq(location_id.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(SalesChannelLocationColumn::DeletedAt.eq(deleted_at.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListSalesChannelLocation {
	pub id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub sales_channel_id: Option<String>,
...
impl ListSalesChannelLocation {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListSalesChannelLocation>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListSalesChannelLocation::default(),
		};
		let mut query = SalesChannelLocation::find();
		if let Some(id) = &model.id {
			query = query.filter(SalesChannelLocationColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateSalesChannelLocation {
...
impl CreateSalesChannelLocation {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateSalesChannelLocation>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = SalesChannelLocationActiveModel {
			id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateSalesChannelLocation {
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub sales_channel_id: Option<String>,
	pub location_id: Option<String>,
...
impl UpdateSalesChannelLocation {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<SalesChannelLocationParmns>,
		json_data: Option<web::Json<UpdateSalesChannelLocation>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = SalesChannelLocation::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: SalesChannelLocationActiveModel = table.into();
					if let Some(created_at) = &model.created_at {
					...
					if let Some(sales_channel_id) = &model.sales_channel_id {
						model_active.sales_channel_id = Set(sales_channel_id.clone());
					}
					if let Some(location_id) = &model.location_id {
						model_active.location_id = Set(location_id.clone());
					}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteSalesChannelLocation {
	pub id: String,
}

impl DeleteSalesChannelLocation {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<SalesChannelLocationParmns>,
	) -> impl Responder {
		let res = SalesChannelLocation::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseSalesChannelLocation {
	pub id: String,
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
	pub sales_channel_id: String,
