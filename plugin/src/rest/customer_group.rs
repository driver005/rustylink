...
#[derive(Deserialize)]
pub struct CustomerGroupParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/customer_group")
			.route("/", web::get().to(GetCustomerGroup::route))
			.route("/", web::post().to(CreateCustomerGroup::route))
			.route("/list", web::get().to(ListCustomerGroup::route))
			.route("/{id}", web::put().to(UpdateCustomerGroup::route))
			.route("/{id}", web::get().to(DeleteCustomerGroup::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetCustomerGroup {
...
impl GetCustomerGroup {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetCustomerGroup>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetCustomerGroup::default(),
		};
		let mut query = CustomerGroup::find();
		if let Some(id) = &model.id {
			query = query.filter(CustomerGroupColumn::Id.eq(id.clone()));
		}
		if let Some(name) = &model.name {
			query = query.filter(CustomerGroupColumn::Name.eq(name.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(CustomerGroupColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(CustomerGroupColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(CustomerGroupColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(CustomerGroupColumn::Metadata.eq(metadata.clone()));
		}
...
impl ListCustomerGroup {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListCustomerGroup>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListCustomerGroup::default(),
		};
		let mut query = CustomerGroup::find();
		if let Some(id) = &model.id {
			query = query.filter(CustomerGroupColumn::Id.eq(id.clone()));
...
impl CreateCustomerGroup {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateCustomerGroup>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = CustomerGroupActiveModel {
			id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateCustomerGroup {
	pub name: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub deleted_at: Option<DateTimeWithTimeZone>,
...
impl UpdateCustomerGroup {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<CustomerGroupParmns>,
		json_data: Option<web::Json<UpdateCustomerGroup>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = CustomerGroup::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: CustomerGroupActiveModel = table.into();
					if let Some(name) = &model.name {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteCustomerGroup {
	pub id: String,
}

impl DeleteCustomerGroup {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<CustomerGroupParmns>,
	) -> impl Responder {
		let res = CustomerGroup::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseCustomerGroup {
	pub id: String,
	pub name: String,
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
