...
#[derive(Deserialize)]
pub struct PaymentSessionParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/payment_session")
			.route("/", web::get().to(GetPaymentSession::route))
			.route("/", web::post().to(CreatePaymentSession::route))
			.route("/list", web::get().to(ListPaymentSession::route))
			.route("/{id}", web::put().to(UpdatePaymentSession::route))
			.route("/{id}", web::get().to(DeletePaymentSession::route)),
	);
}
...
pub struct GetPaymentSession {
	...
	pub status: Option<PaymentSessionStatusEnum>,
	...
	pub is_initiated: Option<bool>,
}

impl GetPaymentSession {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetPaymentSession>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetPaymentSession::default(),
		};
		let mut query = PaymentSession::find();
		if let Some(id) = &model.id {
			query = query.filter(PaymentSessionColumn::Id.eq(id.clone()));
		}
		if let Some(cart_id) = &model.cart_id {
			query = query.filter(PaymentSessionColumn::CartId.eq(cart_id.clone()));
		}
		if let Some(provider_id) = &model.provider_id {
			query = query.filter(PaymentSessionColumn::ProviderId.eq(provider_id.clone()));
		}
		if let Some(is_selected) = &model.is_selected {
			query = query.filter(PaymentSessionColumn::IsSelected.eq(is_selected.clone()));
		}
		if let Some(status) = &model.status {
			query = query.filter(PaymentSessionColumn::Status.eq(status.clone()));
		}
		if let Some(data) = &model.data {
			query = query.filter(PaymentSessionColumn::Data.eq(data.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(PaymentSessionColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(PaymentSessionColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(idempotency_key) = &model.idempotency_key {
			query = query.filter(PaymentSessionColumn::IdempotencyKey.eq(idempotency_key.clone()));
		}
		if let Some(payment_authorized_at) = &model.payment_authorized_at {
			query = query.filter(
				PaymentSessionColumn::PaymentAuthorizedAt.eq(payment_authorized_at.clone()),
			);
		}
		if let Some(amount) = &model.amount {
			query = query.filter(PaymentSessionColumn::Amount.eq(amount.clone()));
		}
		if let Some(is_initiated) = &model.is_initiated {
			query = query.filter(PaymentSessionColumn::IsInitiated.eq(is_initiated.clone()));
		}
...
impl ListPaymentSession {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListPaymentSession>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListPaymentSession::default(),
		};
		let mut query = PaymentSession::find();
		if let Some(id) = &model.id {
			query = query.filter(PaymentSessionColumn::Id.eq(id.clone()));
...
impl CreatePaymentSession {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreatePaymentSession>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let status = sea_orm_active_enums::PaymentSessionStatusEnum::from(*&model.status);
		let new_table = PaymentSessionActiveModel {
			id: Set("id".to_string()),
			cart_id: Set(model.cart_id.clone()),
			provider_id: Set(model.provider_id.clone()),
			is_selected: Set(model.is_selected.clone()),
			status: Set(status),
			...
			is_initiated: Set(model.is_initiated.clone()),
		};
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdatePaymentSession {
	pub cart_id: Option<String>,
	pub provider_id: Option<String>,
	pub is_selected: Option<bool>,
	pub status: Option<PaymentSessionStatusEnum>,
...
impl UpdatePaymentSession {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<PaymentSessionParmns>,
		json_data: Option<web::Json<UpdatePaymentSession>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = PaymentSession::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: PaymentSessionActiveModel = table.into();
					model_active.cart_id = Set(model.cart_id);
					if let Some(provider_id) = &model.provider_id {
						model_active.provider_id = Set(provider_id.clone());
					}
					model_active.is_selected = Set(model.is_selected);
					if let Some(value) = &model.status {
						let status =
							Some(sea_orm_active_enums::PaymentSessionStatusEnum::from(*value));
					...
					model_active.amount = Set(model.amount);
					if let Some(is_initiated) = &model.is_initiated {
						model_active.is_initiated = Set(is_initiated.clone());
					}
...
impl DeletePaymentSession {
	async fn route(
		...
		let res = PaymentSession::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponsePaymentSession {
	pub id: String,
	pub cart_id: Option<String>,
	pub provider_id: String,
	pub is_selected: Option<bool>,
