...
#[derive(Deserialize)]
pub struct PriceListParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/price_list")
			.route("/", web::get().to(GetPriceList::route))
			.route("/", web::post().to(CreatePriceList::route))
			.route("/list", web::get().to(ListPriceList::route))
			.route("/{id}", web::put().to(UpdatePriceList::route))
			.route("/{id}", web::get().to(DeletePriceList::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetPriceList {
	pub id: Option<String>,
	pub name: Option<String>,
	pub description: Option<String>,
	pub r#type: Option<PriceListTypeEnum>,
	pub status: Option<PriceListStatusEnum>,
...
impl GetPriceList {
	async fn route(
		...
		json_data: Option<web::Json<GetPriceList>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetPriceList::default(),
		};
		let mut query = PriceList::find();
		if let Some(id) = &model.id {
			query = query.filter(PriceListColumn::Id.eq(id.clone()));
		}
		if let Some(name) = &model.name {
			query = query.filter(PriceListColumn::Name.eq(name.clone()));
		}
		if let Some(description) = &model.description {
			query = query.filter(PriceListColumn::Description.eq(description.clone()));
		}
		if let Some(r#type) = &model.r#type {
			query = query.filter(PriceListColumn::Type.eq(r#type.clone()));
		}
		if let Some(status) = &model.status {
			query = query.filter(PriceListColumn::Status.eq(status.clone()));
		}
		if let Some(starts_at) = &model.starts_at {
			query = query.filter(PriceListColumn::StartsAt.eq(starts_at.clone()));
		}
		if let Some(ends_at) = &model.ends_at {
			query = query.filter(PriceListColumn::EndsAt.eq(ends_at.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(PriceListColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(PriceListColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(PriceListColumn::DeletedAt.eq(deleted_at.clone()));
		}
...
impl ListPriceList {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListPriceList>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListPriceList::default(),
		};
		let mut query = PriceList::find();
		if let Some(id) = &model.id {
			query = query.filter(PriceListColumn::Id.eq(id.clone()));
...
impl CreatePriceList {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreatePriceList>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let r#type = sea_orm_active_enums::PriceListTypeEnum::from(*&model.r#type);
		let status = sea_orm_active_enums::PriceListStatusEnum::from(*&model.status);
		let new_table = PriceListActiveModel {
			id: Set("id".to_string()),
...
impl UpdatePriceList {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<PriceListParmns>,
		json_data: Option<web::Json<UpdatePriceList>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = PriceList::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: PriceListActiveModel = table.into();
					if let Some(name) = &model.name {
					...
					if let Some(value) = &model.r#type {
						let r#type = Some(sea_orm_active_enums::PriceListTypeEnum::from(*value));
						model_active.r#type = Set(r#type);
					}
					if let Some(value) = &model.status {
						let status = Some(sea_orm_active_enums::PriceListStatusEnum::from(*value));
						model_active.status = Set(status);
					}
					model_active.starts_at = Set(model.starts_at);
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeletePriceList {
	pub id: String,
}

impl DeletePriceList {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<PriceListParmns>,
	) -> impl Responder {
		let res = PriceList::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
