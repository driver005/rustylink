...
#[derive(Deserialize)]
pub struct ProductShippingProfileParmns {
	profile_id: String,
	product_id: String,
}
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/product_shipping_profile")
			.route("/", web::get().to(GetProductShippingProfile::route))
			.route("/", web::post().to(CreateProductShippingProfile::route))
			.route("/list", web::get().to(ListProductShippingProfile::route))
			.route("/{id}", web::put().to(UpdateProductShippingProfile::route))
			.route("/{id}", web::get().to(DeleteProductShippingProfile::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetProductShippingProfile {
	pub profile_id: Option<String>,
	pub product_id: Option<String>,
}

impl GetProductShippingProfile {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetProductShippingProfile>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetProductShippingProfile::default(),
		};
		let mut query = ProductShippingProfile::find();
		if let Some(profile_id) = &model.profile_id {
			query = query.filter(ProductShippingProfileColumn::ProfileId.eq(profile_id.clone()));
		}
		if let Some(product_id) = &model.product_id {
			query = query.filter(ProductShippingProfileColumn::ProductId.eq(product_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListProductShippingProfile {
	pub profile_id: Option<String>,
	pub product_id: Option<String>,
}

impl ListProductShippingProfile {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListProductShippingProfile>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListProductShippingProfile::default(),
		};
		let mut query = ProductShippingProfile::find();
		if let Some(profile_id) = &model.profile_id {
			query = query.filter(ProductShippingProfileColumn::ProfileId.eq(profile_id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateProductShippingProfile {}

impl CreateProductShippingProfile {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateProductShippingProfile>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ProductShippingProfileActiveModel {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateProductShippingProfile {}

impl UpdateProductShippingProfile {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductShippingProfileParmns>,
		json_data: Option<web::Json<UpdateProductShippingProfile>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		...
		let old_table =
			ProductShippingProfile::find_by_id((path.profile_id.clone(), path.product_id.clone()))
				.one(db.get_ref())
				.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ProductShippingProfileActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteProductShippingProfile {
	pub profile_id: String,
	pub product_id: String,
}

impl DeleteProductShippingProfile {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductShippingProfileParmns>,
	) -> impl Responder {
		let res = ProductShippingProfile::delete_by_id((
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseProductShippingProfile {
	pub profile_id: String,
	pub product_id: String,
}
