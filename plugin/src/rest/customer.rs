...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/customer")
			.route("/", web::get().to(GetCustomer::route))
			.route("/", web::post().to(CreateCustomer::route))
			.route("/list", web::get().to(ListCustomer::route))
			.route("/{id}", web::put().to(UpdateCustomer::route))
			.route("/{id}", web::get().to(DeleteCustomer::route)),
	);
}
...
impl GetCustomer {
	async fn route(
		...
		json_data: Option<web::Json<GetCustomer>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetCustomer::default(),
		};
		let mut query = Customer::find();
		if let Some(id) = &model.id {
			query = query.filter(CustomerColumn::Id.eq(id.clone()));
		}
		if let Some(email) = &model.email {
			query = query.filter(CustomerColumn::Email.eq(email.clone()));
		}
		if let Some(first_name) = &model.first_name {
			query = query.filter(CustomerColumn::FirstName.eq(first_name.clone()));
		}
		if let Some(last_name) = &model.last_name {
			query = query.filter(CustomerColumn::LastName.eq(last_name.clone()));
		}
		if let Some(billing_address_id) = &model.billing_address_id {
			query = query.filter(CustomerColumn::BillingAddressId.eq(billing_address_id.clone()));
		}
		if let Some(password_hash) = &model.password_hash {
			query = query.filter(CustomerColumn::PasswordHash.eq(password_hash.clone()));
		}
		if let Some(phone) = &model.phone {
			query = query.filter(CustomerColumn::Phone.eq(phone.clone()));
		}
		if let Some(has_account) = &model.has_account {
			query = query.filter(CustomerColumn::HasAccount.eq(has_account.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(CustomerColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(CustomerColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(CustomerColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(CustomerColumn::Metadata.eq(metadata.clone()));
		}
...
impl ListCustomer {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListCustomer>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListCustomer::default(),
		};
		let mut query = Customer::find();
		if let Some(id) = &model.id {
			query = query.filter(CustomerColumn::Id.eq(id.clone()));
...
impl CreateCustomer {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateCustomer>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = CustomerActiveModel {
			id: Set("id".to_string()),
			...
			has_account: Set(model.has_account.clone()),
...
impl UpdateCustomer {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<CustomerParmns>,
		json_data: Option<web::Json<UpdateCustomer>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Customer::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: CustomerActiveModel = table.into();
					if let Some(email) = &model.email {
					...
					model_active.password_hash = Set(model.password_hash);
					model_active.phone = Set(model.phone);
					if let Some(has_account) = &model.has_account {
						model_active.has_account = Set(has_account.clone());
					}
					if let Some(created_at) = &model.created_at {
...
impl DeleteCustomer {
	async fn route(
		...
		let res = Customer::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
