...
#[derive(Deserialize)]
pub struct PublishableApiKeyParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/publishable_api_key")
			.route("/", web::get().to(GetPublishableApiKey::route))
			.route("/", web::post().to(CreatePublishableApiKey::route))
			.route("/list", web::get().to(ListPublishableApiKey::route))
			.route("/{id}", web::put().to(UpdatePublishableApiKey::route))
			.route("/{id}", web::get().to(DeletePublishableApiKey::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetPublishableApiKey {
...
impl GetPublishableApiKey {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetPublishableApiKey>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetPublishableApiKey::default(),
		};
		let mut query = PublishableApiKey::find();
		if let Some(id) = &model.id {
			query = query.filter(PublishableApiKeyColumn::Id.eq(id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(PublishableApiKeyColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(PublishableApiKeyColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(created_by) = &model.created_by {
			query = query.filter(PublishableApiKeyColumn::CreatedBy.eq(created_by.clone()));
		}
		if let Some(revoked_by) = &model.revoked_by {
			query = query.filter(PublishableApiKeyColumn::RevokedBy.eq(revoked_by.clone()));
		}
		if let Some(revoked_at) = &model.revoked_at {
			query = query.filter(PublishableApiKeyColumn::RevokedAt.eq(revoked_at.clone()));
		}
		if let Some(title) = &model.title {
			query = query.filter(PublishableApiKeyColumn::Title.eq(title.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListPublishableApiKey {
	pub id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub created_by: Option<String>,
...
impl ListPublishableApiKey {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListPublishableApiKey>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListPublishableApiKey::default(),
		};
		let mut query = PublishableApiKey::find();
		if let Some(id) = &model.id {
			query = query.filter(PublishableApiKeyColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreatePublishableApiKey {
...
impl CreatePublishableApiKey {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreatePublishableApiKey>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = PublishableApiKeyActiveModel {
			id: Set("id".to_string()),
			created_at: Set(model.created_at.clone()),
			updated_at: Set(model.updated_at.clone()),
			created_by: Set(model.created_by.clone()),
			revoked_by: Set(model.revoked_by.clone()),
			revoked_at: Set(model.revoked_at.clone()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdatePublishableApiKey {
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub created_by: Option<String>,
	pub revoked_by: Option<String>,
...
impl UpdatePublishableApiKey {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<PublishableApiKeyParmns>,
		json_data: Option<web::Json<UpdatePublishableApiKey>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = PublishableApiKey::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: PublishableApiKeyActiveModel = table.into();
					if let Some(created_at) = &model.created_at {
					...
					model_active.revoked_by = Set(model.revoked_by);
					model_active.revoked_at = Set(model.revoked_at);
					if let Some(title) = &model.title {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeletePublishableApiKey {
	pub id: String,
}

impl DeletePublishableApiKey {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<PublishableApiKeyParmns>,
	) -> impl Responder {
		let res = PublishableApiKey::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponsePublishableApiKey {
	pub id: String,
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
	pub created_by: Option<String>,
