...
#[derive(Deserialize)]
pub struct RegionPaymentProvidersParmns {
	region_id: String,
	provider_id: String,
}
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/region_payment_providers")
			.route("/", web::get().to(GetRegionPaymentProviders::route))
			.route("/", web::post().to(CreateRegionPaymentProviders::route))
			.route("/list", web::get().to(ListRegionPaymentProviders::route))
			.route("/{id}", web::put().to(UpdateRegionPaymentProviders::route))
			.route("/{id}", web::get().to(DeleteRegionPaymentProviders::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetRegionPaymentProviders {
	pub region_id: Option<String>,
	pub provider_id: Option<String>,
}

impl GetRegionPaymentProviders {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetRegionPaymentProviders>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetRegionPaymentProviders::default(),
		};
		let mut query = RegionPaymentProviders::find();
		if let Some(region_id) = &model.region_id {
			query = query.filter(RegionPaymentProvidersColumn::RegionId.eq(region_id.clone()));
		}
		if let Some(provider_id) = &model.provider_id {
			query = query.filter(RegionPaymentProvidersColumn::ProviderId.eq(provider_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListRegionPaymentProviders {
	pub region_id: Option<String>,
	pub provider_id: Option<String>,
}

impl ListRegionPaymentProviders {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListRegionPaymentProviders>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListRegionPaymentProviders::default(),
		};
		let mut query = RegionPaymentProviders::find();
		if let Some(region_id) = &model.region_id {
			query = query.filter(RegionPaymentProvidersColumn::RegionId.eq(region_id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateRegionPaymentProviders {}

impl CreateRegionPaymentProviders {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateRegionPaymentProviders>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = RegionPaymentProvidersActiveModel {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateRegionPaymentProviders {}

impl UpdateRegionPaymentProviders {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<RegionPaymentProvidersParmns>,
		json_data: Option<web::Json<UpdateRegionPaymentProviders>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		...
		let old_table =
			RegionPaymentProviders::find_by_id((path.region_id.clone(), path.provider_id.clone()))
				.one(db.get_ref())
				.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: RegionPaymentProvidersActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteRegionPaymentProviders {
	pub region_id: String,
	pub provider_id: String,
}

impl DeleteRegionPaymentProviders {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<RegionPaymentProvidersParmns>,
	) -> impl Responder {
		let res = RegionPaymentProviders::delete_by_id((
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseRegionPaymentProviders {
	pub region_id: String,
	pub provider_id: String,
}
