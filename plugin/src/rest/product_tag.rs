...
#[derive(Deserialize)]
pub struct ProductTagParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/product_tag")
			.route("/", web::get().to(GetProductTag::route))
			.route("/", web::post().to(CreateProductTag::route))
			.route("/list", web::get().to(ListProductTag::route))
			.route("/{id}", web::put().to(UpdateProductTag::route))
			.route("/{id}", web::get().to(DeleteProductTag::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetProductTag {
...
impl GetProductTag {
	async fn route(
		...
		json_data: Option<web::Json<GetProductTag>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetProductTag::default(),
		};
		let mut query = ProductTag::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductTagColumn::Id.eq(id.clone()));
		}
		if let Some(value) = &model.value {
			query = query.filter(ProductTagColumn::Value.eq(value.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ProductTagColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ProductTagColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ProductTagColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ProductTagColumn::Metadata.eq(metadata.clone()));
		}
...
impl ListProductTag {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListProductTag>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListProductTag::default(),
		};
		let mut query = ProductTag::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductTagColumn::Id.eq(id.clone()));
...
impl CreateProductTag {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateProductTag>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ProductTagActiveModel {
			id: Set("id".to_string()),
...
impl UpdateProductTag {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductTagParmns>,
		json_data: Option<web::Json<UpdateProductTag>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ProductTag::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ProductTagActiveModel = table.into();
					if let Some(value) = &model.value {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteProductTag {
	pub id: String,
}

impl DeleteProductTag {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductTagParmns>,
	) -> impl Responder {
		let res = ProductTag::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
