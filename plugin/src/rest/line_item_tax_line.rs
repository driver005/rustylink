...
#[derive(Deserialize)]
pub struct LineItemTaxLineParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/line_item_tax_line")
			.route("/", web::get().to(GetLineItemTaxLine::route))
			.route("/", web::post().to(CreateLineItemTaxLine::route))
			.route("/list", web::get().to(ListLineItemTaxLine::route))
			.route("/{id}", web::put().to(UpdateLineItemTaxLine::route))
			.route("/{id}", web::get().to(DeleteLineItemTaxLine::route)),
	);
}

#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct GetLineItemTaxLine {
...
impl GetLineItemTaxLine {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetLineItemTaxLine>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetLineItemTaxLine::default(),
		};
		let mut query = LineItemTaxLine::find();
		if let Some(id) = &model.id {
			query = query.filter(LineItemTaxLineColumn::Id.eq(id.clone()));
		}
		if let Some(rate) = &model.rate {
			query = query.filter(LineItemTaxLineColumn::Rate.eq(rate.clone()));
		}
		if let Some(name) = &model.name {
			query = query.filter(LineItemTaxLineColumn::Name.eq(name.clone()));
		}
		if let Some(code) = &model.code {
			query = query.filter(LineItemTaxLineColumn::Code.eq(code.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(LineItemTaxLineColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(LineItemTaxLineColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(LineItemTaxLineColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(item_id) = &model.item_id {
			query = query.filter(LineItemTaxLineColumn::ItemId.eq(item_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct ListLineItemTaxLine {
	pub id: Option<String>,
	pub rate: Option<f32>,
	pub name: Option<String>,
	pub code: Option<String>,
...
impl ListLineItemTaxLine {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListLineItemTaxLine>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListLineItemTaxLine::default(),
		};
		let mut query = LineItemTaxLine::find();
		if let Some(id) = &model.id {
			query = query.filter(LineItemTaxLineColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct CreateLineItemTaxLine {
	pub rate: f32,
	pub name: String,
	pub code: Option<String>,
...
impl CreateLineItemTaxLine {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateLineItemTaxLine>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = LineItemTaxLineActiveModel {
			id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct UpdateLineItemTaxLine {
	pub rate: Option<f32>,
	pub name: Option<String>,
	pub code: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
...
impl UpdateLineItemTaxLine {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<LineItemTaxLineParmns>,
		json_data: Option<web::Json<UpdateLineItemTaxLine>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = LineItemTaxLine::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: LineItemTaxLineActiveModel = table.into();
					if let Some(rate) = &model.rate {
...
pub struct DeleteLineItemTaxLine {
	pub id: String,
}

impl DeleteLineItemTaxLine {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<LineItemTaxLineParmns>,
	) -> impl Responder {
		let res = LineItemTaxLine::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct ResponseLineItemTaxLine {
	pub id: String,
	pub rate: f32,
	pub name: String,
	pub code: Option<String>,
