...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/tax_rate")
			.route("/", web::get().to(GetTaxRate::route))
			.route("/", web::post().to(CreateTaxRate::route))
			.route("/list", web::get().to(ListTaxRate::route))
			.route("/{id}", web::put().to(UpdateTaxRate::route))
			.route("/{id}", web::get().to(DeleteTaxRate::route)),
	);
}

#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct GetTaxRate {
...
impl GetTaxRate {
	async fn route(
		...
		json_data: Option<web::Json<GetTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetTaxRate::default(),
		};
		let mut query = TaxRate::find();
		if let Some(id) = &model.id {
			query = query.filter(TaxRateColumn::Id.eq(id.clone()));
		}
		if let Some(rate) = &model.rate {
			query = query.filter(TaxRateColumn::Rate.eq(rate.clone()));
		}
		if let Some(code) = &model.code {
			query = query.filter(TaxRateColumn::Code.eq(code.clone()));
		}
		if let Some(name) = &model.name {
			query = query.filter(TaxRateColumn::Name.eq(name.clone()));
		}
		if let Some(region_id) = &model.region_id {
			query = query.filter(TaxRateColumn::RegionId.eq(region_id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(TaxRateColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(TaxRateColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(TaxRateColumn::Metadata.eq(metadata.clone()));
		}
...
impl ListTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListTaxRate::default(),
		};
		let mut query = TaxRate::find();
		if let Some(id) = &model.id {
			query = query.filter(TaxRateColumn::Id.eq(id.clone()));
...
impl CreateTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = TaxRateActiveModel {
			id: Set("id".to_string()),
...
impl UpdateTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<TaxRateParmns>,
		json_data: Option<web::Json<UpdateTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = TaxRate::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: TaxRateActiveModel = table.into();
					model_active.rate = Set(model.rate);
...
impl DeleteTaxRate {
	async fn route(
		...
		let res = TaxRate::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
