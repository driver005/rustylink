...
#[derive(Deserialize)]
pub struct TrackingLinkParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/tracking_link")
			.route("/", web::get().to(GetTrackingLink::route))
			.route("/", web::post().to(CreateTrackingLink::route))
			.route("/list", web::get().to(ListTrackingLink::route))
			.route("/{id}", web::put().to(UpdateTrackingLink::route))
			.route("/{id}", web::get().to(DeleteTrackingLink::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetTrackingLink {
...
impl GetTrackingLink {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetTrackingLink>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetTrackingLink::default(),
		};
		let mut query = TrackingLink::find();
		if let Some(id) = &model.id {
			query = query.filter(TrackingLinkColumn::Id.eq(id.clone()));
		}
		if let Some(url) = &model.url {
			query = query.filter(TrackingLinkColumn::Url.eq(url.clone()));
		}
		if let Some(tracking_number) = &model.tracking_number {
			query = query.filter(TrackingLinkColumn::TrackingNumber.eq(tracking_number.clone()));
		}
		if let Some(fulfillment_id) = &model.fulfillment_id {
			query = query.filter(TrackingLinkColumn::FulfillmentId.eq(fulfillment_id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(TrackingLinkColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(TrackingLinkColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(TrackingLinkColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(TrackingLinkColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(idempotency_key) = &model.idempotency_key {
			query = query.filter(TrackingLinkColumn::IdempotencyKey.eq(idempotency_key.clone()));
		}
...
impl ListTrackingLink {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListTrackingLink>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListTrackingLink::default(),
		};
		let mut query = TrackingLink::find();
		if let Some(id) = &model.id {
			query = query.filter(TrackingLinkColumn::Id.eq(id.clone()));
...
impl CreateTrackingLink {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateTrackingLink>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = TrackingLinkActiveModel {
			id: Set("id".to_string()),
			url: Set(model.url.clone()),
			tracking_number: Set(model.tracking_number.clone()),
			fulfillment_id: Set(model.fulfillment_id.clone()),
...
impl UpdateTrackingLink {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<TrackingLinkParmns>,
		json_data: Option<web::Json<UpdateTrackingLink>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = TrackingLink::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: TrackingLinkActiveModel = table.into();
					model_active.url = Set(model.url);
					if let Some(tracking_number) = &model.tracking_number {
						model_active.tracking_number = Set(tracking_number.clone());
					}
					if let Some(fulfillment_id) = &model.fulfillment_id {
						model_active.fulfillment_id = Set(fulfillment_id.clone());
					}
					if let Some(created_at) = &model.created_at {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteTrackingLink {
	pub id: String,
}

impl DeleteTrackingLink {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<TrackingLinkParmns>,
	) -> impl Responder {
		let res = TrackingLink::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseTrackingLink {
	pub id: String,
	pub url: Option<String>,
	pub tracking_number: String,
	pub fulfillment_id: String,
