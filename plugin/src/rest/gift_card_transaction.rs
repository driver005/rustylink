...
#[derive(Deserialize)]
pub struct GiftCardTransactionParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/gift_card_transaction")
			.route("/", web::get().to(GetGiftCardTransaction::route))
			.route("/", web::post().to(CreateGiftCardTransaction::route))
			.route("/list", web::get().to(ListGiftCardTransaction::route))
			.route("/{id}", web::put().to(UpdateGiftCardTransaction::route))
			.route("/{id}", web::get().to(DeleteGiftCardTransaction::route)),
	);
}

#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct GetGiftCardTransaction {
...
impl GetGiftCardTransaction {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetGiftCardTransaction>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetGiftCardTransaction::default(),
		};
		let mut query = GiftCardTransaction::find();
		if let Some(id) = &model.id {
			query = query.filter(GiftCardTransactionColumn::Id.eq(id.clone()));
		}
		if let Some(gift_card_id) = &model.gift_card_id {
			query = query.filter(GiftCardTransactionColumn::GiftCardId.eq(gift_card_id.clone()));
		}
		if let Some(order_id) = &model.order_id {
			query = query.filter(GiftCardTransactionColumn::OrderId.eq(order_id.clone()));
		}
		if let Some(amount) = &model.amount {
			query = query.filter(GiftCardTransactionColumn::Amount.eq(amount.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(GiftCardTransactionColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(is_taxable) = &model.is_taxable {
			query = query.filter(GiftCardTransactionColumn::IsTaxable.eq(is_taxable.clone()));
		}
		if let Some(tax_rate) = &model.tax_rate {
			query = query.filter(GiftCardTransactionColumn::TaxRate.eq(tax_rate.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct ListGiftCardTransaction {
	pub id: Option<String>,
	pub gift_card_id: Option<String>,
	pub order_id: Option<String>,
	pub amount: Option<i32>,
...
impl ListGiftCardTransaction {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListGiftCardTransaction>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListGiftCardTransaction::default(),
		};
		let mut query = GiftCardTransaction::find();
		if let Some(id) = &model.id {
			query = query.filter(GiftCardTransactionColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct CreateGiftCardTransaction {
...
impl CreateGiftCardTransaction {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateGiftCardTransaction>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = GiftCardTransactionActiveModel {
			id: Set("id".to_string()),
			gift_card_id: Set(model.gift_card_id.clone()),
			order_id: Set(model.order_id.clone()),
			amount: Set(model.amount.clone()),
			created_at: Set(model.created_at.clone()),
			is_taxable: Set(model.is_taxable.clone()),
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct UpdateGiftCardTransaction {
	pub gift_card_id: Option<String>,
	pub order_id: Option<String>,
	pub amount: Option<i32>,
	pub created_at: Option<DateTimeWithTimeZone>,
...
impl UpdateGiftCardTransaction {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<GiftCardTransactionParmns>,
		json_data: Option<web::Json<UpdateGiftCardTransaction>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = GiftCardTransaction::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: GiftCardTransactionActiveModel = table.into();
					if let Some(gift_card_id) = &model.gift_card_id {
						model_active.gift_card_id = Set(gift_card_id.clone());
					}
					if let Some(order_id) = &model.order_id {
					...
					model_active.is_taxable = Set(model.is_taxable);
...
pub struct DeleteGiftCardTransaction {
	pub id: String,
}

impl DeleteGiftCardTransaction {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<GiftCardTransactionParmns>,
	) -> impl Responder {
		let res = GiftCardTransaction::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct ResponseGiftCardTransaction {
	pub id: String,
	pub gift_card_id: String,
	pub order_id: String,
	pub amount: i32,
