...
#[derive(Deserialize)]
pub struct SalesChannelParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/sales_channel")
			.route("/", web::get().to(GetSalesChannel::route))
			.route("/", web::post().to(CreateSalesChannel::route))
			.route("/list", web::get().to(ListSalesChannel::route))
			.route("/{id}", web::put().to(UpdateSalesChannel::route))
			.route("/{id}", web::get().to(DeleteSalesChannel::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetSalesChannel {
...
impl GetSalesChannel {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetSalesChannel>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetSalesChannel::default(),
		};
		let mut query = SalesChannel::find();
		if let Some(id) = &model.id {
			query = query.filter(SalesChannelColumn::Id.eq(id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(SalesChannelColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(SalesChannelColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(SalesChannelColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(name) = &model.name {
			query = query.filter(SalesChannelColumn::Name.eq(name.clone()));
		}
		if let Some(description) = &model.description {
			query = query.filter(SalesChannelColumn::Description.eq(description.clone()));
		}
		if let Some(is_disabled) = &model.is_disabled {
			query = query.filter(SalesChannelColumn::IsDisabled.eq(is_disabled.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(SalesChannelColumn::Metadata.eq(metadata.clone()));
		}
...
impl ListSalesChannel {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListSalesChannel>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListSalesChannel::default(),
		};
		let mut query = SalesChannel::find();
		if let Some(id) = &model.id {
			query = query.filter(SalesChannelColumn::Id.eq(id.clone()));
...
impl CreateSalesChannel {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateSalesChannel>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = SalesChannelActiveModel {
			id: Set("id".to_string()),
...
impl UpdateSalesChannel {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<SalesChannelParmns>,
		json_data: Option<web::Json<UpdateSalesChannel>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = SalesChannel::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: SalesChannelActiveModel = table.into();
					if let Some(created_at) = &model.created_at {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteSalesChannel {
	pub id: String,
}

impl DeleteSalesChannel {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<SalesChannelParmns>,
	) -> impl Responder {
		let res = SalesChannel::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseSalesChannel {
	pub id: String,
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
	pub deleted_at: Option<DateTimeWithTimeZone>,
