...
#[derive(Deserialize)]
pub struct DraftOrderParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/draft_order")
			.route("/", web::get().to(GetDraftOrder::route))
			.route("/", web::post().to(CreateDraftOrder::route))
			.route("/list", web::get().to(ListDraftOrder::route))
			.route("/{id}", web::put().to(UpdateDraftOrder::route))
			.route("/{id}", web::get().to(DeleteDraftOrder::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetDraftOrder {
	pub id: Option<String>,
	pub status: Option<DraftOrderStatusEnum>,
...
impl GetDraftOrder {
	async fn route(
		...
		json_data: Option<web::Json<GetDraftOrder>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetDraftOrder::default(),
		};
		let mut query = DraftOrder::find();
		if let Some(id) = &model.id {
			query = query.filter(DraftOrderColumn::Id.eq(id.clone()));
		}
		if let Some(status) = &model.status {
			query = query.filter(DraftOrderColumn::Status.eq(status.clone()));
		}
		if let Some(display_id) = &model.display_id {
			query = query.filter(DraftOrderColumn::DisplayId.eq(display_id.clone()));
		}
		if let Some(cart_id) = &model.cart_id {
			query = query.filter(DraftOrderColumn::CartId.eq(cart_id.clone()));
		}
		if let Some(order_id) = &model.order_id {
			query = query.filter(DraftOrderColumn::OrderId.eq(order_id.clone()));
		}
		if let Some(canceled_at) = &model.canceled_at {
			query = query.filter(DraftOrderColumn::CanceledAt.eq(canceled_at.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(DraftOrderColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(DraftOrderColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(completed_at) = &model.completed_at {
			query = query.filter(DraftOrderColumn::CompletedAt.eq(completed_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(DraftOrderColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(idempotency_key) = &model.idempotency_key {
			query = query.filter(DraftOrderColumn::IdempotencyKey.eq(idempotency_key.clone()));
		}
		if let Some(no_notification_order) = &model.no_notification_order {
			query = query
				.filter(DraftOrderColumn::NoNotificationOrder.eq(no_notification_order.clone()));
		}
...
impl ListDraftOrder {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListDraftOrder>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListDraftOrder::default(),
		};
		let mut query = DraftOrder::find();
		if let Some(id) = &model.id {
			query = query.filter(DraftOrderColumn::Id.eq(id.clone()));
...
impl CreateDraftOrder {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateDraftOrder>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let status = sea_orm_active_enums::DraftOrderStatusEnum::from(*&model.status);
		let new_table = DraftOrderActiveModel {
			id: Set("id".to_string()),
			status: Set(status),
			...
			no_notification_order: Set(model.no_notification_order.clone()),
		};
...
impl UpdateDraftOrder {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<DraftOrderParmns>,
		json_data: Option<web::Json<UpdateDraftOrder>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = DraftOrder::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: DraftOrderActiveModel = table.into();
					if let Some(value) = &model.status {
						let status = Some(sea_orm_active_enums::DraftOrderStatusEnum::from(*value));
					...
					model_active.no_notification_order = Set(model.no_notification_order);
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteDraftOrder {
	pub id: String,
}

impl DeleteDraftOrder {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<DraftOrderParmns>,
	) -> impl Responder {
		let res = DraftOrder::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
