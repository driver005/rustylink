...
#[derive(Deserialize)]
pub struct ClaimOrderParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/claim_order")
			.route("/", web::get().to(GetClaimOrder::route))
			.route("/", web::post().to(CreateClaimOrder::route))
			.route("/list", web::get().to(ListClaimOrder::route))
			.route("/{id}", web::put().to(UpdateClaimOrder::route))
			.route("/{id}", web::get().to(DeleteClaimOrder::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetClaimOrder {
	pub id: Option<String>,
	pub payment_status: Option<ClaimOrderPaymentStatusEnum>,
	pub fulfillment_status: Option<ClaimOrderFulfillmentStatusEnum>,
	pub r#type: Option<ClaimOrderTypeEnum>,
...
impl GetClaimOrder {
	async fn route(
		...
		json_data: Option<web::Json<GetClaimOrder>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetClaimOrder::default(),
		};
		let mut query = ClaimOrder::find();
		if let Some(id) = &model.id {
			query = query.filter(ClaimOrderColumn::Id.eq(id.clone()));
		}
		if let Some(payment_status) = &model.payment_status {
			query = query.filter(ClaimOrderColumn::PaymentStatus.eq(payment_status.clone()));
		}
		if let Some(fulfillment_status) = &model.fulfillment_status {
			query =
				query.filter(ClaimOrderColumn::FulfillmentStatus.eq(fulfillment_status.clone()));
		}
		if let Some(r#type) = &model.r#type {
			query = query.filter(ClaimOrderColumn::Type.eq(r#type.clone()));
		}
		if let Some(order_id) = &model.order_id {
			query = query.filter(ClaimOrderColumn::OrderId.eq(order_id.clone()));
		}
		if let Some(shipping_address_id) = &model.shipping_address_id {
			query =
				query.filter(ClaimOrderColumn::ShippingAddressId.eq(shipping_address_id.clone()));
		}
		if let Some(refund_amount) = &model.refund_amount {
			query = query.filter(ClaimOrderColumn::RefundAmount.eq(refund_amount.clone()));
		}
		if let Some(canceled_at) = &model.canceled_at {
			query = query.filter(ClaimOrderColumn::CanceledAt.eq(canceled_at.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ClaimOrderColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ClaimOrderColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ClaimOrderColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ClaimOrderColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(idempotency_key) = &model.idempotency_key {
			query = query.filter(ClaimOrderColumn::IdempotencyKey.eq(idempotency_key.clone()));
		}
		if let Some(no_notification) = &model.no_notification {
			query = query.filter(ClaimOrderColumn::NoNotification.eq(no_notification.clone()));
		}
...
impl ListClaimOrder {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListClaimOrder>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListClaimOrder::default(),
		};
		let mut query = ClaimOrder::find();
		if let Some(id) = &model.id {
			query = query.filter(ClaimOrderColumn::Id.eq(id.clone()));
...
impl CreateClaimOrder {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateClaimOrder>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
		...
		let payment_status =
			sea_orm_active_enums::ClaimOrderPaymentStatusEnum::from(*&model.payment_status);
		let fulfillment_status =
			sea_orm_active_enums::ClaimOrderFulfillmentStatusEnum::from(*&model.fulfillment_status);
		let r#type = sea_orm_active_enums::ClaimOrderTypeEnum::from(*&model.r#type);
		let new_table = ClaimOrderActiveModel {
			id: Set("id".to_string()),
			...
			refund_amount: Set(model.refund_amount.clone()),
...
impl UpdateClaimOrder {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ClaimOrderParmns>,
		json_data: Option<web::Json<UpdateClaimOrder>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ClaimOrder::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ClaimOrderActiveModel = table.into();
					if let Some(value) = &model.payment_status {
						let payment_status =
							Some(sea_orm_active_enums::ClaimOrderPaymentStatusEnum::from(*value));
						model_active.payment_status = Set(payment_status);
					}
					if let Some(value) = &model.fulfillment_status {
						let fulfillment_status = Some(
							sea_orm_active_enums::ClaimOrderFulfillmentStatusEnum::from(*value),
						);
						model_active.fulfillment_status = Set(fulfillment_status);
					}
					if let Some(value) = &model.r#type {
						let r#type = Some(sea_orm_active_enums::ClaimOrderTypeEnum::from(*value));
						model_active.r#type = Set(r#type);
					}
					if let Some(order_id) = &model.order_id {
						model_active.order_id = Set(order_id.clone());
					}
					model_active.shipping_address_id = Set(model.shipping_address_id);
					model_active.refund_amount = Set(model.refund_amount);
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteClaimOrder {
	pub id: String,
}

impl DeleteClaimOrder {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ClaimOrderParmns>,
	) -> impl Responder {
		let res = ClaimOrder::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
