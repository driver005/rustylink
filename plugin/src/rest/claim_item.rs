...
#[derive(Deserialize)]
pub struct ClaimItemParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/claim_item")
			.route("/", web::get().to(GetClaimItem::route))
			.route("/", web::post().to(CreateClaimItem::route))
			.route("/list", web::get().to(ListClaimItem::route))
			.route("/{id}", web::put().to(UpdateClaimItem::route))
			.route("/{id}", web::get().to(DeleteClaimItem::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetClaimItem {
	pub id: Option<String>,
	pub claim_order_id: Option<String>,
	pub item_id: Option<String>,
	pub variant_id: Option<String>,
	pub reason: Option<ClaimItemReasonEnum>,
...
impl GetClaimItem {
	async fn route(
		...
		json_data: Option<web::Json<GetClaimItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetClaimItem::default(),
		};
		let mut query = ClaimItem::find();
		if let Some(id) = &model.id {
			query = query.filter(ClaimItemColumn::Id.eq(id.clone()));
		}
		if let Some(claim_order_id) = &model.claim_order_id {
			query = query.filter(ClaimItemColumn::ClaimOrderId.eq(claim_order_id.clone()));
		}
		if let Some(item_id) = &model.item_id {
			query = query.filter(ClaimItemColumn::ItemId.eq(item_id.clone()));
		}
		if let Some(variant_id) = &model.variant_id {
			query = query.filter(ClaimItemColumn::VariantId.eq(variant_id.clone()));
		}
		if let Some(reason) = &model.reason {
			query = query.filter(ClaimItemColumn::Reason.eq(reason.clone()));
		}
		if let Some(note) = &model.note {
			query = query.filter(ClaimItemColumn::Note.eq(note.clone()));
		}
		if let Some(quantity) = &model.quantity {
			query = query.filter(ClaimItemColumn::Quantity.eq(quantity.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ClaimItemColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ClaimItemColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ClaimItemColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ClaimItemColumn::Metadata.eq(metadata.clone()));
		}
...
impl ListClaimItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListClaimItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListClaimItem::default(),
		};
		let mut query = ClaimItem::find();
		if let Some(id) = &model.id {
			query = query.filter(ClaimItemColumn::Id.eq(id.clone()));
...
impl CreateClaimItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateClaimItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let reason = sea_orm_active_enums::ClaimItemReasonEnum::from(*&model.reason);
		let new_table = ClaimItemActiveModel {
			id: Set("id".to_string()),
			claim_order_id: Set(model.claim_order_id.clone()),
			item_id: Set(model.item_id.clone()),
...
impl UpdateClaimItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ClaimItemParmns>,
		json_data: Option<web::Json<UpdateClaimItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ClaimItem::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ClaimItemActiveModel = table.into();
					if let Some(claim_order_id) = &model.claim_order_id {
						model_active.claim_order_id = Set(claim_order_id.clone());
					}
					if let Some(item_id) = &model.item_id {
						model_active.item_id = Set(item_id.clone());
					}
					if let Some(variant_id) = &model.variant_id {
						model_active.variant_id = Set(variant_id.clone());
					}
					if let Some(value) = &model.reason {
						let reason = Some(sea_orm_active_enums::ClaimItemReasonEnum::from(*value));
						model_active.reason = Set(reason);
					}
					model_active.note = Set(model.note);
					if let Some(quantity) = &model.quantity {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteClaimItem {
	pub id: String,
}

impl DeleteClaimItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ClaimItemParmns>,
	) -> impl Responder {
		let res = ClaimItem::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
