...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/refund")
			.route("/", web::get().to(GetRefund::route))
			.route("/", web::post().to(CreateRefund::route))
			.route("/list", web::get().to(ListRefund::route))
			.route("/{id}", web::put().to(UpdateRefund::route))
			.route("/{id}", web::get().to(DeleteRefund::route)),
	);
}
...
pub struct GetRefund {
	...
	pub reason: Option<RefundReasonEnum>,
...
impl GetRefund {
	async fn route(
		...
		json_data: Option<web::Json<GetRefund>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetRefund::default(),
		};
		let mut query = Refund::find();
		if let Some(id) = &model.id {
			query = query.filter(RefundColumn::Id.eq(id.clone()));
		}
		if let Some(order_id) = &model.order_id {
			query = query.filter(RefundColumn::OrderId.eq(order_id.clone()));
		}
		if let Some(amount) = &model.amount {
			query = query.filter(RefundColumn::Amount.eq(amount.clone()));
		}
		if let Some(note) = &model.note {
			query = query.filter(RefundColumn::Note.eq(note.clone()));
		}
		if let Some(reason) = &model.reason {
			query = query.filter(RefundColumn::Reason.eq(reason.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(RefundColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(RefundColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(RefundColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(idempotency_key) = &model.idempotency_key {
			query = query.filter(RefundColumn::IdempotencyKey.eq(idempotency_key.clone()));
		}
		if let Some(payment_id) = &model.payment_id {
			query = query.filter(RefundColumn::PaymentId.eq(payment_id.clone()));
		}
...
impl ListRefund {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListRefund>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListRefund::default(),
		};
		let mut query = Refund::find();
		if let Some(id) = &model.id {
			query = query.filter(RefundColumn::Id.eq(id.clone()));
...
impl CreateRefund {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateRefund>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let reason = sea_orm_active_enums::RefundReasonEnum::from(*&model.reason);
		let new_table = RefundActiveModel {
			id: Set("id".to_string()),
...
impl UpdateRefund {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<RefundParmns>,
		json_data: Option<web::Json<UpdateRefund>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Refund::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: RefundActiveModel = table.into();
					...
					if let Some(value) = &model.reason {
						let reason = Some(sea_orm_active_enums::RefundReasonEnum::from(*value));
...
impl DeleteRefund {
	async fn route(
		...
		let res = Refund::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
