...
#[derive(Deserialize)]
pub struct PriceListCustomerGroupsParmns {
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/price_list_customer_groups")
			.route("/", web::get().to(GetPriceListCustomerGroups::route))
			.route("/", web::post().to(CreatePriceListCustomerGroups::route))
			.route("/list", web::get().to(ListPriceListCustomerGroups::route))
			.route("/{id}", web::put().to(UpdatePriceListCustomerGroups::route))
			.route("/{id}", web::get().to(DeletePriceListCustomerGroups::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetPriceListCustomerGroups {
	pub price_list_id: Option<String>,
	pub customer_group_id: Option<String>,
}

impl GetPriceListCustomerGroups {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetPriceListCustomerGroups>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetPriceListCustomerGroups::default(),
		};
		let mut query = PriceListCustomerGroups::find();
		if let Some(price_list_id) = &model.price_list_id {
			query =
				query.filter(PriceListCustomerGroupsColumn::PriceListId.eq(price_list_id.clone()));
		}
		if let Some(customer_group_id) = &model.customer_group_id {
			query = query.filter(
				PriceListCustomerGroupsColumn::CustomerGroupId.eq(customer_group_id.clone()),
			);
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListPriceListCustomerGroups {
	pub price_list_id: Option<String>,
	pub customer_group_id: Option<String>,
}

impl ListPriceListCustomerGroups {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListPriceListCustomerGroups>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListPriceListCustomerGroups::default(),
		};
		let mut query = PriceListCustomerGroups::find();
		if let Some(price_list_id) = &model.price_list_id {
			query =
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreatePriceListCustomerGroups {}

impl CreatePriceListCustomerGroups {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreatePriceListCustomerGroups>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = PriceListCustomerGroupsActiveModel {
			price_list_id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdatePriceListCustomerGroups {}

impl UpdatePriceListCustomerGroups {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<PriceListCustomerGroupsParmns>,
		json_data: Option<web::Json<UpdatePriceListCustomerGroups>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = PriceListCustomerGroups::find_by_id((
			path.price_list_id.clone(),
		...
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: PriceListCustomerGroupsActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeletePriceListCustomerGroups {
	pub price_list_id: String,
	pub customer_group_id: String,
}

impl DeletePriceListCustomerGroups {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<PriceListCustomerGroupsParmns>,
	) -> impl Responder {
		let res = PriceListCustomerGroups::delete_by_id((
			path.price_list_id.clone(),
			path.customer_group_id.clone(),
		))
		.exec(db.get_ref())
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponsePriceListCustomerGroups {
	pub price_list_id: String,
	pub customer_group_id: String,
}
