...
#[derive(Deserialize)]
pub struct ProductVariantInventoryItemParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/product_variant_inventory_item")
			.route("/", web::get().to(GetProductVariantInventoryItem::route))
			.route("/", web::post().to(CreateProductVariantInventoryItem::route))
			.route("/list", web::get().to(ListProductVariantInventoryItem::route))
			.route("/{id}", web::put().to(UpdateProductVariantInventoryItem::route))
			.route("/{id}", web::get().to(DeleteProductVariantInventoryItem::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetProductVariantInventoryItem {
...
impl GetProductVariantInventoryItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetProductVariantInventoryItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetProductVariantInventoryItem::default(),
		};
		let mut query = ProductVariantInventoryItem::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductVariantInventoryItemColumn::Id.eq(id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query =
				query.filter(ProductVariantInventoryItemColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query =
				query.filter(ProductVariantInventoryItemColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(inventory_item_id) = &model.inventory_item_id {
			query = query.filter(
				ProductVariantInventoryItemColumn::InventoryItemId.eq(inventory_item_id.clone()),
			);
		}
		if let Some(variant_id) = &model.variant_id {
			query =
				query.filter(ProductVariantInventoryItemColumn::VariantId.eq(variant_id.clone()));
		}
		if let Some(required_quantity) = &model.required_quantity {
			query = query.filter(
				ProductVariantInventoryItemColumn::RequiredQuantity.eq(required_quantity.clone()),
			);
		}
		if let Some(deleted_at) = &model.deleted_at {
			query =
				query.filter(ProductVariantInventoryItemColumn::DeletedAt.eq(deleted_at.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListProductVariantInventoryItem {
	pub id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub inventory_item_id: Option<String>,
...
impl ListProductVariantInventoryItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListProductVariantInventoryItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListProductVariantInventoryItem::default(),
		};
		let mut query = ProductVariantInventoryItem::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductVariantInventoryItemColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateProductVariantInventoryItem {
...
impl CreateProductVariantInventoryItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateProductVariantInventoryItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ProductVariantInventoryItemActiveModel {
			id: Set("id".to_string()),
			created_at: Set(model.created_at.clone()),
			updated_at: Set(model.updated_at.clone()),
			inventory_item_id: Set(model.inventory_item_id.clone()),
			variant_id: Set(model.variant_id.clone()),
			required_quantity: Set(model.required_quantity.clone()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateProductVariantInventoryItem {
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub inventory_item_id: Option<String>,
	pub variant_id: Option<String>,
...
impl UpdateProductVariantInventoryItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductVariantInventoryItemParmns>,
		json_data: Option<web::Json<UpdateProductVariantInventoryItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		...
		let old_table =
			ProductVariantInventoryItem::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ProductVariantInventoryItemActiveModel = table.into();
					if let Some(created_at) = &model.created_at {
					...
					if let Some(inventory_item_id) = &model.inventory_item_id {
						model_active.inventory_item_id = Set(inventory_item_id.clone());
					}
					if let Some(variant_id) = &model.variant_id {
						model_active.variant_id = Set(variant_id.clone());
					}
					if let Some(required_quantity) = &model.required_quantity {
						model_active.required_quantity = Set(required_quantity.clone());
					}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteProductVariantInventoryItem {
	pub id: String,
}

impl DeleteProductVariantInventoryItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductVariantInventoryItemParmns>,
	) -> impl Responder {
		let res =
			ProductVariantInventoryItem::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseProductVariantInventoryItem {
	pub id: String,
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
	pub inventory_item_id: String,
