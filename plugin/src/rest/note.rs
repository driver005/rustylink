...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/note")
			.route("/", web::get().to(GetNote::route))
			.route("/", web::post().to(CreateNote::route))
			.route("/list", web::get().to(ListNote::route))
			.route("/{id}", web::put().to(UpdateNote::route))
			.route("/{id}", web::get().to(DeleteNote::route)),
	);
}
...
impl GetNote {
	async fn route(
		...
		json_data: Option<web::Json<GetNote>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetNote::default(),
		};
		let mut query = Note::find();
		if let Some(id) = &model.id {
			query = query.filter(NoteColumn::Id.eq(id.clone()));
		}
		if let Some(value) = &model.value {
			query = query.filter(NoteColumn::Value.eq(value.clone()));
		}
		if let Some(resource_type) = &model.resource_type {
			query = query.filter(NoteColumn::ResourceType.eq(resource_type.clone()));
		}
		if let Some(resource_id) = &model.resource_id {
			query = query.filter(NoteColumn::ResourceId.eq(resource_id.clone()));
		}
		if let Some(author_id) = &model.author_id {
			query = query.filter(NoteColumn::AuthorId.eq(author_id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(NoteColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(NoteColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(NoteColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(NoteColumn::Metadata.eq(metadata.clone()));
		}
...
impl ListNote {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListNote>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListNote::default(),
		};
		let mut query = Note::find();
		if let Some(id) = &model.id {
			query = query.filter(NoteColumn::Id.eq(id.clone()));
...
impl UpdateNote {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<NoteParmns>,
		json_data: Option<web::Json<UpdateNote>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Note::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: NoteActiveModel = table.into();
					if let Some(value) = &model.value {
					...
					model_active.author_id = Set(model.author_id);
					if let Some(created_at) = &model.created_at {
...
impl DeleteNote {
	async fn route(
		...
		let res = Note::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
