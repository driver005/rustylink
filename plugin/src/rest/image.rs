...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/image")
			.route("/", web::get().to(GetImage::route))
			.route("/", web::post().to(CreateImage::route))
			.route("/list", web::get().to(ListImage::route))
			.route("/{id}", web::put().to(UpdateImage::route))
			.route("/{id}", web::get().to(DeleteImage::route)),
	);
}
...
impl GetImage {
	async fn route(
		...
		json_data: Option<web::Json<GetImage>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetImage::default(),
		};
		let mut query = Image::find();
		if let Some(id) = &model.id {
			query = query.filter(ImageColumn::Id.eq(id.clone()));
		}
		if let Some(url) = &model.url {
			query = query.filter(ImageColumn::Url.eq(url.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ImageColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ImageColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ImageColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ImageColumn::Metadata.eq(metadata.clone()));
		}
...
impl ListImage {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListImage>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListImage::default(),
		};
		let mut query = Image::find();
		if let Some(id) = &model.id {
			query = query.filter(ImageColumn::Id.eq(id.clone()));
...
impl UpdateImage {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ImageParmns>,
		json_data: Option<web::Json<UpdateImage>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Image::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ImageActiveModel = table.into();
					if let Some(url) = &model.url {
...
impl DeleteImage {
	async fn route(
		...
		let res = Image::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
