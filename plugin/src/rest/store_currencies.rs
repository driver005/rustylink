...
#[derive(Deserialize)]
pub struct StoreCurrenciesParmns {
	store_id: String,
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/store_currencies")
			.route("/", web::get().to(GetStoreCurrencies::route))
			.route("/", web::post().to(CreateStoreCurrencies::route))
			.route("/list", web::get().to(ListStoreCurrencies::route))
			.route("/{id}", web::put().to(UpdateStoreCurrencies::route))
			.route("/{id}", web::get().to(DeleteStoreCurrencies::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetStoreCurrencies {
	pub store_id: Option<String>,
	pub currency_code: Option<String>,
}

impl GetStoreCurrencies {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetStoreCurrencies>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetStoreCurrencies::default(),
		};
		let mut query = StoreCurrencies::find();
		if let Some(store_id) = &model.store_id {
			query = query.filter(StoreCurrenciesColumn::StoreId.eq(store_id.clone()));
		}
		if let Some(currency_code) = &model.currency_code {
			query = query.filter(StoreCurrenciesColumn::CurrencyCode.eq(currency_code.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListStoreCurrencies {
	pub store_id: Option<String>,
	pub currency_code: Option<String>,
}

impl ListStoreCurrencies {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListStoreCurrencies>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListStoreCurrencies::default(),
		};
		let mut query = StoreCurrencies::find();
		if let Some(store_id) = &model.store_id {
			query = query.filter(StoreCurrenciesColumn::StoreId.eq(store_id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateStoreCurrencies {}

impl CreateStoreCurrencies {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateStoreCurrencies>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = StoreCurrenciesActiveModel {
			store_id: Set("id".to_string()),
			currency_code: Set("id".to_string()),
		};
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateStoreCurrencies {}

impl UpdateStoreCurrencies {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<StoreCurrenciesParmns>,
		json_data: Option<web::Json<UpdateStoreCurrencies>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		...
		let old_table =
			StoreCurrencies::find_by_id((path.store_id.clone(), path.currency_code.clone()))
				.one(db.get_ref())
				.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: StoreCurrenciesActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteStoreCurrencies {
	pub store_id: String,
	pub currency_code: String,
}

impl DeleteStoreCurrencies {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<StoreCurrenciesParmns>,
	) -> impl Responder {
		let res =
			StoreCurrencies::delete_by_id((path.store_id.clone(), path.currency_code.clone()))
				.exec(db.get_ref())
				.await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseStoreCurrencies {
	pub store_id: String,
	pub currency_code: String,
}
