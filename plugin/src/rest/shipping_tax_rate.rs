...
#[derive(Deserialize)]
pub struct ShippingTaxRateParmns {
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/shipping_tax_rate")
			.route("/", web::get().to(GetShippingTaxRate::route))
			.route("/", web::post().to(CreateShippingTaxRate::route))
			.route("/list", web::get().to(ListShippingTaxRate::route))
			.route("/{id}", web::put().to(UpdateShippingTaxRate::route))
			.route("/{id}", web::get().to(DeleteShippingTaxRate::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetShippingTaxRate {
...
impl GetShippingTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetShippingTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetShippingTaxRate::default(),
		};
		let mut query = ShippingTaxRate::find();
		if let Some(shipping_option_id) = &model.shipping_option_id {
			query = query
				.filter(ShippingTaxRateColumn::ShippingOptionId.eq(shipping_option_id.clone()));
		}
		if let Some(rate_id) = &model.rate_id {
			query = query.filter(ShippingTaxRateColumn::RateId.eq(rate_id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ShippingTaxRateColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ShippingTaxRateColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ShippingTaxRateColumn::Metadata.eq(metadata.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListShippingTaxRate {
	pub shipping_option_id: Option<String>,
	pub rate_id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
...
impl ListShippingTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListShippingTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListShippingTaxRate::default(),
		};
		let mut query = ShippingTaxRate::find();
		if let Some(shipping_option_id) = &model.shipping_option_id {
			query = query
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateShippingTaxRate {
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
	pub metadata: Option<Json>,
}

impl CreateShippingTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateShippingTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ShippingTaxRateActiveModel {
			shipping_option_id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateShippingTaxRate {
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub metadata: Option<Json>,
}

impl UpdateShippingTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ShippingTaxRateParmns>,
		json_data: Option<web::Json<UpdateShippingTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		...
		let old_table =
			ShippingTaxRate::find_by_id((path.shipping_option_id.clone(), path.rate_id.clone()))
				.one(db.get_ref())
				.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ShippingTaxRateActiveModel = table.into();
					if let Some(created_at) = &model.created_at {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteShippingTaxRate {
	pub shipping_option_id: String,
	pub rate_id: String,
}

impl DeleteShippingTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ShippingTaxRateParmns>,
	) -> impl Responder {
		let res =
			ShippingTaxRate::delete_by_id((path.shipping_option_id.clone(), path.rate_id.clone()))
				.exec(db.get_ref())
				.await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseShippingTaxRate {
	pub shipping_option_id: String,
	pub rate_id: String,
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
