...
#[derive(Deserialize)]
pub struct PaymentCollectionParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/payment_collection")
			.route("/", web::get().to(GetPaymentCollection::route))
			.route("/", web::post().to(CreatePaymentCollection::route))
			.route("/list", web::get().to(ListPaymentCollection::route))
			.route("/{id}", web::put().to(UpdatePaymentCollection::route))
			.route("/{id}", web::get().to(DeletePaymentCollection::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetPaymentCollection {
	pub id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub deleted_at: Option<DateTimeWithTimeZone>,
	pub r#type: Option<PaymentCollectionTypeEnum>,
	pub status: Option<PaymentCollectionStatusEnum>,
...
impl GetPaymentCollection {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetPaymentCollection>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetPaymentCollection::default(),
		};
		let mut query = PaymentCollection::find();
		if let Some(id) = &model.id {
			query = query.filter(PaymentCollectionColumn::Id.eq(id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(PaymentCollectionColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(PaymentCollectionColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(PaymentCollectionColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(r#type) = &model.r#type {
			query = query.filter(PaymentCollectionColumn::Type.eq(r#type.clone()));
		}
		if let Some(status) = &model.status {
			query = query.filter(PaymentCollectionColumn::Status.eq(status.clone()));
		}
		if let Some(description) = &model.description {
			query = query.filter(PaymentCollectionColumn::Description.eq(description.clone()));
		}
		if let Some(amount) = &model.amount {
			query = query.filter(PaymentCollectionColumn::Amount.eq(amount.clone()));
		}
		if let Some(authorized_amount) = &model.authorized_amount {
			query = query
				.filter(PaymentCollectionColumn::AuthorizedAmount.eq(authorized_amount.clone()));
		}
		if let Some(region_id) = &model.region_id {
			query = query.filter(PaymentCollectionColumn::RegionId.eq(region_id.clone()));
		}
		if let Some(currency_code) = &model.currency_code {
			query = query.filter(PaymentCollectionColumn::CurrencyCode.eq(currency_code.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(PaymentCollectionColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(created_by) = &model.created_by {
			query = query.filter(PaymentCollectionColumn::CreatedBy.eq(created_by.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListPaymentCollection {
	pub id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub deleted_at: Option<DateTimeWithTimeZone>,
...
impl ListPaymentCollection {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListPaymentCollection>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListPaymentCollection::default(),
		};
		let mut query = PaymentCollection::find();
		if let Some(id) = &model.id {
			query = query.filter(PaymentCollectionColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreatePaymentCollection {
...
impl CreatePaymentCollection {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreatePaymentCollection>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let r#type = sea_orm_active_enums::PaymentCollectionTypeEnum::from(*&model.r#type);
		let status = sea_orm_active_enums::PaymentCollectionStatusEnum::from(*&model.status);
		let new_table = PaymentCollectionActiveModel {
			id: Set("id".to_string()),
			...
			authorized_amount: Set(model.authorized_amount.clone()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdatePaymentCollection {
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub deleted_at: Option<DateTimeWithTimeZone>,
	pub r#type: Option<PaymentCollectionTypeEnum>,
...
impl UpdatePaymentCollection {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<PaymentCollectionParmns>,
		json_data: Option<web::Json<UpdatePaymentCollection>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = PaymentCollection::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: PaymentCollectionActiveModel = table.into();
					if let Some(created_at) = &model.created_at {
					...
					if let Some(value) = &model.r#type {
						let r#type =
							Some(sea_orm_active_enums::PaymentCollectionTypeEnum::from(*value));
						model_active.r#type = Set(r#type);
					}
					if let Some(value) = &model.status {
						let status =
							Some(sea_orm_active_enums::PaymentCollectionStatusEnum::from(*value));
						model_active.status = Set(status);
					}
					model_active.description = Set(model.description);
					if let Some(amount) = &model.amount {
						model_active.amount = Set(amount.clone());
					}
					model_active.authorized_amount = Set(model.authorized_amount);
					if let Some(region_id) = &model.region_id {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeletePaymentCollection {
	pub id: String,
}

impl DeletePaymentCollection {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<PaymentCollectionParmns>,
	) -> impl Responder {
		let res = PaymentCollection::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponsePaymentCollection {
	pub id: String,
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
	pub deleted_at: Option<DateTimeWithTimeZone>,
