...
#[derive(Deserialize)]
pub struct OnboardingStateParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/onboarding_state")
			.route("/", web::get().to(GetOnboardingState::route))
			.route("/", web::post().to(CreateOnboardingState::route))
			.route("/list", web::get().to(ListOnboardingState::route))
			.route("/{id}", web::put().to(UpdateOnboardingState::route))
			.route("/{id}", web::get().to(DeleteOnboardingState::route)),
	);
}
...
impl GetOnboardingState {
	async fn route(
		...
		json_data: Option<web::Json<GetOnboardingState>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetOnboardingState::default(),
		};
		let mut query = OnboardingState::find();
		if let Some(id) = &model.id {
			query = query.filter(OnboardingStateColumn::Id.eq(id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(OnboardingStateColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(OnboardingStateColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(current_step) = &model.current_step {
			query = query.filter(OnboardingStateColumn::CurrentStep.eq(current_step.clone()));
		}
		if let Some(is_complete) = &model.is_complete {
			query = query.filter(OnboardingStateColumn::IsComplete.eq(is_complete.clone()));
		}
		if let Some(product_id) = &model.product_id {
			query = query.filter(OnboardingStateColumn::ProductId.eq(product_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListOnboardingState {
	pub id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub current_step: Option<String>,
...
impl ListOnboardingState {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListOnboardingState>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListOnboardingState::default(),
		};
		let mut query = OnboardingState::find();
		if let Some(id) = &model.id {
			query = query.filter(OnboardingStateColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateOnboardingState {
...
impl CreateOnboardingState {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateOnboardingState>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = OnboardingStateActiveModel {
			id: Set("id".to_string()),
			created_at: Set(model.created_at.clone()),
			updated_at: Set(model.updated_at.clone()),
			current_step: Set(model.current_step.clone()),
			is_complete: Set(model.is_complete.clone()),
...
impl UpdateOnboardingState {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<OnboardingStateParmns>,
		json_data: Option<web::Json<UpdateOnboardingState>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = OnboardingState::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: OnboardingStateActiveModel = table.into();
					if let Some(created_at) = &model.created_at {
					...
					model_active.current_step = Set(model.current_step);
					if let Some(is_complete) = &model.is_complete {
						model_active.is_complete = Set(is_complete.clone());
					}
					model_active.product_id = Set(model.product_id);
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteOnboardingState {
	pub id: String,
}

impl DeleteOnboardingState {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<OnboardingStateParmns>,
	) -> impl Responder {
		let res = OnboardingState::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseOnboardingState {
	pub id: String,
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
	pub current_step: Option<String>,
