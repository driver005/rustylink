...
#[derive(Deserialize)]
pub struct LineItemAdjustmentParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/line_item_adjustment")
			.route("/", web::get().to(GetLineItemAdjustment::route))
			.route("/", web::post().to(CreateLineItemAdjustment::route))
			.route("/list", web::get().to(ListLineItemAdjustment::route))
			.route("/{id}", web::put().to(UpdateLineItemAdjustment::route))
			.route("/{id}", web::get().to(DeleteLineItemAdjustment::route)),
	);
}

#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct GetLineItemAdjustment {
...
impl GetLineItemAdjustment {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetLineItemAdjustment>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetLineItemAdjustment::default(),
		};
		let mut query = LineItemAdjustment::find();
		if let Some(id) = &model.id {
			query = query.filter(LineItemAdjustmentColumn::Id.eq(id.clone()));
		}
		if let Some(item_id) = &model.item_id {
			query = query.filter(LineItemAdjustmentColumn::ItemId.eq(item_id.clone()));
		}
		if let Some(description) = &model.description {
			query = query.filter(LineItemAdjustmentColumn::Description.eq(description.clone()));
		}
		if let Some(discount_id) = &model.discount_id {
			query = query.filter(LineItemAdjustmentColumn::DiscountId.eq(discount_id.clone()));
		}
		if let Some(amount) = &model.amount {
			query = query.filter(LineItemAdjustmentColumn::Amount.eq(amount.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(LineItemAdjustmentColumn::Metadata.eq(metadata.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct ListLineItemAdjustment {
	pub id: Option<String>,
	pub item_id: Option<String>,
	pub description: Option<String>,
	pub discount_id: Option<String>,
...
impl ListLineItemAdjustment {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListLineItemAdjustment>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListLineItemAdjustment::default(),
		};
		let mut query = LineItemAdjustment::find();
		if let Some(id) = &model.id {
			query = query.filter(LineItemAdjustmentColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct CreateLineItemAdjustment {
	pub item_id: String,
...
impl CreateLineItemAdjustment {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateLineItemAdjustment>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = LineItemAdjustmentActiveModel {
			id: Set("id".to_string()),
			item_id: Set(model.item_id.clone()),
			description: Set(model.description.clone()),
			discount_id: Set(model.discount_id.clone()),
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct UpdateLineItemAdjustment {
	pub item_id: Option<String>,
	pub description: Option<String>,
	pub discount_id: Option<String>,
	pub amount: Option<Decimal>,
...
impl UpdateLineItemAdjustment {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<LineItemAdjustmentParmns>,
		json_data: Option<web::Json<UpdateLineItemAdjustment>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = LineItemAdjustment::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: LineItemAdjustmentActiveModel = table.into();
					if let Some(item_id) = &model.item_id {
					...
					model_active.discount_id = Set(model.discount_id);
					if let Some(amount) = &model.amount {
...
pub struct DeleteLineItemAdjustment {
	pub id: String,
}

impl DeleteLineItemAdjustment {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<LineItemAdjustmentParmns>,
	) -> impl Responder {
		let res = LineItemAdjustment::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct ResponseLineItemAdjustment {
	pub id: String,
	pub item_id: String,
	pub description: String,
	pub discount_id: Option<String>,
