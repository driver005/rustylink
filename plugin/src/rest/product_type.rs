...
#[derive(Deserialize)]
pub struct ProductTypeParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/product_type")
			.route("/", web::get().to(GetProductType::route))
			.route("/", web::post().to(CreateProductType::route))
			.route("/list", web::get().to(ListProductType::route))
			.route("/{id}", web::put().to(UpdateProductType::route))
			.route("/{id}", web::get().to(DeleteProductType::route)),
	);
}
...
impl GetProductType {
	async fn route(
		...
		json_data: Option<web::Json<GetProductType>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetProductType::default(),
		};
		let mut query = ProductType::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductTypeColumn::Id.eq(id.clone()));
		}
		if let Some(value) = &model.value {
			query = query.filter(ProductTypeColumn::Value.eq(value.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ProductTypeColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ProductTypeColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ProductTypeColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ProductTypeColumn::Metadata.eq(metadata.clone()));
		}
...
impl ListProductType {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListProductType>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListProductType::default(),
		};
		let mut query = ProductType::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductTypeColumn::Id.eq(id.clone()));
...
impl CreateProductType {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateProductType>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ProductTypeActiveModel {
			id: Set("id".to_string()),
...
impl UpdateProductType {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductTypeParmns>,
		json_data: Option<web::Json<UpdateProductType>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ProductType::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ProductTypeActiveModel = table.into();
					if let Some(value) = &model.value {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteProductType {
	pub id: String,
}

impl DeleteProductType {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductTypeParmns>,
	) -> impl Responder {
		let res = ProductType::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseProductType {
	pub id: String,
	pub value: String,
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
