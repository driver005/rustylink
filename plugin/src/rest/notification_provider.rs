...
#[derive(Deserialize)]
pub struct NotificationProviderParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/notification_provider")
			.route("/", web::get().to(GetNotificationProvider::route))
			.route("/", web::post().to(CreateNotificationProvider::route))
			.route("/list", web::get().to(ListNotificationProvider::route))
			.route("/{id}", web::put().to(UpdateNotificationProvider::route))
			.route("/{id}", web::get().to(DeleteNotificationProvider::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetNotificationProvider {
	pub id: Option<String>,
	pub is_installed: Option<bool>,
}

impl GetNotificationProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetNotificationProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetNotificationProvider::default(),
		};
		let mut query = NotificationProvider::find();
		if let Some(id) = &model.id {
			query = query.filter(NotificationProviderColumn::Id.eq(id.clone()));
		}
		if let Some(is_installed) = &model.is_installed {
			query = query.filter(NotificationProviderColumn::IsInstalled.eq(is_installed.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListNotificationProvider {
	pub id: Option<String>,
	pub is_installed: Option<bool>,
}

impl ListNotificationProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListNotificationProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListNotificationProvider::default(),
		};
		let mut query = NotificationProvider::find();
		if let Some(id) = &model.id {
			query = query.filter(NotificationProviderColumn::Id.eq(id.clone()));
...
impl CreateNotificationProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateNotificationProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = NotificationProviderActiveModel {
			id: Set("id".to_string()),
...
impl UpdateNotificationProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<NotificationProviderParmns>,
		json_data: Option<web::Json<UpdateNotificationProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = NotificationProvider::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: NotificationProviderActiveModel = table.into();
					if let Some(is_installed) = &model.is_installed {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteNotificationProvider {
	pub id: String,
}

impl DeleteNotificationProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<NotificationProviderParmns>,
	) -> impl Responder {
		let res = NotificationProvider::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseNotificationProvider {
	pub id: String,
	pub is_installed: bool,
}
