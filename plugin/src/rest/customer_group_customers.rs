...
#[derive(Deserialize)]
pub struct CustomerGroupCustomersParmns {
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/customer_group_customers")
			.route("/", web::get().to(GetCustomerGroupCustomers::route))
			.route("/", web::post().to(CreateCustomerGroupCustomers::route))
			.route("/list", web::get().to(ListCustomerGroupCustomers::route))
			.route("/{id}", web::put().to(UpdateCustomerGroupCustomers::route))
			.route("/{id}", web::get().to(DeleteCustomerGroupCustomers::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetCustomerGroupCustomers {
	pub customer_group_id: Option<String>,
	pub customer_id: Option<String>,
}

impl GetCustomerGroupCustomers {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetCustomerGroupCustomers>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetCustomerGroupCustomers::default(),
		};
		let mut query = CustomerGroupCustomers::find();
		if let Some(customer_group_id) = &model.customer_group_id {
			query = query.filter(
				CustomerGroupCustomersColumn::CustomerGroupId.eq(customer_group_id.clone()),
			);
		}
		if let Some(customer_id) = &model.customer_id {
			query = query.filter(CustomerGroupCustomersColumn::CustomerId.eq(customer_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListCustomerGroupCustomers {
	pub customer_group_id: Option<String>,
	pub customer_id: Option<String>,
}

impl ListCustomerGroupCustomers {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListCustomerGroupCustomers>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListCustomerGroupCustomers::default(),
		};
		let mut query = CustomerGroupCustomers::find();
		if let Some(customer_group_id) = &model.customer_group_id {
			query = query.filter(
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateCustomerGroupCustomers {}

impl CreateCustomerGroupCustomers {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateCustomerGroupCustomers>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = CustomerGroupCustomersActiveModel {
			customer_group_id: Set("id".to_string()),
			customer_id: Set("id".to_string()),
		};
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateCustomerGroupCustomers {}

impl UpdateCustomerGroupCustomers {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<CustomerGroupCustomersParmns>,
		json_data: Option<web::Json<UpdateCustomerGroupCustomers>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = CustomerGroupCustomers::find_by_id((
			path.customer_group_id.clone(),
			path.customer_id.clone(),
		))
		.one(db.get_ref())
		.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: CustomerGroupCustomersActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteCustomerGroupCustomers {
	pub customer_group_id: String,
	pub customer_id: String,
}

impl DeleteCustomerGroupCustomers {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<CustomerGroupCustomersParmns>,
	) -> impl Responder {
		let res = CustomerGroupCustomers::delete_by_id((
			path.customer_group_id.clone(),
			path.customer_id.clone(),
		))
		.exec(db.get_ref())
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseCustomerGroupCustomers {
	pub customer_group_id: String,
	pub customer_id: String,
}
