...
#[derive(Deserialize)]
pub struct FulfillmentItemParmns {
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/fulfillment_item")
			.route("/", web::get().to(GetFulfillmentItem::route))
			.route("/", web::post().to(CreateFulfillmentItem::route))
			.route("/list", web::get().to(ListFulfillmentItem::route))
			.route("/{id}", web::put().to(UpdateFulfillmentItem::route))
			.route("/{id}", web::get().to(DeleteFulfillmentItem::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetFulfillmentItem {
	pub fulfillment_id: Option<String>,
	pub item_id: Option<String>,
	pub quantity: Option<i32>,
}

impl GetFulfillmentItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetFulfillmentItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetFulfillmentItem::default(),
		};
		let mut query = FulfillmentItem::find();
		if let Some(fulfillment_id) = &model.fulfillment_id {
			query = query.filter(FulfillmentItemColumn::FulfillmentId.eq(fulfillment_id.clone()));
		}
		if let Some(item_id) = &model.item_id {
			query = query.filter(FulfillmentItemColumn::ItemId.eq(item_id.clone()));
		}
		if let Some(quantity) = &model.quantity {
			query = query.filter(FulfillmentItemColumn::Quantity.eq(quantity.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListFulfillmentItem {
	pub fulfillment_id: Option<String>,
	pub item_id: Option<String>,
	pub quantity: Option<i32>,
}

impl ListFulfillmentItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListFulfillmentItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListFulfillmentItem::default(),
		};
		let mut query = FulfillmentItem::find();
		if let Some(fulfillment_id) = &model.fulfillment_id {
			query = query.filter(FulfillmentItemColumn::FulfillmentId.eq(fulfillment_id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateFulfillmentItem {
	pub quantity: i32,
}

impl CreateFulfillmentItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateFulfillmentItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = FulfillmentItemActiveModel {
			fulfillment_id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateFulfillmentItem {
	pub quantity: Option<i32>,
}

impl UpdateFulfillmentItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<FulfillmentItemParmns>,
		json_data: Option<web::Json<UpdateFulfillmentItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		...
		let old_table =
			FulfillmentItem::find_by_id((path.fulfillment_id.clone(), path.item_id.clone()))
				.one(db.get_ref())
				.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: FulfillmentItemActiveModel = table.into();
					if let Some(quantity) = &model.quantity {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteFulfillmentItem {
	pub fulfillment_id: String,
	pub item_id: String,
}

impl DeleteFulfillmentItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<FulfillmentItemParmns>,
	) -> impl Responder {
		let res =
			FulfillmentItem::delete_by_id((path.fulfillment_id.clone(), path.item_id.clone()))
				.exec(db.get_ref())
				.await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseFulfillmentItem {
	pub fulfillment_id: String,
	pub item_id: String,
	pub quantity: i32,
}
