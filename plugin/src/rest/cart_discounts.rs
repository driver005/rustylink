...
#[derive(Deserialize)]
pub struct CartDiscountsParmns {
	cart_id: String,
	discount_id: String,
}
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/cart_discounts")
			.route("/", web::get().to(GetCartDiscounts::route))
			.route("/", web::post().to(CreateCartDiscounts::route))
			.route("/list", web::get().to(ListCartDiscounts::route))
			.route("/{id}", web::put().to(UpdateCartDiscounts::route))
			.route("/{id}", web::get().to(DeleteCartDiscounts::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetCartDiscounts {
	pub cart_id: Option<String>,
	pub discount_id: Option<String>,
}

impl GetCartDiscounts {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetCartDiscounts>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetCartDiscounts::default(),
		};
		let mut query = CartDiscounts::find();
		if let Some(cart_id) = &model.cart_id {
			query = query.filter(CartDiscountsColumn::CartId.eq(cart_id.clone()));
		}
		if let Some(discount_id) = &model.discount_id {
			query = query.filter(CartDiscountsColumn::DiscountId.eq(discount_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListCartDiscounts {
	pub cart_id: Option<String>,
	pub discount_id: Option<String>,
}

impl ListCartDiscounts {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListCartDiscounts>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListCartDiscounts::default(),
		};
		let mut query = CartDiscounts::find();
		if let Some(cart_id) = &model.cart_id {
			query = query.filter(CartDiscountsColumn::CartId.eq(cart_id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateCartDiscounts {}

impl CreateCartDiscounts {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateCartDiscounts>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = CartDiscountsActiveModel {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateCartDiscounts {}

impl UpdateCartDiscounts {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<CartDiscountsParmns>,
		json_data: Option<web::Json<UpdateCartDiscounts>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = CartDiscounts::find_by_id((path.cart_id.clone(), path.discount_id.clone()))
			.one(db.get_ref())
			.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: CartDiscountsActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteCartDiscounts {
	pub cart_id: String,
	pub discount_id: String,
}

impl DeleteCartDiscounts {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<CartDiscountsParmns>,
	) -> impl Responder {
		let res = CartDiscounts::delete_by_id((path.cart_id.clone(), path.discount_id.clone()))
			.exec(db.get_ref())
			.await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseCartDiscounts {
	pub cart_id: String,
	pub discount_id: String,
}
