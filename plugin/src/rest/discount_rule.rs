...
#[derive(Deserialize)]
pub struct DiscountRuleParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/discount_rule")
			.route("/", web::get().to(GetDiscountRule::route))
			.route("/", web::post().to(CreateDiscountRule::route))
			.route("/list", web::get().to(ListDiscountRule::route))
			.route("/{id}", web::put().to(UpdateDiscountRule::route))
			.route("/{id}", web::get().to(DeleteDiscountRule::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetDiscountRule {
	pub id: Option<String>,
	pub description: Option<String>,
	pub r#type: Option<DiscountRuleTypeEnum>,
...
impl GetDiscountRule {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetDiscountRule>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetDiscountRule::default(),
		};
		let mut query = DiscountRule::find();
		if let Some(id) = &model.id {
			query = query.filter(DiscountRuleColumn::Id.eq(id.clone()));
		}
		if let Some(description) = &model.description {
			query = query.filter(DiscountRuleColumn::Description.eq(description.clone()));
		}
		if let Some(r#type) = &model.r#type {
			query = query.filter(DiscountRuleColumn::Type.eq(r#type.clone()));
		}
		if let Some(value) = &model.value {
			query = query.filter(DiscountRuleColumn::Value.eq(value.clone()));
		}
		if let Some(allocation) = &model.allocation {
			query = query.filter(DiscountRuleColumn::Allocation.eq(allocation.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(DiscountRuleColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(DiscountRuleColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(DiscountRuleColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(DiscountRuleColumn::Metadata.eq(metadata.clone()));
		}
...
impl ListDiscountRule {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListDiscountRule>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListDiscountRule::default(),
		};
		let mut query = DiscountRule::find();
		if let Some(id) = &model.id {
			query = query.filter(DiscountRuleColumn::Id.eq(id.clone()));
...
impl CreateDiscountRule {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateDiscountRule>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let r#type = sea_orm_active_enums::DiscountRuleTypeEnum::from(*&model.r#type);
		let allocation = if let Some(value) = &model.allocation {
			Some(sea_orm_active_enums::DiscountRuleAllocationEnum::from(*value))
		} else {
			None
		};
		let new_table = DiscountRuleActiveModel {
			id: Set("id".to_string()),
...
impl UpdateDiscountRule {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<DiscountRuleParmns>,
		json_data: Option<web::Json<UpdateDiscountRule>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = DiscountRule::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: DiscountRuleActiveModel = table.into();
					model_active.description = Set(model.description);
					if let Some(value) = &model.r#type {
						let r#type = Some(sea_orm_active_enums::DiscountRuleTypeEnum::from(*value));
						model_active.r#type = Set(r#type);
					}
					if let Some(value) = &model.value {
						model_active.value = Set(value.clone());
					}
					if let Some(value) = &model.allocation {
						let allocation =
							Some(sea_orm_active_enums::DiscountRuleAllocationEnum::from(*value));
						model_active.allocation = Set(allocation);
					}
					if let Some(created_at) = &model.created_at {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteDiscountRule {
	pub id: String,
}

impl DeleteDiscountRule {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<DiscountRuleParmns>,
	) -> impl Responder {
		let res = DiscountRule::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseDiscountRule {
	pub id: String,
	pub description: Option<String>,
	pub r#type: DiscountRuleTypeEnum,
	pub value: i32,
