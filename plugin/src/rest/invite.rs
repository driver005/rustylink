...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/invite")
			.route("/", web::get().to(GetInvite::route))
			.route("/", web::post().to(CreateInvite::route))
			.route("/list", web::get().to(ListInvite::route))
			.route("/{id}", web::put().to(UpdateInvite::route))
			.route("/{id}", web::get().to(DeleteInvite::route)),
	);
}
...
impl GetInvite {
	async fn route(
		...
		json_data: Option<web::Json<GetInvite>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetInvite::default(),
		};
		let mut query = Invite::find();
		if let Some(id) = &model.id {
			query = query.filter(InviteColumn::Id.eq(id.clone()));
		}
		if let Some(user_email) = &model.user_email {
			query = query.filter(InviteColumn::UserEmail.eq(user_email.clone()));
		}
		if let Some(role) = &model.role {
			query = query.filter(InviteColumn::Role.eq(role.clone()));
		}
		if let Some(accepted) = &model.accepted {
			query = query.filter(InviteColumn::Accepted.eq(accepted.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(InviteColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(InviteColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(InviteColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(InviteColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(token) = &model.token {
			query = query.filter(InviteColumn::Token.eq(token.clone()));
		}
		if let Some(expires_at) = &model.expires_at {
			query = query.filter(InviteColumn::ExpiresAt.eq(expires_at.clone()));
		}
...
impl ListInvite {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListInvite>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListInvite::default(),
		};
		let mut query = Invite::find();
		if let Some(id) = &model.id {
			query = query.filter(InviteColumn::Id.eq(id.clone()));
...
impl CreateInvite {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateInvite>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let role = if let Some(value) = &model.role {
			Some(sea_orm_active_enums::InviteRoleEnum::from(*value))
		} else {
			None
		};
		let new_table = InviteActiveModel {
			id: Set("id".to_string()),
			user_email: Set(model.user_email.clone()),
			role: Set(role),
			accepted: Set(model.accepted.clone()),
			...
			token: Set(model.token.clone()),
			expires_at: Set(model.expires_at.clone()),
		};
...
impl UpdateInvite {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<InviteParmns>,
		json_data: Option<web::Json<UpdateInvite>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Invite::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: InviteActiveModel = table.into();
					if let Some(user_email) = &model.user_email {
						model_active.user_email = Set(user_email.clone());
					}
					if let Some(value) = &model.role {
						let role = Some(sea_orm_active_enums::InviteRoleEnum::from(*value));
						model_active.role = Set(role);
					}
					if let Some(accepted) = &model.accepted {
						model_active.accepted = Set(accepted.clone());
					}
					if let Some(created_at) = &model.created_at {
					...
					if let Some(token) = &model.token {
						model_active.token = Set(token.clone());
					}
					if let Some(expires_at) = &model.expires_at {
						model_active.expires_at = Set(expires_at.clone());
					}
...
impl DeleteInvite {
	async fn route(
		...
		let res = Invite::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
