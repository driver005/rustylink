...
#[derive(Deserialize)]
pub struct ProductSalesChannelParmns {
	product_id: String,
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/product_sales_channel")
			.route("/", web::get().to(GetProductSalesChannel::route))
			.route("/", web::post().to(CreateProductSalesChannel::route))
			.route("/list", web::get().to(ListProductSalesChannel::route))
			.route("/{id}", web::put().to(UpdateProductSalesChannel::route))
			.route("/{id}", web::get().to(DeleteProductSalesChannel::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetProductSalesChannel {
	pub product_id: Option<String>,
	pub sales_channel_id: Option<String>,
}

impl GetProductSalesChannel {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetProductSalesChannel>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetProductSalesChannel::default(),
		};
		let mut query = ProductSalesChannel::find();
		if let Some(product_id) = &model.product_id {
			query = query.filter(ProductSalesChannelColumn::ProductId.eq(product_id.clone()));
		}
		if let Some(sales_channel_id) = &model.sales_channel_id {
			query = query
				.filter(ProductSalesChannelColumn::SalesChannelId.eq(sales_channel_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListProductSalesChannel {
	pub product_id: Option<String>,
	pub sales_channel_id: Option<String>,
}

impl ListProductSalesChannel {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListProductSalesChannel>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListProductSalesChannel::default(),
		};
		let mut query = ProductSalesChannel::find();
		if let Some(product_id) = &model.product_id {
			query = query.filter(ProductSalesChannelColumn::ProductId.eq(product_id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateProductSalesChannel {}

impl CreateProductSalesChannel {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateProductSalesChannel>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ProductSalesChannelActiveModel {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateProductSalesChannel {}

impl UpdateProductSalesChannel {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductSalesChannelParmns>,
		json_data: Option<web::Json<UpdateProductSalesChannel>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ProductSalesChannel::find_by_id((
		...
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ProductSalesChannelActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteProductSalesChannel {
	pub product_id: String,
	pub sales_channel_id: String,
}

impl DeleteProductSalesChannel {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductSalesChannelParmns>,
	) -> impl Responder {
		let res = ProductSalesChannel::delete_by_id((
			path.product_id.clone(),
			path.sales_channel_id.clone(),
		))
		.exec(db.get_ref())
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseProductSalesChannel {
	pub product_id: String,
	pub sales_channel_id: String,
}
