...
#[derive(Deserialize)]
pub struct PublishableApiKeySalesChannelParmns {
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/publishable_api_key_sales_channel")
			.route("/", web::get().to(GetPublishableApiKeySalesChannel::route))
			.route("/", web::post().to(CreatePublishableApiKeySalesChannel::route))
			.route("/list", web::get().to(ListPublishableApiKeySalesChannel::route))
			.route("/{id}", web::put().to(UpdatePublishableApiKeySalesChannel::route))
			.route("/{id}", web::get().to(DeletePublishableApiKeySalesChannel::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetPublishableApiKeySalesChannel {
	pub sales_channel_id: Option<String>,
	pub publishable_key_id: Option<String>,
...
impl GetPublishableApiKeySalesChannel {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetPublishableApiKeySalesChannel>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetPublishableApiKeySalesChannel::default(),
		};
		let mut query = PublishableApiKeySalesChannel::find();
		if let Some(sales_channel_id) = &model.sales_channel_id {
			query = query.filter(
				PublishableApiKeySalesChannelColumn::SalesChannelId.eq(sales_channel_id.clone()),
			);
		}
		if let Some(publishable_key_id) = &model.publishable_key_id {
			query = query.filter(
				PublishableApiKeySalesChannelColumn::PublishableKeyId
					.eq(publishable_key_id.clone()),
			);
		}
		if let Some(id) = &model.id {
			query = query.filter(PublishableApiKeySalesChannelColumn::Id.eq(id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query =
				query.filter(PublishableApiKeySalesChannelColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query =
				query.filter(PublishableApiKeySalesChannelColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query =
				query.filter(PublishableApiKeySalesChannelColumn::DeletedAt.eq(deleted_at.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListPublishableApiKeySalesChannel {
	pub sales_channel_id: Option<String>,
	pub publishable_key_id: Option<String>,
	pub id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
...
impl ListPublishableApiKeySalesChannel {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListPublishableApiKeySalesChannel>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListPublishableApiKeySalesChannel::default(),
		};
		let mut query = PublishableApiKeySalesChannel::find();
		if let Some(sales_channel_id) = &model.sales_channel_id {
			query = query.filter(
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreatePublishableApiKeySalesChannel {
	pub id: String,
...
impl CreatePublishableApiKeySalesChannel {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreatePublishableApiKeySalesChannel>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = PublishableApiKeySalesChannelActiveModel {
			sales_channel_id: Set("id".to_string()),
			publishable_key_id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdatePublishableApiKeySalesChannel {
	pub id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub deleted_at: Option<DateTimeWithTimeZone>,
...
impl UpdatePublishableApiKeySalesChannel {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<PublishableApiKeySalesChannelParmns>,
		json_data: Option<web::Json<UpdatePublishableApiKeySalesChannel>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = PublishableApiKeySalesChannel::find_by_id((
			path.sales_channel_id.clone(),
			path.publishable_key_id.clone(),
		))
		.one(db.get_ref())
		.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: PublishableApiKeySalesChannelActiveModel = table.into();
					if let Some(id) = &model.id {
						model_active.id = Set(id.clone());
					}
					if let Some(created_at) = &model.created_at {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeletePublishableApiKeySalesChannel {
	pub sales_channel_id: String,
	pub publishable_key_id: String,
}

impl DeletePublishableApiKeySalesChannel {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<PublishableApiKeySalesChannelParmns>,
	) -> impl Responder {
		let res = PublishableApiKeySalesChannel::delete_by_id((
			path.sales_channel_id.clone(),
			path.publishable_key_id.clone(),
		))
		.exec(db.get_ref())
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponsePublishableApiKeySalesChannel {
	pub sales_channel_id: String,
	pub publishable_key_id: String,
	pub id: String,
	pub created_at: DateTimeWithTimeZone,
