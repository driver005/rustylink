...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/return_item")
			.route("/", web::get().to(GetReturnItem::route))
			.route("/", web::post().to(CreateReturnItem::route))
			.route("/list", web::get().to(ListReturnItem::route))
			.route("/{id}", web::put().to(UpdateReturnItem::route))
			.route("/{id}", web::get().to(DeleteReturnItem::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetReturnItem {
	pub return_id: Option<String>,
	pub item_id: Option<String>,
	pub quantity: Option<i32>,
	pub is_requested: Option<bool>,
...
impl GetReturnItem {
	async fn route(
		...
		json_data: Option<web::Json<GetReturnItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetReturnItem::default(),
		};
		let mut query = ReturnItem::find();
		if let Some(return_id) = &model.return_id {
			query = query.filter(ReturnItemColumn::ReturnId.eq(return_id.clone()));
		}
		if let Some(item_id) = &model.item_id {
			query = query.filter(ReturnItemColumn::ItemId.eq(item_id.clone()));
		}
		if let Some(quantity) = &model.quantity {
			query = query.filter(ReturnItemColumn::Quantity.eq(quantity.clone()));
		}
		if let Some(is_requested) = &model.is_requested {
			query = query.filter(ReturnItemColumn::IsRequested.eq(is_requested.clone()));
		}
		if let Some(requested_quantity) = &model.requested_quantity {
			query =
				query.filter(ReturnItemColumn::RequestedQuantity.eq(requested_quantity.clone()));
		}
		if let Some(received_quantity) = &model.received_quantity {
			query = query.filter(ReturnItemColumn::ReceivedQuantity.eq(received_quantity.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ReturnItemColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(reason_id) = &model.reason_id {
			query = query.filter(ReturnItemColumn::ReasonId.eq(reason_id.clone()));
		}
		if let Some(note) = &model.note {
			query = query.filter(ReturnItemColumn::Note.eq(note.clone()));
		}
...
impl ListReturnItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListReturnItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListReturnItem::default(),
		};
		let mut query = ReturnItem::find();
		if let Some(return_id) = &model.return_id {
			query = query.filter(ReturnItemColumn::ReturnId.eq(return_id.clone()));
...
impl CreateReturnItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateReturnItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ReturnItemActiveModel {
			return_id: Set("id".to_string()),
			item_id: Set("id".to_string()),
			quantity: Set(model.quantity.clone()),
			is_requested: Set(model.is_requested.clone()),
			requested_quantity: Set(model.requested_quantity.clone()),
			received_quantity: Set(model.received_quantity.clone()),
			metadata: Set(model.metadata.clone()),
			reason_id: Set(model.reason_id.clone()),
...
impl UpdateReturnItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ReturnItemParmns>,
		json_data: Option<web::Json<UpdateReturnItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ReturnItem::find_by_id((path.return_id.clone(), path.item_id.clone()))
			.one(db.get_ref())
			.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ReturnItemActiveModel = table.into();
					if let Some(quantity) = &model.quantity {
						model_active.quantity = Set(quantity.clone());
					}
					if let Some(is_requested) = &model.is_requested {
						model_active.is_requested = Set(is_requested.clone());
					}
					model_active.requested_quantity = Set(model.requested_quantity);
					model_active.received_quantity = Set(model.received_quantity);
					model_active.metadata = Set(model.metadata);
					model_active.reason_id = Set(model.reason_id);
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteReturnItem {
	pub return_id: String,
	pub item_id: String,
}

impl DeleteReturnItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ReturnItemParmns>,
	) -> impl Responder {
		let res = ReturnItem::delete_by_id((path.return_id.clone(), path.item_id.clone()))
			.exec(db.get_ref())
			.await;
		match res {
