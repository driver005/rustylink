...
#[derive(Deserialize)]
pub struct ProductTaxRateParmns {
	product_id: String,
	rate_id: String,
}
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/product_tax_rate")
			.route("/", web::get().to(GetProductTaxRate::route))
			.route("/", web::post().to(CreateProductTaxRate::route))
			.route("/list", web::get().to(ListProductTaxRate::route))
			.route("/{id}", web::put().to(UpdateProductTaxRate::route))
			.route("/{id}", web::get().to(DeleteProductTaxRate::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetProductTaxRate {
...
impl GetProductTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetProductTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetProductTaxRate::default(),
		};
		let mut query = ProductTaxRate::find();
		if let Some(product_id) = &model.product_id {
			query = query.filter(ProductTaxRateColumn::ProductId.eq(product_id.clone()));
		}
		if let Some(rate_id) = &model.rate_id {
			query = query.filter(ProductTaxRateColumn::RateId.eq(rate_id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ProductTaxRateColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ProductTaxRateColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ProductTaxRateColumn::Metadata.eq(metadata.clone()));
		}
...
impl ListProductTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListProductTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListProductTaxRate::default(),
		};
		let mut query = ProductTaxRate::find();
		if let Some(product_id) = &model.product_id {
			query = query.filter(ProductTaxRateColumn::ProductId.eq(product_id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateProductTaxRate {
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
	pub metadata: Option<Json>,
}

impl CreateProductTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateProductTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ProductTaxRateActiveModel {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateProductTaxRate {
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub metadata: Option<Json>,
}

impl UpdateProductTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductTaxRateParmns>,
		json_data: Option<web::Json<UpdateProductTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ProductTaxRate::find_by_id((path.product_id.clone(), path.rate_id.clone()))
			.one(db.get_ref())
			.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ProductTaxRateActiveModel = table.into();
					if let Some(created_at) = &model.created_at {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteProductTaxRate {
	pub product_id: String,
	pub rate_id: String,
}

impl DeleteProductTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductTaxRateParmns>,
	) -> impl Responder {
		let res = ProductTaxRate::delete_by_id((path.product_id.clone(), path.rate_id.clone()))
			.exec(db.get_ref())
			.await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseProductTaxRate {
	pub product_id: String,
	pub rate_id: String,
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
