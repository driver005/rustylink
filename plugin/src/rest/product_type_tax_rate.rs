...
#[derive(Deserialize)]
pub struct ProductTypeTaxRateParmns {
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/product_type_tax_rate")
			.route("/", web::get().to(GetProductTypeTaxRate::route))
			.route("/", web::post().to(CreateProductTypeTaxRate::route))
			.route("/list", web::get().to(ListProductTypeTaxRate::route))
			.route("/{id}", web::put().to(UpdateProductTypeTaxRate::route))
			.route("/{id}", web::get().to(DeleteProductTypeTaxRate::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetProductTypeTaxRate {
...
impl GetProductTypeTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetProductTypeTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetProductTypeTaxRate::default(),
		};
		let mut query = ProductTypeTaxRate::find();
		if let Some(product_type_id) = &model.product_type_id {
			query =
				query.filter(ProductTypeTaxRateColumn::ProductTypeId.eq(product_type_id.clone()));
		}
		if let Some(rate_id) = &model.rate_id {
			query = query.filter(ProductTypeTaxRateColumn::RateId.eq(rate_id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ProductTypeTaxRateColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ProductTypeTaxRateColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ProductTypeTaxRateColumn::Metadata.eq(metadata.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListProductTypeTaxRate {
	pub product_type_id: Option<String>,
	pub rate_id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
...
impl ListProductTypeTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListProductTypeTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListProductTypeTaxRate::default(),
		};
		let mut query = ProductTypeTaxRate::find();
		if let Some(product_type_id) = &model.product_type_id {
			query =
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateProductTypeTaxRate {
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
	pub metadata: Option<Json>,
}

impl CreateProductTypeTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateProductTypeTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ProductTypeTaxRateActiveModel {
			product_type_id: Set("id".to_string()),
			rate_id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateProductTypeTaxRate {
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub metadata: Option<Json>,
}

impl UpdateProductTypeTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductTypeTaxRateParmns>,
		json_data: Option<web::Json<UpdateProductTypeTaxRate>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		...
		let old_table =
			ProductTypeTaxRate::find_by_id((path.product_type_id.clone(), path.rate_id.clone()))
				.one(db.get_ref())
				.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ProductTypeTaxRateActiveModel = table.into();
					if let Some(created_at) = &model.created_at {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteProductTypeTaxRate {
	pub product_type_id: String,
	pub rate_id: String,
}

impl DeleteProductTypeTaxRate {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductTypeTaxRateParmns>,
	) -> impl Responder {
		let res =
			ProductTypeTaxRate::delete_by_id((path.product_type_id.clone(), path.rate_id.clone()))
				.exec(db.get_ref())
				.await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseProductTypeTaxRate {
	pub product_type_id: String,
	pub rate_id: String,
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
