...
#[derive(Deserialize)]
pub struct ShippingProfileParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/shipping_profile")
			.route("/", web::get().to(GetShippingProfile::route))
			.route("/", web::post().to(CreateShippingProfile::route))
			.route("/list", web::get().to(ListShippingProfile::route))
			.route("/{id}", web::put().to(UpdateShippingProfile::route))
			.route("/{id}", web::get().to(DeleteShippingProfile::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetShippingProfile {
	pub id: Option<String>,
	pub name: Option<String>,
	pub r#type: Option<ShippingProfileTypeEnum>,
...
impl GetShippingProfile {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetShippingProfile>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetShippingProfile::default(),
		};
		let mut query = ShippingProfile::find();
		if let Some(id) = &model.id {
			query = query.filter(ShippingProfileColumn::Id.eq(id.clone()));
		}
		if let Some(name) = &model.name {
			query = query.filter(ShippingProfileColumn::Name.eq(name.clone()));
		}
		if let Some(r#type) = &model.r#type {
			query = query.filter(ShippingProfileColumn::Type.eq(r#type.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ShippingProfileColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ShippingProfileColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ShippingProfileColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ShippingProfileColumn::Metadata.eq(metadata.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListShippingProfile {
	pub id: Option<String>,
	pub name: Option<String>,
	pub r#type: Option<ShippingProfileTypeEnum>,
	pub created_at: Option<DateTimeWithTimeZone>,
...
impl ListShippingProfile {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListShippingProfile>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListShippingProfile::default(),
		};
		let mut query = ShippingProfile::find();
		if let Some(id) = &model.id {
			query = query.filter(ShippingProfileColumn::Id.eq(id.clone()));
...
impl CreateShippingProfile {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateShippingProfile>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let r#type = sea_orm_active_enums::ShippingProfileTypeEnum::from(*&model.r#type);
		let new_table = ShippingProfileActiveModel {
			id: Set("id".to_string()),
...
impl UpdateShippingProfile {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ShippingProfileParmns>,
		json_data: Option<web::Json<UpdateShippingProfile>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ShippingProfile::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ShippingProfileActiveModel = table.into();
					if let Some(name) = &model.name {
						model_active.name = Set(name.clone());
					}
					if let Some(value) = &model.r#type {
						let r#type =
							Some(sea_orm_active_enums::ShippingProfileTypeEnum::from(*value));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteShippingProfile {
	pub id: String,
}

impl DeleteShippingProfile {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ShippingProfileParmns>,
	) -> impl Responder {
		let res = ShippingProfile::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseShippingProfile {
	pub id: String,
	pub name: String,
	pub r#type: ShippingProfileTypeEnum,
	pub created_at: DateTimeWithTimeZone,
