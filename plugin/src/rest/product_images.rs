...
#[derive(Deserialize)]
pub struct ProductImagesParmns {
	product_id: String,
	image_id: String,
}
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/product_images")
			.route("/", web::get().to(GetProductImages::route))
			.route("/", web::post().to(CreateProductImages::route))
			.route("/list", web::get().to(ListProductImages::route))
			.route("/{id}", web::put().to(UpdateProductImages::route))
			.route("/{id}", web::get().to(DeleteProductImages::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetProductImages {
	pub product_id: Option<String>,
	pub image_id: Option<String>,
}

impl GetProductImages {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetProductImages>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetProductImages::default(),
		};
		let mut query = ProductImages::find();
		if let Some(product_id) = &model.product_id {
			query = query.filter(ProductImagesColumn::ProductId.eq(product_id.clone()));
		}
		if let Some(image_id) = &model.image_id {
			query = query.filter(ProductImagesColumn::ImageId.eq(image_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListProductImages {
	pub product_id: Option<String>,
	pub image_id: Option<String>,
}

impl ListProductImages {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListProductImages>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListProductImages::default(),
		};
		let mut query = ProductImages::find();
		if let Some(product_id) = &model.product_id {
			query = query.filter(ProductImagesColumn::ProductId.eq(product_id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateProductImages {}

impl CreateProductImages {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateProductImages>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ProductImagesActiveModel {
			product_id: Set("id".to_string()),
			image_id: Set("id".to_string()),
		};
...
impl UpdateProductImages {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductImagesParmns>,
		json_data: Option<web::Json<UpdateProductImages>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ProductImages::find_by_id((path.product_id.clone(), path.image_id.clone()))
			.one(db.get_ref())
			.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ProductImagesActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteProductImages {
	pub product_id: String,
	pub image_id: String,
}

impl DeleteProductImages {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductImagesParmns>,
	) -> impl Responder {
		let res = ProductImages::delete_by_id((path.product_id.clone(), path.image_id.clone()))
			.exec(db.get_ref())
			.await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseProductImages {
	pub product_id: String,
	pub image_id: String,
}
