...
#[derive(Deserialize)]
pub struct MoneyAmountParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/money_amount")
			.route("/", web::get().to(GetMoneyAmount::route))
			.route("/", web::post().to(CreateMoneyAmount::route))
			.route("/list", web::get().to(ListMoneyAmount::route))
			.route("/{id}", web::put().to(UpdateMoneyAmount::route))
			.route("/{id}", web::get().to(DeleteMoneyAmount::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetMoneyAmount {
...
impl GetMoneyAmount {
	async fn route(
		...
		json_data: Option<web::Json<GetMoneyAmount>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetMoneyAmount::default(),
		};
		let mut query = MoneyAmount::find();
		if let Some(id) = &model.id {
			query = query.filter(MoneyAmountColumn::Id.eq(id.clone()));
		}
		if let Some(currency_code) = &model.currency_code {
			query = query.filter(MoneyAmountColumn::CurrencyCode.eq(currency_code.clone()));
		}
		if let Some(amount) = &model.amount {
			query = query.filter(MoneyAmountColumn::Amount.eq(amount.clone()));
		}
		if let Some(region_id) = &model.region_id {
			query = query.filter(MoneyAmountColumn::RegionId.eq(region_id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(MoneyAmountColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(MoneyAmountColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(MoneyAmountColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(min_quantity) = &model.min_quantity {
			query = query.filter(MoneyAmountColumn::MinQuantity.eq(min_quantity.clone()));
		}
		if let Some(max_quantity) = &model.max_quantity {
			query = query.filter(MoneyAmountColumn::MaxQuantity.eq(max_quantity.clone()));
		}
		if let Some(price_list_id) = &model.price_list_id {
			query = query.filter(MoneyAmountColumn::PriceListId.eq(price_list_id.clone()));
		}
...
impl ListMoneyAmount {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListMoneyAmount>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListMoneyAmount::default(),
		};
		let mut query = MoneyAmount::find();
		if let Some(id) = &model.id {
			query = query.filter(MoneyAmountColumn::Id.eq(id.clone()));
...
impl CreateMoneyAmount {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateMoneyAmount>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = MoneyAmountActiveModel {
			id: Set("id".to_string()),
			...
			min_quantity: Set(model.min_quantity.clone()),
			max_quantity: Set(model.max_quantity.clone()),
			price_list_id: Set(model.price_list_id.clone()),
		};
...
impl UpdateMoneyAmount {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<MoneyAmountParmns>,
		json_data: Option<web::Json<UpdateMoneyAmount>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = MoneyAmount::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: MoneyAmountActiveModel = table.into();
					if let Some(currency_code) = &model.currency_code {
					...
					model_active.region_id = Set(model.region_id);
					if let Some(created_at) = &model.created_at {
					...
					model_active.min_quantity = Set(model.min_quantity);
					model_active.max_quantity = Set(model.max_quantity);
					model_active.price_list_id = Set(model.price_list_id);
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteMoneyAmount {
	pub id: String,
}

impl DeleteMoneyAmount {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<MoneyAmountParmns>,
	) -> impl Responder {
		let res = MoneyAmount::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseMoneyAmount {
	pub id: String,
	pub currency_code: String,
	pub amount: i32,
	pub region_id: Option<String>,
