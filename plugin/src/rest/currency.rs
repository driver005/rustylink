...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/currency")
			.route("/", web::get().to(GetCurrency::route))
			.route("/", web::post().to(CreateCurrency::route))
			.route("/list", web::get().to(ListCurrency::route))
			.route("/{id}", web::put().to(UpdateCurrency::route))
			.route("/{id}", web::get().to(DeleteCurrency::route)),
	);
}
...
impl GetCurrency {
	async fn route(
		...
		json_data: Option<web::Json<GetCurrency>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetCurrency::default(),
		};
		let mut query = Currency::find();
		if let Some(code) = &model.code {
			query = query.filter(CurrencyColumn::Code.eq(code.clone()));
		}
		if let Some(symbol) = &model.symbol {
			query = query.filter(CurrencyColumn::Symbol.eq(symbol.clone()));
		}
		if let Some(symbol_native) = &model.symbol_native {
			query = query.filter(CurrencyColumn::SymbolNative.eq(symbol_native.clone()));
		}
		if let Some(name) = &model.name {
			query = query.filter(CurrencyColumn::Name.eq(name.clone()));
		}
...
impl ListCurrency {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListCurrency>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListCurrency::default(),
		};
		let mut query = Currency::find();
		if let Some(code) = &model.code {
			query = query.filter(CurrencyColumn::Code.eq(code.clone()));
...
impl CreateCurrency {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateCurrency>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = CurrencyActiveModel {
			code: Set("id".to_string()),
			symbol: Set(model.symbol.clone()),
			symbol_native: Set(model.symbol_native.clone()),
...
impl UpdateCurrency {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<CurrencyParmns>,
		json_data: Option<web::Json<UpdateCurrency>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Currency::find_by_id(path.code.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: CurrencyActiveModel = table.into();
					if let Some(symbol) = &model.symbol {
						model_active.symbol = Set(symbol.clone());
					}
					if let Some(symbol_native) = &model.symbol_native {
						model_active.symbol_native = Set(symbol_native.clone());
					}
					if let Some(name) = &model.name {
...
impl DeleteCurrency {
	async fn route(
		...
		let res = Currency::delete_by_id(path.code.clone()).exec(db.get_ref()).await;
		match res {
