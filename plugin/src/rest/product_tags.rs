...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/product_tags")
			.route("/", web::get().to(GetProductTags::route))
			.route("/", web::post().to(CreateProductTags::route))
			.route("/list", web::get().to(ListProductTags::route))
			.route("/{id}", web::put().to(UpdateProductTags::route))
			.route("/{id}", web::get().to(DeleteProductTags::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetProductTags {
...
impl GetProductTags {
	async fn route(
		...
		json_data: Option<web::Json<GetProductTags>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetProductTags::default(),
		};
		let mut query = ProductTags::find();
		if let Some(product_id) = &model.product_id {
			query = query.filter(ProductTagsColumn::ProductId.eq(product_id.clone()));
		}
		if let Some(product_tag_id) = &model.product_tag_id {
			query = query.filter(ProductTagsColumn::ProductTagId.eq(product_tag_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListProductTags {
	pub product_id: Option<String>,
	pub product_tag_id: Option<String>,
}

impl ListProductTags {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListProductTags>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListProductTags::default(),
		};
		let mut query = ProductTags::find();
		if let Some(product_id) = &model.product_id {
			query = query.filter(ProductTagsColumn::ProductId.eq(product_id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateProductTags {}

impl CreateProductTags {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateProductTags>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ProductTagsActiveModel {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateProductTags {}

impl UpdateProductTags {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductTagsParmns>,
		json_data: Option<web::Json<UpdateProductTags>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		...
		let old_table =
			ProductTags::find_by_id((path.product_id.clone(), path.product_tag_id.clone()))
				.one(db.get_ref())
				.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ProductTagsActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteProductTags {
	pub product_id: String,
	pub product_tag_id: String,
}

impl DeleteProductTags {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductTagsParmns>,
	) -> impl Responder {
		let res = ProductTags::delete_by_id((path.product_id.clone(), path.product_tag_id.clone()))
			.exec(db.get_ref())
			.await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseProductTags {
	pub product_id: String,
	pub product_tag_id: String,
}
