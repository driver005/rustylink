...
#[derive(Deserialize)]
pub struct ShippingMethodTaxLineParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/shipping_method_tax_line")
			.route("/", web::get().to(GetShippingMethodTaxLine::route))
			.route("/", web::post().to(CreateShippingMethodTaxLine::route))
			.route("/list", web::get().to(ListShippingMethodTaxLine::route))
			.route("/{id}", web::put().to(UpdateShippingMethodTaxLine::route))
			.route("/{id}", web::get().to(DeleteShippingMethodTaxLine::route)),
	);
}

#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct GetShippingMethodTaxLine {
...
impl GetShippingMethodTaxLine {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetShippingMethodTaxLine>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetShippingMethodTaxLine::default(),
		};
		let mut query = ShippingMethodTaxLine::find();
		if let Some(id) = &model.id {
			query = query.filter(ShippingMethodTaxLineColumn::Id.eq(id.clone()));
		}
		if let Some(rate) = &model.rate {
			query = query.filter(ShippingMethodTaxLineColumn::Rate.eq(rate.clone()));
		}
		if let Some(name) = &model.name {
			query = query.filter(ShippingMethodTaxLineColumn::Name.eq(name.clone()));
		}
		if let Some(code) = &model.code {
			query = query.filter(ShippingMethodTaxLineColumn::Code.eq(code.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ShippingMethodTaxLineColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ShippingMethodTaxLineColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ShippingMethodTaxLineColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(shipping_method_id) = &model.shipping_method_id {
			query = query.filter(
				ShippingMethodTaxLineColumn::ShippingMethodId.eq(shipping_method_id.clone()),
			);
		}
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct ListShippingMethodTaxLine {
	pub id: Option<String>,
	pub rate: Option<f32>,
	pub name: Option<String>,
	pub code: Option<String>,
...
impl ListShippingMethodTaxLine {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListShippingMethodTaxLine>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListShippingMethodTaxLine::default(),
		};
		let mut query = ShippingMethodTaxLine::find();
		if let Some(id) = &model.id {
			query = query.filter(ShippingMethodTaxLineColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct CreateShippingMethodTaxLine {
	pub rate: f32,
	pub name: String,
	pub code: Option<String>,
...
impl CreateShippingMethodTaxLine {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateShippingMethodTaxLine>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ShippingMethodTaxLineActiveModel {
			id: Set("id".to_string()),
			rate: Set(model.rate.clone()),
			...
			shipping_method_id: Set(model.shipping_method_id.clone()),
		};
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct UpdateShippingMethodTaxLine {
	pub rate: Option<f32>,
	pub name: Option<String>,
	pub code: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
...
impl UpdateShippingMethodTaxLine {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ShippingMethodTaxLineParmns>,
		json_data: Option<web::Json<UpdateShippingMethodTaxLine>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ShippingMethodTaxLine::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ShippingMethodTaxLineActiveModel = table.into();
					if let Some(rate) = &model.rate {
						model_active.rate = Set(rate.clone());
					}
					if let Some(name) = &model.name {
						model_active.name = Set(name.clone());
					}
					model_active.code = Set(model.code);
					if let Some(created_at) = &model.created_at {
					...
					if let Some(shipping_method_id) = &model.shipping_method_id {
						model_active.shipping_method_id = Set(shipping_method_id.clone());
					}
...
pub struct DeleteShippingMethodTaxLine {
	pub id: String,
}

impl DeleteShippingMethodTaxLine {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ShippingMethodTaxLineParmns>,
	) -> impl Responder {
		let res = ShippingMethodTaxLine::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Serialize, Deserialize)]
pub struct ResponseShippingMethodTaxLine {
	pub id: String,
	pub rate: f32,
	pub name: String,
	pub code: Option<String>,
