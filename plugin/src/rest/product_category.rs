...
#[derive(Deserialize)]
pub struct ProductCategoryParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/product_category")
			.route("/", web::get().to(GetProductCategory::route))
			.route("/", web::post().to(CreateProductCategory::route))
			.route("/list", web::get().to(ListProductCategory::route))
			.route("/{id}", web::put().to(UpdateProductCategory::route))
			.route("/{id}", web::get().to(DeleteProductCategory::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetProductCategory {
...
impl GetProductCategory {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetProductCategory>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetProductCategory::default(),
		};
		let mut query = ProductCategory::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductCategoryColumn::Id.eq(id.clone()));
		}
		if let Some(name) = &model.name {
			query = query.filter(ProductCategoryColumn::Name.eq(name.clone()));
		}
		if let Some(handle) = &model.handle {
			query = query.filter(ProductCategoryColumn::Handle.eq(handle.clone()));
		}
		if let Some(parent_category_id) = &model.parent_category_id {
			query = query
				.filter(ProductCategoryColumn::ParentCategoryId.eq(parent_category_id.clone()));
		}
		if let Some(mpath) = &model.mpath {
			query = query.filter(ProductCategoryColumn::Mpath.eq(mpath.clone()));
		}
		if let Some(is_active) = &model.is_active {
			query = query.filter(ProductCategoryColumn::IsActive.eq(is_active.clone()));
		}
		if let Some(is_internal) = &model.is_internal {
			query = query.filter(ProductCategoryColumn::IsInternal.eq(is_internal.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ProductCategoryColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ProductCategoryColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(rank) = &model.rank {
			query = query.filter(ProductCategoryColumn::Rank.eq(rank.clone()));
		}
		if let Some(description) = &model.description {
			query = query.filter(ProductCategoryColumn::Description.eq(description.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ProductCategoryColumn::Metadata.eq(metadata.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListProductCategory {
	pub id: Option<String>,
	pub name: Option<String>,
	pub handle: Option<String>,
	pub parent_category_id: Option<String>,
...
impl ListProductCategory {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListProductCategory>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListProductCategory::default(),
		};
		let mut query = ProductCategory::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductCategoryColumn::Id.eq(id.clone()));
...
impl CreateProductCategory {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateProductCategory>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ProductCategoryActiveModel {
			id: Set("id".to_string()),
			name: Set(model.name.clone()),
			handle: Set(model.handle.clone()),
			parent_category_id: Set(model.parent_category_id.clone()),
			mpath: Set(model.mpath.clone()),
			is_active: Set(model.is_active.clone()),
			is_internal: Set(model.is_internal.clone()),
			created_at: Set(model.created_at.clone()),
			updated_at: Set(model.updated_at.clone()),
			rank: Set(model.rank.clone()),
...
impl UpdateProductCategory {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductCategoryParmns>,
		json_data: Option<web::Json<UpdateProductCategory>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ProductCategory::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ProductCategoryActiveModel = table.into();
					if let Some(name) = &model.name {
						model_active.name = Set(name.clone());
					}
					if let Some(handle) = &model.handle {
						model_active.handle = Set(handle.clone());
					}
					model_active.parent_category_id = Set(model.parent_category_id);
					model_active.mpath = Set(model.mpath);
					model_active.is_active = Set(model.is_active);
					model_active.is_internal = Set(model.is_internal);
					if let Some(created_at) = &model.created_at {
					...
					if let Some(rank) = &model.rank {
						model_active.rank = Set(rank.clone());
					}
					if let Some(description) = &model.description {
						model_active.description = Set(description.clone());
					}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteProductCategory {
	pub id: String,
}

impl DeleteProductCategory {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductCategoryParmns>,
	) -> impl Responder {
		let res = ProductCategory::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseProductCategory {
	pub id: String,
	pub name: String,
	pub handle: String,
	pub parent_category_id: Option<String>,
