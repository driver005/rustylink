...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/product")
			.route("/", web::get().to(GetProduct::route))
			.route("/", web::post().to(CreateProduct::route))
			.route("/list", web::get().to(ListProduct::route))
			.route("/{id}", web::put().to(UpdateProduct::route))
			.route("/{id}", web::get().to(DeleteProduct::route)),
	);
}
...
impl GetProduct {
	async fn route(
		...
		json_data: Option<web::Json<GetProduct>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetProduct::default(),
		};
		let mut query = Product::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductColumn::Id.eq(id.clone()));
		}
		if let Some(title) = &model.title {
			query = query.filter(ProductColumn::Title.eq(title.clone()));
		}
		if let Some(subtitle) = &model.subtitle {
			query = query.filter(ProductColumn::Subtitle.eq(subtitle.clone()));
		}
		if let Some(description) = &model.description {
			query = query.filter(ProductColumn::Description.eq(description.clone()));
		}
		if let Some(handle) = &model.handle {
			query = query.filter(ProductColumn::Handle.eq(handle.clone()));
		}
		if let Some(is_giftcard) = &model.is_giftcard {
			query = query.filter(ProductColumn::IsGiftcard.eq(is_giftcard.clone()));
		}
		if let Some(thumbnail) = &model.thumbnail {
			query = query.filter(ProductColumn::Thumbnail.eq(thumbnail.clone()));
		}
		if let Some(weight) = &model.weight {
			query = query.filter(ProductColumn::Weight.eq(weight.clone()));
		}
		if let Some(length) = &model.length {
			query = query.filter(ProductColumn::Length.eq(length.clone()));
		}
		if let Some(height) = &model.height {
			query = query.filter(ProductColumn::Height.eq(height.clone()));
		}
		if let Some(width) = &model.width {
			query = query.filter(ProductColumn::Width.eq(width.clone()));
		}
		if let Some(hs_code) = &model.hs_code {
			query = query.filter(ProductColumn::HsCode.eq(hs_code.clone()));
		}
		if let Some(origin_country) = &model.origin_country {
			query = query.filter(ProductColumn::OriginCountry.eq(origin_country.clone()));
		}
		if let Some(mid_code) = &model.mid_code {
			query = query.filter(ProductColumn::MidCode.eq(mid_code.clone()));
		}
		if let Some(material) = &model.material {
			query = query.filter(ProductColumn::Material.eq(material.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ProductColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ProductColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ProductColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ProductColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(collection_id) = &model.collection_id {
			query = query.filter(ProductColumn::CollectionId.eq(collection_id.clone()));
		}
		if let Some(type_id) = &model.type_id {
			query = query.filter(ProductColumn::TypeId.eq(type_id.clone()));
		}
		if let Some(discountable) = &model.discountable {
			query = query.filter(ProductColumn::Discountable.eq(discountable.clone()));
		}
		if let Some(status) = &model.status {
			query = query.filter(ProductColumn::Status.eq(status.clone()));
		}
		if let Some(external_id) = &model.external_id {
			query = query.filter(ProductColumn::ExternalId.eq(external_id.clone()));
		}
...
impl ListProduct {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListProduct>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListProduct::default(),
		};
		let mut query = Product::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductColumn::Id.eq(id.clone()));
...
impl CreateProduct {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateProduct>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let status = sea_orm_active_enums::ProductStatusEnum::from(*&model.status);
		let new_table = ProductActiveModel {
			id: Set("id".to_string()),
			title: Set(model.title.clone()),
			subtitle: Set(model.subtitle.clone()),
			...
			collection_id: Set(model.collection_id.clone()),
			type_id: Set(model.type_id.clone()),
			discountable: Set(model.discountable.clone()),
			status: Set(status),
...
impl UpdateProduct {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductParmns>,
		json_data: Option<web::Json<UpdateProduct>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Product::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ProductActiveModel = table.into();
					if let Some(title) = &model.title {
						model_active.title = Set(title.clone());
					}
					model_active.subtitle = Set(model.subtitle);
					model_active.description = Set(model.description);
					model_active.handle = Set(model.handle);
					if let Some(is_giftcard) = &model.is_giftcard {
					...
					model_active.collection_id = Set(model.collection_id);
					model_active.type_id = Set(model.type_id);
					if let Some(discountable) = &model.discountable {
						model_active.discountable = Set(discountable.clone());
					}
					if let Some(value) = &model.status {
						let status = Some(sea_orm_active_enums::ProductStatusEnum::from(*value));
...
impl DeleteProduct {
	async fn route(
		...
		let res = Product::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
