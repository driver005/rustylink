...
#[derive(Deserialize)]
pub struct ProductCollectionParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/product_collection")
			.route("/", web::get().to(GetProductCollection::route))
			.route("/", web::post().to(CreateProductCollection::route))
			.route("/list", web::get().to(ListProductCollection::route))
			.route("/{id}", web::put().to(UpdateProductCollection::route))
			.route("/{id}", web::get().to(DeleteProductCollection::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetProductCollection {
...
impl GetProductCollection {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetProductCollection>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetProductCollection::default(),
		};
		let mut query = ProductCollection::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductCollectionColumn::Id.eq(id.clone()));
		}
		if let Some(title) = &model.title {
			query = query.filter(ProductCollectionColumn::Title.eq(title.clone()));
		}
		if let Some(handle) = &model.handle {
			query = query.filter(ProductCollectionColumn::Handle.eq(handle.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ProductCollectionColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ProductCollectionColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ProductCollectionColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ProductCollectionColumn::Metadata.eq(metadata.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListProductCollection {
	pub id: Option<String>,
	pub title: Option<String>,
	pub handle: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
...
impl ListProductCollection {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListProductCollection>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListProductCollection::default(),
		};
		let mut query = ProductCollection::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductCollectionColumn::Id.eq(id.clone()));
...
impl CreateProductCollection {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateProductCollection>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ProductCollectionActiveModel {
			id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateProductCollection {
	pub title: Option<String>,
	pub handle: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
...
impl UpdateProductCollection {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductCollectionParmns>,
		json_data: Option<web::Json<UpdateProductCollection>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ProductCollection::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ProductCollectionActiveModel = table.into();
					if let Some(title) = &model.title {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteProductCollection {
	pub id: String,
}

impl DeleteProductCollection {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductCollectionParmns>,
	) -> impl Responder {
		let res = ProductCollection::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseProductCollection {
	pub id: String,
	pub title: String,
	pub handle: Option<String>,
	pub created_at: DateTimeWithTimeZone,
