...
#[derive(Deserialize)]
pub struct StagedJobParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/staged_job")
			.route("/", web::get().to(GetStagedJob::route))
			.route("/", web::post().to(CreateStagedJob::route))
			.route("/list", web::get().to(ListStagedJob::route))
			.route("/{id}", web::put().to(UpdateStagedJob::route))
			.route("/{id}", web::get().to(DeleteStagedJob::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetStagedJob {
...
impl GetStagedJob {
	async fn route(
		...
		json_data: Option<web::Json<GetStagedJob>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetStagedJob::default(),
		};
		let mut query = StagedJob::find();
		if let Some(id) = &model.id {
			query = query.filter(StagedJobColumn::Id.eq(id.clone()));
		}
		if let Some(event_name) = &model.event_name {
			query = query.filter(StagedJobColumn::EventName.eq(event_name.clone()));
		}
		if let Some(data) = &model.data {
			query = query.filter(StagedJobColumn::Data.eq(data.clone()));
		}
		if let Some(options) = &model.options {
			query = query.filter(StagedJobColumn::Options.eq(options.clone()));
		}
...
impl ListStagedJob {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListStagedJob>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListStagedJob::default(),
		};
		let mut query = StagedJob::find();
		if let Some(id) = &model.id {
			query = query.filter(StagedJobColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateStagedJob {
	pub event_name: String,
	pub data: Json,
	pub options: Json,
}

impl CreateStagedJob {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateStagedJob>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = StagedJobActiveModel {
			id: Set("id".to_string()),
			event_name: Set(model.event_name.clone()),
			data: Set(model.data.clone()),
			options: Set(model.options.clone()),
		};
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateStagedJob {
	pub event_name: Option<String>,
	pub data: Option<Json>,
	pub options: Option<Json>,
}

impl UpdateStagedJob {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<StagedJobParmns>,
		json_data: Option<web::Json<UpdateStagedJob>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = StagedJob::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: StagedJobActiveModel = table.into();
					if let Some(event_name) = &model.event_name {
						model_active.event_name = Set(event_name.clone());
					}
					if let Some(data) = &model.data {
						model_active.data = Set(data.clone());
					}
					if let Some(options) = &model.options {
						model_active.options = Set(options.clone());
					}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteStagedJob {
	pub id: String,
}

impl DeleteStagedJob {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<StagedJobParmns>,
	) -> impl Responder {
		let res = StagedJob::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
