...
#[derive(Deserialize)]
pub struct RegionFulfillmentProvidersParmns {
	region_id: String,
	provider_id: String,
}
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/region_fulfillment_providers")
			.route("/", web::get().to(GetRegionFulfillmentProviders::route))
			.route("/", web::post().to(CreateRegionFulfillmentProviders::route))
			.route("/list", web::get().to(ListRegionFulfillmentProviders::route))
			.route("/{id}", web::put().to(UpdateRegionFulfillmentProviders::route))
			.route("/{id}", web::get().to(DeleteRegionFulfillmentProviders::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetRegionFulfillmentProviders {
	pub region_id: Option<String>,
	pub provider_id: Option<String>,
}

impl GetRegionFulfillmentProviders {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetRegionFulfillmentProviders>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetRegionFulfillmentProviders::default(),
		};
		let mut query = RegionFulfillmentProviders::find();
		if let Some(region_id) = &model.region_id {
			query = query.filter(RegionFulfillmentProvidersColumn::RegionId.eq(region_id.clone()));
		}
		if let Some(provider_id) = &model.provider_id {
			query =
				query.filter(RegionFulfillmentProvidersColumn::ProviderId.eq(provider_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListRegionFulfillmentProviders {
	pub region_id: Option<String>,
	pub provider_id: Option<String>,
}

impl ListRegionFulfillmentProviders {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListRegionFulfillmentProviders>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListRegionFulfillmentProviders::default(),
		};
		let mut query = RegionFulfillmentProviders::find();
		if let Some(region_id) = &model.region_id {
			query = query.filter(RegionFulfillmentProvidersColumn::RegionId.eq(region_id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateRegionFulfillmentProviders {}

impl CreateRegionFulfillmentProviders {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateRegionFulfillmentProviders>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = RegionFulfillmentProvidersActiveModel {
			region_id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateRegionFulfillmentProviders {}

impl UpdateRegionFulfillmentProviders {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<RegionFulfillmentProvidersParmns>,
		json_data: Option<web::Json<UpdateRegionFulfillmentProviders>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = RegionFulfillmentProviders::find_by_id((
			path.region_id.clone(),
		...
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: RegionFulfillmentProvidersActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteRegionFulfillmentProviders {
	pub region_id: String,
	pub provider_id: String,
}

impl DeleteRegionFulfillmentProviders {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<RegionFulfillmentProvidersParmns>,
	) -> impl Responder {
		let res = RegionFulfillmentProviders::delete_by_id((
			path.region_id.clone(),
			path.provider_id.clone(),
		))
		.exec(db.get_ref())
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseRegionFulfillmentProviders {
	pub region_id: String,
	pub provider_id: String,
}
