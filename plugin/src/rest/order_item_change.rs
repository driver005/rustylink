...
#[derive(Deserialize)]
pub struct OrderItemChangeParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/order_item_change")
			.route("/", web::get().to(GetOrderItemChange::route))
			.route("/", web::post().to(CreateOrderItemChange::route))
			.route("/list", web::get().to(ListOrderItemChange::route))
			.route("/{id}", web::put().to(UpdateOrderItemChange::route))
			.route("/{id}", web::get().to(DeleteOrderItemChange::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetOrderItemChange {
	pub id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub deleted_at: Option<DateTimeWithTimeZone>,
	pub r#type: Option<OrderItemChangeTypeEnum>,
	pub order_edit_id: Option<String>,
	pub original_line_item_id: Option<String>,
	pub line_item_id: Option<String>,
}

impl GetOrderItemChange {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetOrderItemChange>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetOrderItemChange::default(),
		};
		let mut query = OrderItemChange::find();
		if let Some(id) = &model.id {
			query = query.filter(OrderItemChangeColumn::Id.eq(id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(OrderItemChangeColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(OrderItemChangeColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(OrderItemChangeColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(r#type) = &model.r#type {
			query = query.filter(OrderItemChangeColumn::Type.eq(r#type.clone()));
		}
		if let Some(order_edit_id) = &model.order_edit_id {
			query = query.filter(OrderItemChangeColumn::OrderEditId.eq(order_edit_id.clone()));
		}
		if let Some(original_line_item_id) = &model.original_line_item_id {
			query = query.filter(
				OrderItemChangeColumn::OriginalLineItemId.eq(original_line_item_id.clone()),
			);
		}
		if let Some(line_item_id) = &model.line_item_id {
			query = query.filter(OrderItemChangeColumn::LineItemId.eq(line_item_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListOrderItemChange {
	pub id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub deleted_at: Option<DateTimeWithTimeZone>,
...
impl ListOrderItemChange {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListOrderItemChange>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListOrderItemChange::default(),
		};
		let mut query = OrderItemChange::find();
		if let Some(id) = &model.id {
			query = query.filter(OrderItemChangeColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateOrderItemChange {
...
impl CreateOrderItemChange {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateOrderItemChange>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let r#type = sea_orm_active_enums::OrderItemChangeTypeEnum::from(*&model.r#type);
		let new_table = OrderItemChangeActiveModel {
			id: Set("id".to_string()),
			...
			original_line_item_id: Set(model.original_line_item_id.clone()),
			line_item_id: Set(model.line_item_id.clone()),
		};
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateOrderItemChange {
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub deleted_at: Option<DateTimeWithTimeZone>,
	pub r#type: Option<OrderItemChangeTypeEnum>,
...
impl UpdateOrderItemChange {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<OrderItemChangeParmns>,
		json_data: Option<web::Json<UpdateOrderItemChange>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = OrderItemChange::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: OrderItemChangeActiveModel = table.into();
					if let Some(created_at) = &model.created_at {
					...
					if let Some(value) = &model.r#type {
						let r#type =
							Some(sea_orm_active_enums::OrderItemChangeTypeEnum::from(*value));
						model_active.r#type = Set(r#type);
					}
					if let Some(order_edit_id) = &model.order_edit_id {
						model_active.order_edit_id = Set(order_edit_id.clone());
					}
					model_active.original_line_item_id = Set(model.original_line_item_id);
					model_active.line_item_id = Set(model.line_item_id);
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteOrderItemChange {
	pub id: String,
}

impl DeleteOrderItemChange {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<OrderItemChangeParmns>,
	) -> impl Responder {
		let res = OrderItemChange::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseOrderItemChange {
	pub id: String,
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
	pub deleted_at: Option<DateTimeWithTimeZone>,
