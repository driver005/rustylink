...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/oauth")
			.route("/", web::get().to(GetOauth::route))
			.route("/", web::post().to(CreateOauth::route))
			.route("/list", web::get().to(ListOauth::route))
			.route("/{id}", web::put().to(UpdateOauth::route))
			.route("/{id}", web::get().to(DeleteOauth::route)),
	);
}
...
impl GetOauth {
	async fn route(
		...
		json_data: Option<web::Json<GetOauth>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetOauth::default(),
		};
		let mut query = Oauth::find();
		if let Some(id) = &model.id {
			query = query.filter(OauthColumn::Id.eq(id.clone()));
		}
		if let Some(display_name) = &model.display_name {
			query = query.filter(OauthColumn::DisplayName.eq(display_name.clone()));
		}
		if let Some(application_name) = &model.application_name {
			query = query.filter(OauthColumn::ApplicationName.eq(application_name.clone()));
		}
		if let Some(install_url) = &model.install_url {
			query = query.filter(OauthColumn::InstallUrl.eq(install_url.clone()));
		}
		if let Some(uninstall_url) = &model.uninstall_url {
			query = query.filter(OauthColumn::UninstallUrl.eq(uninstall_url.clone()));
		}
		if let Some(data) = &model.data {
			query = query.filter(OauthColumn::Data.eq(data.clone()));
		}
...
impl ListOauth {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListOauth>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListOauth::default(),
		};
		let mut query = Oauth::find();
		if let Some(id) = &model.id {
			query = query.filter(OauthColumn::Id.eq(id.clone()));
...
impl CreateOauth {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateOauth>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = OauthActiveModel {
			id: Set("id".to_string()),
			display_name: Set(model.display_name.clone()),
			application_name: Set(model.application_name.clone()),
			install_url: Set(model.install_url.clone()),
			uninstall_url: Set(model.uninstall_url.clone()),
...
impl UpdateOauth {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<OauthParmns>,
		json_data: Option<web::Json<UpdateOauth>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Oauth::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: OauthActiveModel = table.into();
					if let Some(display_name) = &model.display_name {
						model_active.display_name = Set(display_name.clone());
					}
					if let Some(application_name) = &model.application_name {
						model_active.application_name = Set(application_name.clone());
					}
					model_active.install_url = Set(model.install_url);
					model_active.uninstall_url = Set(model.uninstall_url);
					model_active.data = Set(model.data);
...
impl DeleteOauth {
	async fn route(
		...
		let res = Oauth::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
