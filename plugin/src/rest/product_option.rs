...
#[derive(Deserialize)]
pub struct ProductOptionParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/product_option")
			.route("/", web::get().to(GetProductOption::route))
			.route("/", web::post().to(CreateProductOption::route))
			.route("/list", web::get().to(ListProductOption::route))
			.route("/{id}", web::put().to(UpdateProductOption::route))
			.route("/{id}", web::get().to(DeleteProductOption::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetProductOption {
...
impl GetProductOption {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetProductOption>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetProductOption::default(),
		};
		let mut query = ProductOption::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductOptionColumn::Id.eq(id.clone()));
		}
		if let Some(title) = &model.title {
			query = query.filter(ProductOptionColumn::Title.eq(title.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ProductOptionColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ProductOptionColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ProductOptionColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ProductOptionColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(product_id) = &model.product_id {
			query = query.filter(ProductOptionColumn::ProductId.eq(product_id.clone()));
		}
...
impl ListProductOption {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListProductOption>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListProductOption::default(),
		};
		let mut query = ProductOption::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductOptionColumn::Id.eq(id.clone()));
...
impl CreateProductOption {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateProductOption>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ProductOptionActiveModel {
			id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateProductOption {
	pub title: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub deleted_at: Option<DateTimeWithTimeZone>,
...
impl UpdateProductOption {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductOptionParmns>,
		json_data: Option<web::Json<UpdateProductOption>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ProductOption::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ProductOptionActiveModel = table.into();
					if let Some(title) = &model.title {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteProductOption {
	pub id: String,
}

impl DeleteProductOption {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductOptionParmns>,
	) -> impl Responder {
		let res = ProductOption::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseProductOption {
	pub id: String,
	pub title: String,
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
