...
#[derive(Deserialize)]
pub struct OrderGiftCardsParmns {
	order_id: String,
	gift_card_id: String,
}
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/order_gift_cards")
			.route("/", web::get().to(GetOrderGiftCards::route))
			.route("/", web::post().to(CreateOrderGiftCards::route))
			.route("/list", web::get().to(ListOrderGiftCards::route))
			.route("/{id}", web::put().to(UpdateOrderGiftCards::route))
			.route("/{id}", web::get().to(DeleteOrderGiftCards::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetOrderGiftCards {
	pub order_id: Option<String>,
	pub gift_card_id: Option<String>,
}

impl GetOrderGiftCards {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetOrderGiftCards>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetOrderGiftCards::default(),
		};
		let mut query = OrderGiftCards::find();
		if let Some(order_id) = &model.order_id {
			query = query.filter(OrderGiftCardsColumn::OrderId.eq(order_id.clone()));
		}
		if let Some(gift_card_id) = &model.gift_card_id {
			query = query.filter(OrderGiftCardsColumn::GiftCardId.eq(gift_card_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListOrderGiftCards {
	pub order_id: Option<String>,
	pub gift_card_id: Option<String>,
}

impl ListOrderGiftCards {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListOrderGiftCards>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListOrderGiftCards::default(),
		};
		let mut query = OrderGiftCards::find();
		if let Some(order_id) = &model.order_id {
			query = query.filter(OrderGiftCardsColumn::OrderId.eq(order_id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateOrderGiftCards {}

impl CreateOrderGiftCards {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateOrderGiftCards>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = OrderGiftCardsActiveModel {
			order_id: Set("id".to_string()),
			gift_card_id: Set("id".to_string()),
		};
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateOrderGiftCards {}

impl UpdateOrderGiftCards {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<OrderGiftCardsParmns>,
		json_data: Option<web::Json<UpdateOrderGiftCards>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		...
		let old_table =
			OrderGiftCards::find_by_id((path.order_id.clone(), path.gift_card_id.clone()))
				.one(db.get_ref())
				.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: OrderGiftCardsActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteOrderGiftCards {
	pub order_id: String,
	pub gift_card_id: String,
}

impl DeleteOrderGiftCards {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<OrderGiftCardsParmns>,
	) -> impl Responder {
		let res = OrderGiftCards::delete_by_id((path.order_id.clone(), path.gift_card_id.clone()))
			.exec(db.get_ref())
			.await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseOrderGiftCards {
	pub order_id: String,
	pub gift_card_id: String,
}
