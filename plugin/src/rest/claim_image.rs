...
#[derive(Deserialize)]
pub struct ClaimImageParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/claim_image")
			.route("/", web::get().to(GetClaimImage::route))
			.route("/", web::post().to(CreateClaimImage::route))
			.route("/list", web::get().to(ListClaimImage::route))
			.route("/{id}", web::put().to(UpdateClaimImage::route))
			.route("/{id}", web::get().to(DeleteClaimImage::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetClaimImage {
	pub id: Option<String>,
	pub claim_item_id: Option<String>,
...
impl GetClaimImage {
	async fn route(
		...
		json_data: Option<web::Json<GetClaimImage>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetClaimImage::default(),
		};
		let mut query = ClaimImage::find();
		if let Some(id) = &model.id {
			query = query.filter(ClaimImageColumn::Id.eq(id.clone()));
		}
		if let Some(claim_item_id) = &model.claim_item_id {
			query = query.filter(ClaimImageColumn::ClaimItemId.eq(claim_item_id.clone()));
		}
		if let Some(url) = &model.url {
			query = query.filter(ClaimImageColumn::Url.eq(url.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ClaimImageColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ClaimImageColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ClaimImageColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ClaimImageColumn::Metadata.eq(metadata.clone()));
		}
...
impl ListClaimImage {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListClaimImage>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListClaimImage::default(),
		};
		let mut query = ClaimImage::find();
		if let Some(id) = &model.id {
			query = query.filter(ClaimImageColumn::Id.eq(id.clone()));
...
impl CreateClaimImage {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateClaimImage>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ClaimImageActiveModel {
			id: Set("id".to_string()),
			claim_item_id: Set(model.claim_item_id.clone()),
...
impl UpdateClaimImage {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ClaimImageParmns>,
		json_data: Option<web::Json<UpdateClaimImage>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ClaimImage::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ClaimImageActiveModel = table.into();
					if let Some(claim_item_id) = &model.claim_item_id {
						model_active.claim_item_id = Set(claim_item_id.clone());
					}
					if let Some(url) = &model.url {
						model_active.url = Set(url.clone());
					}
					if let Some(created_at) = &model.created_at {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteClaimImage {
	pub id: String,
}

impl DeleteClaimImage {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ClaimImageParmns>,
	) -> impl Responder {
		let res = ClaimImage::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
