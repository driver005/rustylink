...
#[derive(Deserialize)]
pub struct ClaimItemTagsParmns {
	item_id: String,
	tag_id: String,
}
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/claim_item_tags")
			.route("/", web::get().to(GetClaimItemTags::route))
			.route("/", web::post().to(CreateClaimItemTags::route))
			.route("/list", web::get().to(ListClaimItemTags::route))
			.route("/{id}", web::put().to(UpdateClaimItemTags::route))
			.route("/{id}", web::get().to(DeleteClaimItemTags::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetClaimItemTags {
	pub item_id: Option<String>,
	pub tag_id: Option<String>,
}

impl GetClaimItemTags {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetClaimItemTags>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetClaimItemTags::default(),
		};
		let mut query = ClaimItemTags::find();
		if let Some(item_id) = &model.item_id {
			query = query.filter(ClaimItemTagsColumn::ItemId.eq(item_id.clone()));
		}
		if let Some(tag_id) = &model.tag_id {
			query = query.filter(ClaimItemTagsColumn::TagId.eq(tag_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListClaimItemTags {
	pub item_id: Option<String>,
	pub tag_id: Option<String>,
}

impl ListClaimItemTags {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListClaimItemTags>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListClaimItemTags::default(),
		};
		let mut query = ClaimItemTags::find();
		if let Some(item_id) = &model.item_id {
			query = query.filter(ClaimItemTagsColumn::ItemId.eq(item_id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateClaimItemTags {}

impl CreateClaimItemTags {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateClaimItemTags>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ClaimItemTagsActiveModel {
			item_id: Set("id".to_string()),
			tag_id: Set("id".to_string()),
		};
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateClaimItemTags {}

impl UpdateClaimItemTags {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ClaimItemTagsParmns>,
		json_data: Option<web::Json<UpdateClaimItemTags>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ClaimItemTags::find_by_id((path.item_id.clone(), path.tag_id.clone()))
			.one(db.get_ref())
			.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ClaimItemTagsActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteClaimItemTags {
	pub item_id: String,
	pub tag_id: String,
}

impl DeleteClaimItemTags {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ClaimItemTagsParmns>,
	) -> impl Responder {
		let res = ClaimItemTags::delete_by_id((path.item_id.clone(), path.tag_id.clone()))
			.exec(db.get_ref())
			.await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseClaimItemTags {
	pub item_id: String,
	pub tag_id: String,
}
