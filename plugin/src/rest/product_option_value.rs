...
#[derive(Deserialize)]
pub struct ProductOptionValueParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/product_option_value")
			.route("/", web::get().to(GetProductOptionValue::route))
			.route("/", web::post().to(CreateProductOptionValue::route))
			.route("/list", web::get().to(ListProductOptionValue::route))
			.route("/{id}", web::put().to(UpdateProductOptionValue::route))
			.route("/{id}", web::get().to(DeleteProductOptionValue::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetProductOptionValue {
...
impl GetProductOptionValue {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetProductOptionValue>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetProductOptionValue::default(),
		};
		let mut query = ProductOptionValue::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductOptionValueColumn::Id.eq(id.clone()));
		}
		if let Some(value) = &model.value {
			query = query.filter(ProductOptionValueColumn::Value.eq(value.clone()));
		}
		if let Some(option_id) = &model.option_id {
			query = query.filter(ProductOptionValueColumn::OptionId.eq(option_id.clone()));
		}
		if let Some(variant_id) = &model.variant_id {
			query = query.filter(ProductOptionValueColumn::VariantId.eq(variant_id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ProductOptionValueColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ProductOptionValueColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ProductOptionValueColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ProductOptionValueColumn::Metadata.eq(metadata.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListProductOptionValue {
	pub id: Option<String>,
	pub value: Option<String>,
	pub option_id: Option<String>,
	pub variant_id: Option<String>,
...
impl ListProductOptionValue {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListProductOptionValue>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListProductOptionValue::default(),
		};
		let mut query = ProductOptionValue::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductOptionValueColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateProductOptionValue {
	pub value: String,
...
impl CreateProductOptionValue {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateProductOptionValue>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ProductOptionValueActiveModel {
			id: Set("id".to_string()),
			value: Set(model.value.clone()),
			option_id: Set(model.option_id.clone()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateProductOptionValue {
	pub value: Option<String>,
	pub option_id: Option<String>,
	pub variant_id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
...
impl UpdateProductOptionValue {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductOptionValueParmns>,
		json_data: Option<web::Json<UpdateProductOptionValue>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ProductOptionValue::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ProductOptionValueActiveModel = table.into();
					if let Some(value) = &model.value {
						model_active.value = Set(value.clone());
					}
					if let Some(option_id) = &model.option_id {
						model_active.option_id = Set(option_id.clone());
					}
					if let Some(variant_id) = &model.variant_id {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteProductOptionValue {
	pub id: String,
}

impl DeleteProductOptionValue {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductOptionValueParmns>,
	) -> impl Responder {
		let res = ProductOptionValue::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseProductOptionValue {
	pub id: String,
	pub value: String,
	pub option_id: String,
	pub variant_id: String,
