...
#[derive(Deserialize)]
pub struct TaxProviderParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/tax_provider")
			.route("/", web::get().to(GetTaxProvider::route))
			.route("/", web::post().to(CreateTaxProvider::route))
			.route("/list", web::get().to(ListTaxProvider::route))
			.route("/{id}", web::put().to(UpdateTaxProvider::route))
			.route("/{id}", web::get().to(DeleteTaxProvider::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetTaxProvider {
...
impl GetTaxProvider {
	async fn route(
		...
		json_data: Option<web::Json<GetTaxProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetTaxProvider::default(),
		};
		let mut query = TaxProvider::find();
		if let Some(id) = &model.id {
			query = query.filter(TaxProviderColumn::Id.eq(id.clone()));
		}
		if let Some(is_installed) = &model.is_installed {
			query = query.filter(TaxProviderColumn::IsInstalled.eq(is_installed.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListTaxProvider {
	pub id: Option<String>,
	pub is_installed: Option<bool>,
}

impl ListTaxProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListTaxProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListTaxProvider::default(),
		};
		let mut query = TaxProvider::find();
		if let Some(id) = &model.id {
			query = query.filter(TaxProviderColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateTaxProvider {
	pub is_installed: bool,
}

impl CreateTaxProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateTaxProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = TaxProviderActiveModel {
			id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateTaxProvider {
	pub is_installed: Option<bool>,
}

impl UpdateTaxProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<TaxProviderParmns>,
		json_data: Option<web::Json<UpdateTaxProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = TaxProvider::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: TaxProviderActiveModel = table.into();
					if let Some(is_installed) = &model.is_installed {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteTaxProvider {
	pub id: String,
}

impl DeleteTaxProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<TaxProviderParmns>,
	) -> impl Responder {
		let res = TaxProvider::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseTaxProvider {
	pub id: String,
	pub is_installed: bool,
}
