...
#[derive(Deserialize)]
pub struct DiscountRuleProductsParmns {
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/discount_rule_products")
			.route("/", web::get().to(GetDiscountRuleProducts::route))
			.route("/", web::post().to(CreateDiscountRuleProducts::route))
			.route("/list", web::get().to(ListDiscountRuleProducts::route))
			.route("/{id}", web::put().to(UpdateDiscountRuleProducts::route))
			.route("/{id}", web::get().to(DeleteDiscountRuleProducts::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetDiscountRuleProducts {
	pub discount_rule_id: Option<String>,
	pub product_id: Option<String>,
}

impl GetDiscountRuleProducts {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetDiscountRuleProducts>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetDiscountRuleProducts::default(),
		};
		let mut query = DiscountRuleProducts::find();
		if let Some(discount_rule_id) = &model.discount_rule_id {
			query = query
				.filter(DiscountRuleProductsColumn::DiscountRuleId.eq(discount_rule_id.clone()));
		}
		if let Some(product_id) = &model.product_id {
			query = query.filter(DiscountRuleProductsColumn::ProductId.eq(product_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListDiscountRuleProducts {
	pub discount_rule_id: Option<String>,
	pub product_id: Option<String>,
}

impl ListDiscountRuleProducts {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListDiscountRuleProducts>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListDiscountRuleProducts::default(),
		};
		let mut query = DiscountRuleProducts::find();
		if let Some(discount_rule_id) = &model.discount_rule_id {
			query = query
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateDiscountRuleProducts {}

impl CreateDiscountRuleProducts {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateDiscountRuleProducts>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = DiscountRuleProductsActiveModel {
			discount_rule_id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateDiscountRuleProducts {}

impl UpdateDiscountRuleProducts {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<DiscountRuleProductsParmns>,
		json_data: Option<web::Json<UpdateDiscountRuleProducts>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = DiscountRuleProducts::find_by_id((
			path.discount_rule_id.clone(),
		...
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: DiscountRuleProductsActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteDiscountRuleProducts {
	pub discount_rule_id: String,
	pub product_id: String,
}

impl DeleteDiscountRuleProducts {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<DiscountRuleProductsParmns>,
	) -> impl Responder {
		let res = DiscountRuleProducts::delete_by_id((
			path.discount_rule_id.clone(),
			path.product_id.clone(),
		))
		.exec(db.get_ref())
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseDiscountRuleProducts {
	pub discount_rule_id: String,
	pub product_id: String,
}
