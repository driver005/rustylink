...
#[derive(Deserialize)]
pub struct DiscountConditionProductCollectionParmns {
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/discount_condition_product_collection")
			.route("/", web::get().to(GetDiscountConditionProductCollection::route))
			.route("/", web::post().to(CreateDiscountConditionProductCollection::route))
			.route("/list", web::get().to(ListDiscountConditionProductCollection::route))
			.route("/{id}", web::put().to(UpdateDiscountConditionProductCollection::route))
			.route("/{id}", web::get().to(DeleteDiscountConditionProductCollection::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetDiscountConditionProductCollection {
...
impl GetDiscountConditionProductCollection {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetDiscountConditionProductCollection>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetDiscountConditionProductCollection::default(),
		};
		let mut query = DiscountConditionProductCollection::find();
		if let Some(product_collection_id) = &model.product_collection_id {
			query = query.filter(
				DiscountConditionProductCollectionColumn::ProductCollectionId
					.eq(product_collection_id.clone()),
			);
		}
		if let Some(condition_id) = &model.condition_id {
			query = query.filter(
				DiscountConditionProductCollectionColumn::ConditionId.eq(condition_id.clone()),
			);
		}
		if let Some(created_at) = &model.created_at {
			query = query
				.filter(DiscountConditionProductCollectionColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query
				.filter(DiscountConditionProductCollectionColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query
				.filter(DiscountConditionProductCollectionColumn::Metadata.eq(metadata.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListDiscountConditionProductCollection {
	pub product_collection_id: Option<String>,
	pub condition_id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
...
impl ListDiscountConditionProductCollection {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListDiscountConditionProductCollection>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListDiscountConditionProductCollection::default(),
		};
		let mut query = DiscountConditionProductCollection::find();
		if let Some(product_collection_id) = &model.product_collection_id {
			query = query.filter(
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateDiscountConditionProductCollection {
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
	pub metadata: Option<Json>,
}

impl CreateDiscountConditionProductCollection {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateDiscountConditionProductCollection>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = DiscountConditionProductCollectionActiveModel {
			product_collection_id: Set("id".to_string()),
			condition_id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateDiscountConditionProductCollection {
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub metadata: Option<Json>,
}

impl UpdateDiscountConditionProductCollection {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<DiscountConditionProductCollectionParmns>,
		json_data: Option<web::Json<UpdateDiscountConditionProductCollection>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = DiscountConditionProductCollection::find_by_id((
			path.product_collection_id.clone(),
			path.condition_id.clone(),
		))
		.one(db.get_ref())
		.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: DiscountConditionProductCollectionActiveModel =
						table.into();
					if let Some(created_at) = &model.created_at {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteDiscountConditionProductCollection {
	pub product_collection_id: String,
	pub condition_id: String,
}

impl DeleteDiscountConditionProductCollection {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<DiscountConditionProductCollectionParmns>,
	) -> impl Responder {
		let res = DiscountConditionProductCollection::delete_by_id((
			path.product_collection_id.clone(),
			path.condition_id.clone(),
		))
		.exec(db.get_ref())
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseDiscountConditionProductCollection {
	pub product_collection_id: String,
	pub condition_id: String,
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
