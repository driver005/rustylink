...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/discount")
			.route("/", web::get().to(GetDiscount::route))
			.route("/", web::post().to(CreateDiscount::route))
			.route("/list", web::get().to(ListDiscount::route))
			.route("/{id}", web::put().to(UpdateDiscount::route))
			.route("/{id}", web::get().to(DeleteDiscount::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetDiscount {
...
impl GetDiscount {
	async fn route(
		...
		json_data: Option<web::Json<GetDiscount>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetDiscount::default(),
		};
		let mut query = Discount::find();
		if let Some(id) = &model.id {
			query = query.filter(DiscountColumn::Id.eq(id.clone()));
		}
		if let Some(code) = &model.code {
			query = query.filter(DiscountColumn::Code.eq(code.clone()));
		}
		if let Some(is_dynamic) = &model.is_dynamic {
			query = query.filter(DiscountColumn::IsDynamic.eq(is_dynamic.clone()));
		}
		if let Some(rule_id) = &model.rule_id {
			query = query.filter(DiscountColumn::RuleId.eq(rule_id.clone()));
		}
		if let Some(is_disabled) = &model.is_disabled {
			query = query.filter(DiscountColumn::IsDisabled.eq(is_disabled.clone()));
		}
		if let Some(parent_discount_id) = &model.parent_discount_id {
			query = query.filter(DiscountColumn::ParentDiscountId.eq(parent_discount_id.clone()));
		}
		if let Some(starts_at) = &model.starts_at {
			query = query.filter(DiscountColumn::StartsAt.eq(starts_at.clone()));
		}
		if let Some(ends_at) = &model.ends_at {
			query = query.filter(DiscountColumn::EndsAt.eq(ends_at.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(DiscountColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(DiscountColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(DiscountColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(DiscountColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(usage_limit) = &model.usage_limit {
			query = query.filter(DiscountColumn::UsageLimit.eq(usage_limit.clone()));
		}
		if let Some(usage_count) = &model.usage_count {
			query = query.filter(DiscountColumn::UsageCount.eq(usage_count.clone()));
		}
		if let Some(valid_duration) = &model.valid_duration {
			query = query.filter(DiscountColumn::ValidDuration.eq(valid_duration.clone()));
		}
...
impl ListDiscount {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListDiscount>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListDiscount::default(),
		};
		let mut query = Discount::find();
		if let Some(id) = &model.id {
			query = query.filter(DiscountColumn::Id.eq(id.clone()));
...
impl CreateDiscount {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateDiscount>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = DiscountActiveModel {
			id: Set("id".to_string()),
			code: Set(model.code.clone()),
			is_dynamic: Set(model.is_dynamic.clone()),
			rule_id: Set(model.rule_id.clone()),
			is_disabled: Set(model.is_disabled.clone()),
			parent_discount_id: Set(model.parent_discount_id.clone()),
			starts_at: Set(model.starts_at.clone()),
			ends_at: Set(model.ends_at.clone()),
			...
			usage_limit: Set(model.usage_limit.clone()),
			usage_count: Set(model.usage_count.clone()),
			valid_duration: Set(model.valid_duration.clone()),
		};
...
impl UpdateDiscount {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<DiscountParmns>,
		json_data: Option<web::Json<UpdateDiscount>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Discount::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: DiscountActiveModel = table.into();
					if let Some(code) = &model.code {
						model_active.code = Set(code.clone());
					}
					if let Some(is_dynamic) = &model.is_dynamic {
						model_active.is_dynamic = Set(is_dynamic.clone());
					}
					model_active.rule_id = Set(model.rule_id);
					if let Some(is_disabled) = &model.is_disabled {
						model_active.is_disabled = Set(is_disabled.clone());
					}
					model_active.parent_discount_id = Set(model.parent_discount_id);
					if let Some(starts_at) = &model.starts_at {
						model_active.starts_at = Set(starts_at.clone());
					}
					model_active.ends_at = Set(model.ends_at);
					if let Some(created_at) = &model.created_at {
					...
					model_active.usage_limit = Set(model.usage_limit);
					if let Some(usage_count) = &model.usage_count {
						model_active.usage_count = Set(usage_count.clone());
					}
					model_active.valid_duration = Set(model.valid_duration);
...
impl DeleteDiscount {
	async fn route(
		...
		let res = Discount::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
