...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/gift_card")
			.route("/", web::get().to(GetGiftCard::route))
			.route("/", web::post().to(CreateGiftCard::route))
			.route("/list", web::get().to(ListGiftCard::route))
			.route("/{id}", web::put().to(UpdateGiftCard::route))
			.route("/{id}", web::get().to(DeleteGiftCard::route)),
	);
}
...
impl GetGiftCard {
	async fn route(
		...
		json_data: Option<web::Json<GetGiftCard>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetGiftCard::default(),
		};
		let mut query = GiftCard::find();
		if let Some(id) = &model.id {
			query = query.filter(GiftCardColumn::Id.eq(id.clone()));
		}
		if let Some(code) = &model.code {
			query = query.filter(GiftCardColumn::Code.eq(code.clone()));
		}
		if let Some(value) = &model.value {
			query = query.filter(GiftCardColumn::Value.eq(value.clone()));
		}
		if let Some(balance) = &model.balance {
			query = query.filter(GiftCardColumn::Balance.eq(balance.clone()));
		}
		if let Some(region_id) = &model.region_id {
			query = query.filter(GiftCardColumn::RegionId.eq(region_id.clone()));
		}
		if let Some(order_id) = &model.order_id {
			query = query.filter(GiftCardColumn::OrderId.eq(order_id.clone()));
		}
		if let Some(is_disabled) = &model.is_disabled {
			query = query.filter(GiftCardColumn::IsDisabled.eq(is_disabled.clone()));
		}
		if let Some(ends_at) = &model.ends_at {
			query = query.filter(GiftCardColumn::EndsAt.eq(ends_at.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(GiftCardColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(GiftCardColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(GiftCardColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(GiftCardColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(tax_rate) = &model.tax_rate {
			query = query.filter(GiftCardColumn::TaxRate.eq(tax_rate.clone()));
		}
...
impl ListGiftCard {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListGiftCard>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListGiftCard::default(),
		};
		let mut query = GiftCard::find();
		if let Some(id) = &model.id {
			query = query.filter(GiftCardColumn::Id.eq(id.clone()));
...
impl UpdateGiftCard {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<GiftCardParmns>,
		json_data: Option<web::Json<UpdateGiftCard>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = GiftCard::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: GiftCardActiveModel = table.into();
					if let Some(code) = &model.code {
						model_active.code = Set(code.clone());
					}
					if let Some(value) = &model.value {
						model_active.value = Set(value.clone());
					}
					if let Some(balance) = &model.balance {
						model_active.balance = Set(balance.clone());
					}
					if let Some(region_id) = &model.region_id {
...
impl DeleteGiftCard {
	async fn route(
		...
		let res = GiftCard::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
