...
#[derive(Deserialize)]
pub struct PaymentProviderParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/payment_provider")
			.route("/", web::get().to(GetPaymentProvider::route))
			.route("/", web::post().to(CreatePaymentProvider::route))
			.route("/list", web::get().to(ListPaymentProvider::route))
			.route("/{id}", web::put().to(UpdatePaymentProvider::route))
			.route("/{id}", web::get().to(DeletePaymentProvider::route)),
	);
}
...
impl GetPaymentProvider {
	async fn route(
		...
		json_data: Option<web::Json<GetPaymentProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetPaymentProvider::default(),
		};
		let mut query = PaymentProvider::find();
		if let Some(id) = &model.id {
			query = query.filter(PaymentProviderColumn::Id.eq(id.clone()));
		}
		if let Some(is_installed) = &model.is_installed {
			query = query.filter(PaymentProviderColumn::IsInstalled.eq(is_installed.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListPaymentProvider {
	pub id: Option<String>,
	pub is_installed: Option<bool>,
}

impl ListPaymentProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListPaymentProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListPaymentProvider::default(),
		};
		let mut query = PaymentProvider::find();
		if let Some(id) = &model.id {
			query = query.filter(PaymentProviderColumn::Id.eq(id.clone()));
...
impl CreatePaymentProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreatePaymentProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = PaymentProviderActiveModel {
			id: Set("id".to_string()),
...
impl UpdatePaymentProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<PaymentProviderParmns>,
		json_data: Option<web::Json<UpdatePaymentProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = PaymentProvider::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: PaymentProviderActiveModel = table.into();
					if let Some(is_installed) = &model.is_installed {
...
impl DeletePaymentProvider {
	async fn route(
		...
		let res = PaymentProvider::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponsePaymentProvider {
	pub id: String,
	pub is_installed: bool,
}
