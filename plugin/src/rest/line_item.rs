...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/line_item")
			.route("/", web::get().to(GetLineItem::route))
			.route("/", web::post().to(CreateLineItem::route))
			.route("/list", web::get().to(ListLineItem::route))
			.route("/{id}", web::put().to(UpdateLineItem::route))
			.route("/{id}", web::get().to(DeleteLineItem::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetLineItem {
	...
	pub allow_discounts: Option<bool>,
...
impl GetLineItem {
	async fn route(
		...
		json_data: Option<web::Json<GetLineItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetLineItem::default(),
		};
		let mut query = LineItem::find();
		if let Some(id) = &model.id {
			query = query.filter(LineItemColumn::Id.eq(id.clone()));
		}
		if let Some(cart_id) = &model.cart_id {
			query = query.filter(LineItemColumn::CartId.eq(cart_id.clone()));
		}
		if let Some(order_id) = &model.order_id {
			query = query.filter(LineItemColumn::OrderId.eq(order_id.clone()));
		}
		if let Some(swap_id) = &model.swap_id {
			query = query.filter(LineItemColumn::SwapId.eq(swap_id.clone()));
		}
		if let Some(title) = &model.title {
			query = query.filter(LineItemColumn::Title.eq(title.clone()));
		}
		if let Some(description) = &model.description {
			query = query.filter(LineItemColumn::Description.eq(description.clone()));
		}
		if let Some(thumbnail) = &model.thumbnail {
			query = query.filter(LineItemColumn::Thumbnail.eq(thumbnail.clone()));
		}
		if let Some(is_giftcard) = &model.is_giftcard {
			query = query.filter(LineItemColumn::IsGiftcard.eq(is_giftcard.clone()));
		}
		if let Some(should_merge) = &model.should_merge {
			query = query.filter(LineItemColumn::ShouldMerge.eq(should_merge.clone()));
		}
		if let Some(allow_discounts) = &model.allow_discounts {
			query = query.filter(LineItemColumn::AllowDiscounts.eq(allow_discounts.clone()));
		}
		if let Some(has_shipping) = &model.has_shipping {
			query = query.filter(LineItemColumn::HasShipping.eq(has_shipping.clone()));
		}
		if let Some(unit_price) = &model.unit_price {
			query = query.filter(LineItemColumn::UnitPrice.eq(unit_price.clone()));
		}
		if let Some(variant_id) = &model.variant_id {
			query = query.filter(LineItemColumn::VariantId.eq(variant_id.clone()));
		}
		if let Some(quantity) = &model.quantity {
			query = query.filter(LineItemColumn::Quantity.eq(quantity.clone()));
		}
		if let Some(fulfilled_quantity) = &model.fulfilled_quantity {
			query = query.filter(LineItemColumn::FulfilledQuantity.eq(fulfilled_quantity.clone()));
		}
		if let Some(returned_quantity) = &model.returned_quantity {
			query = query.filter(LineItemColumn::ReturnedQuantity.eq(returned_quantity.clone()));
		}
		if let Some(shipped_quantity) = &model.shipped_quantity {
			query = query.filter(LineItemColumn::ShippedQuantity.eq(shipped_quantity.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(LineItemColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(LineItemColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(LineItemColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(claim_order_id) = &model.claim_order_id {
			query = query.filter(LineItemColumn::ClaimOrderId.eq(claim_order_id.clone()));
		}
		if let Some(is_return) = &model.is_return {
			query = query.filter(LineItemColumn::IsReturn.eq(is_return.clone()));
		}
		if let Some(original_item_id) = &model.original_item_id {
			query = query.filter(LineItemColumn::OriginalItemId.eq(original_item_id.clone()));
		}
		if let Some(order_edit_id) = &model.order_edit_id {
			query = query.filter(LineItemColumn::OrderEditId.eq(order_edit_id.clone()));
		}
...
impl ListLineItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListLineItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListLineItem::default(),
		};
		let mut query = LineItem::find();
		if let Some(id) = &model.id {
			query = query.filter(LineItemColumn::Id.eq(id.clone()));
...
impl CreateLineItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateLineItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = LineItemActiveModel {
			id: Set("id".to_string()),
			cart_id: Set(model.cart_id.clone()),
			order_id: Set(model.order_id.clone()),
			swap_id: Set(model.swap_id.clone()),
			title: Set(model.title.clone()),
			description: Set(model.description.clone()),
			thumbnail: Set(model.thumbnail.clone()),
			is_giftcard: Set(model.is_giftcard.clone()),
			should_merge: Set(model.should_merge.clone()),
			allow_discounts: Set(model.allow_discounts.clone()),
			has_shipping: Set(model.has_shipping.clone()),
			unit_price: Set(model.unit_price.clone()),
			variant_id: Set(model.variant_id.clone()),
			quantity: Set(model.quantity.clone()),
			fulfilled_quantity: Set(model.fulfilled_quantity.clone()),
			returned_quantity: Set(model.returned_quantity.clone()),
			shipped_quantity: Set(model.shipped_quantity.clone()),
			created_at: Set(model.created_at.clone()),
			updated_at: Set(model.updated_at.clone()),
			metadata: Set(model.metadata.clone()),
			claim_order_id: Set(model.claim_order_id.clone()),
			is_return: Set(model.is_return.clone()),
			original_item_id: Set(model.original_item_id.clone()),
			order_edit_id: Set(model.order_edit_id.clone()),
		};
...
impl UpdateLineItem {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<LineItemParmns>,
		json_data: Option<web::Json<UpdateLineItem>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = LineItem::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: LineItemActiveModel = table.into();
					model_active.cart_id = Set(model.cart_id);
					model_active.order_id = Set(model.order_id);
					model_active.swap_id = Set(model.swap_id);
					if let Some(title) = &model.title {
						model_active.title = Set(title.clone());
					}
					model_active.description = Set(model.description);
					model_active.thumbnail = Set(model.thumbnail);
					if let Some(is_giftcard) = &model.is_giftcard {
						model_active.is_giftcard = Set(is_giftcard.clone());
					}
					if let Some(should_merge) = &model.should_merge {
						model_active.should_merge = Set(should_merge.clone());
					}
					if let Some(allow_discounts) = &model.allow_discounts {
						model_active.allow_discounts = Set(allow_discounts.clone());
					}
					model_active.has_shipping = Set(model.has_shipping);
					if let Some(unit_price) = &model.unit_price {
						model_active.unit_price = Set(unit_price.clone());
					}
					model_active.variant_id = Set(model.variant_id);
					if let Some(quantity) = &model.quantity {
						model_active.quantity = Set(quantity.clone());
					}
					model_active.fulfilled_quantity = Set(model.fulfilled_quantity);
					model_active.returned_quantity = Set(model.returned_quantity);
					model_active.shipped_quantity = Set(model.shipped_quantity);
					if let Some(created_at) = &model.created_at {
					...
					model_active.claim_order_id = Set(model.claim_order_id);
					if let Some(is_return) = &model.is_return {
						model_active.is_return = Set(is_return.clone());
					}
					model_active.original_item_id = Set(model.original_item_id);
					model_active.order_edit_id = Set(model.order_edit_id);
...
impl DeleteLineItem {
	async fn route(
		...
		let res = LineItem::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
