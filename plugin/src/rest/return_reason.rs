...
#[derive(Deserialize)]
pub struct ReturnReasonParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/return_reason")
			.route("/", web::get().to(GetReturnReason::route))
			.route("/", web::post().to(CreateReturnReason::route))
			.route("/list", web::get().to(ListReturnReason::route))
			.route("/{id}", web::put().to(UpdateReturnReason::route))
			.route("/{id}", web::get().to(DeleteReturnReason::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetReturnReason {
...
impl GetReturnReason {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetReturnReason>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetReturnReason::default(),
		};
		let mut query = ReturnReason::find();
		if let Some(id) = &model.id {
			query = query.filter(ReturnReasonColumn::Id.eq(id.clone()));
		}
		if let Some(value) = &model.value {
			query = query.filter(ReturnReasonColumn::Value.eq(value.clone()));
		}
		if let Some(label) = &model.label {
			query = query.filter(ReturnReasonColumn::Label.eq(label.clone()));
		}
		if let Some(description) = &model.description {
			query = query.filter(ReturnReasonColumn::Description.eq(description.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ReturnReasonColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ReturnReasonColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ReturnReasonColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ReturnReasonColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(parent_return_reason_id) = &model.parent_return_reason_id {
			query = query.filter(
				ReturnReasonColumn::ParentReturnReasonId.eq(parent_return_reason_id.clone()),
			);
		}
...
impl ListReturnReason {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListReturnReason>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListReturnReason::default(),
		};
		let mut query = ReturnReason::find();
		if let Some(id) = &model.id {
			query = query.filter(ReturnReasonColumn::Id.eq(id.clone()));
...
impl CreateReturnReason {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateReturnReason>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ReturnReasonActiveModel {
			id: Set("id".to_string()),
			...
			parent_return_reason_id: Set(model.parent_return_reason_id.clone()),
		};
...
impl UpdateReturnReason {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ReturnReasonParmns>,
		json_data: Option<web::Json<UpdateReturnReason>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ReturnReason::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ReturnReasonActiveModel = table.into();
					if let Some(value) = &model.value {
						model_active.value = Set(value.clone());
					}
					if let Some(label) = &model.label {
						model_active.label = Set(label.clone());
					}
					...
					model_active.parent_return_reason_id = Set(model.parent_return_reason_id);
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteReturnReason {
	pub id: String,
}

impl DeleteReturnReason {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ReturnReasonParmns>,
	) -> impl Responder {
		let res = ReturnReason::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseReturnReason {
	pub id: String,
	pub value: String,
	pub label: String,
	pub description: Option<String>,
