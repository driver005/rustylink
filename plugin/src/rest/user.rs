...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/user")
			.route("/", web::get().to(GetUser::route))
			.route("/", web::post().to(CreateUser::route))
			.route("/list", web::get().to(ListUser::route))
			.route("/{id}", web::put().to(UpdateUser::route))
			.route("/{id}", web::get().to(DeleteUser::route)),
	);
}
...
impl GetUser {
	async fn route(
		...
		json_data: Option<web::Json<GetUser>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetUser::default(),
		};
		let mut query = User::find();
		if let Some(id) = &model.id {
			query = query.filter(UserColumn::Id.eq(id.clone()));
		}
		if let Some(email) = &model.email {
			query = query.filter(UserColumn::Email.eq(email.clone()));
		}
		if let Some(first_name) = &model.first_name {
			query = query.filter(UserColumn::FirstName.eq(first_name.clone()));
		}
		if let Some(last_name) = &model.last_name {
			query = query.filter(UserColumn::LastName.eq(last_name.clone()));
		}
		if let Some(password_hash) = &model.password_hash {
			query = query.filter(UserColumn::PasswordHash.eq(password_hash.clone()));
		}
		if let Some(api_token) = &model.api_token {
			query = query.filter(UserColumn::ApiToken.eq(api_token.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(UserColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(UserColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(UserColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(UserColumn::Metadata.eq(metadata.clone()));
		}
...
impl ListUser {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListUser>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListUser::default(),
		};
		let mut query = User::find();
		if let Some(id) = &model.id {
			query = query.filter(UserColumn::Id.eq(id.clone()));
...
impl CreateUser {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateUser>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
		...
		let role = if let Some(value) = &model.role {
			Some(sea_orm_active_enums::UserRoleEnum::from(*value))
		} else {
			None
		};
		let new_table = UserActiveModel {
			id: Set("id".to_string()),
			...
			api_token: Set(model.api_token.clone()),
...
impl UpdateUser {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<UserParmns>,
		json_data: Option<web::Json<UpdateUser>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = User::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: UserActiveModel = table.into();
					if let Some(email) = &model.email {
					...
					model_active.api_token = Set(model.api_token);
					if let Some(created_at) = &model.created_at {
					...
					if let Some(value) = &model.role {
						let role = Some(sea_orm_active_enums::UserRoleEnum::from(*value));
...
impl DeleteUser {
	async fn route(
		...
		let res = User::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
