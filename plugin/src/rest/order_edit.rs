...
#[derive(Deserialize)]
pub struct OrderEditParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/order_edit")
			.route("/", web::get().to(GetOrderEdit::route))
			.route("/", web::post().to(CreateOrderEdit::route))
			.route("/list", web::get().to(ListOrderEdit::route))
			.route("/{id}", web::put().to(UpdateOrderEdit::route))
			.route("/{id}", web::get().to(DeleteOrderEdit::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetOrderEdit {
...
impl GetOrderEdit {
	async fn route(
		...
		json_data: Option<web::Json<GetOrderEdit>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetOrderEdit::default(),
		};
		let mut query = OrderEdit::find();
		if let Some(id) = &model.id {
			query = query.filter(OrderEditColumn::Id.eq(id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(OrderEditColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(OrderEditColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(order_id) = &model.order_id {
			query = query.filter(OrderEditColumn::OrderId.eq(order_id.clone()));
		}
		if let Some(internal_note) = &model.internal_note {
			query = query.filter(OrderEditColumn::InternalNote.eq(internal_note.clone()));
		}
		if let Some(created_by) = &model.created_by {
			query = query.filter(OrderEditColumn::CreatedBy.eq(created_by.clone()));
		}
		if let Some(requested_by) = &model.requested_by {
			query = query.filter(OrderEditColumn::RequestedBy.eq(requested_by.clone()));
		}
		if let Some(requested_at) = &model.requested_at {
			query = query.filter(OrderEditColumn::RequestedAt.eq(requested_at.clone()));
		}
		if let Some(confirmed_by) = &model.confirmed_by {
			query = query.filter(OrderEditColumn::ConfirmedBy.eq(confirmed_by.clone()));
		}
		if let Some(confirmed_at) = &model.confirmed_at {
			query = query.filter(OrderEditColumn::ConfirmedAt.eq(confirmed_at.clone()));
		}
		if let Some(declined_by) = &model.declined_by {
			query = query.filter(OrderEditColumn::DeclinedBy.eq(declined_by.clone()));
		}
		if let Some(declined_reason) = &model.declined_reason {
			query = query.filter(OrderEditColumn::DeclinedReason.eq(declined_reason.clone()));
		}
		if let Some(declined_at) = &model.declined_at {
			query = query.filter(OrderEditColumn::DeclinedAt.eq(declined_at.clone()));
		}
		if let Some(canceled_by) = &model.canceled_by {
			query = query.filter(OrderEditColumn::CanceledBy.eq(canceled_by.clone()));
		}
		if let Some(canceled_at) = &model.canceled_at {
			query = query.filter(OrderEditColumn::CanceledAt.eq(canceled_at.clone()));
		}
		if let Some(payment_collection_id) = &model.payment_collection_id {
			query = query
				.filter(OrderEditColumn::PaymentCollectionId.eq(payment_collection_id.clone()));
		}
...
impl ListOrderEdit {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListOrderEdit>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListOrderEdit::default(),
		};
		let mut query = OrderEdit::find();
		if let Some(id) = &model.id {
			query = query.filter(OrderEditColumn::Id.eq(id.clone()));
...
impl CreateOrderEdit {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateOrderEdit>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = OrderEditActiveModel {
			id: Set("id".to_string()),
			created_at: Set(model.created_at.clone()),
			updated_at: Set(model.updated_at.clone()),
			order_id: Set(model.order_id.clone()),
			internal_note: Set(model.internal_note.clone()),
			created_by: Set(model.created_by.clone()),
			requested_by: Set(model.requested_by.clone()),
			requested_at: Set(model.requested_at.clone()),
			confirmed_by: Set(model.confirmed_by.clone()),
			confirmed_at: Set(model.confirmed_at.clone()),
			declined_by: Set(model.declined_by.clone()),
			declined_reason: Set(model.declined_reason.clone()),
			declined_at: Set(model.declined_at.clone()),
			canceled_by: Set(model.canceled_by.clone()),
			canceled_at: Set(model.canceled_at.clone()),
			payment_collection_id: Set(model.payment_collection_id.clone()),
		};
...
impl UpdateOrderEdit {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<OrderEditParmns>,
		json_data: Option<web::Json<UpdateOrderEdit>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = OrderEdit::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: OrderEditActiveModel = table.into();
					if let Some(created_at) = &model.created_at {
					...
					model_active.internal_note = Set(model.internal_note);
					if let Some(created_by) = &model.created_by {
						model_active.created_by = Set(created_by.clone());
					}
					model_active.requested_by = Set(model.requested_by);
					model_active.requested_at = Set(model.requested_at);
					model_active.confirmed_by = Set(model.confirmed_by);
					model_active.confirmed_at = Set(model.confirmed_at);
					model_active.declined_by = Set(model.declined_by);
					model_active.declined_reason = Set(model.declined_reason);
					model_active.declined_at = Set(model.declined_at);
					model_active.canceled_by = Set(model.canceled_by);
					model_active.canceled_at = Set(model.canceled_at);
					model_active.payment_collection_id = Set(model.payment_collection_id);
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteOrderEdit {
	pub id: String,
}

impl DeleteOrderEdit {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<OrderEditParmns>,
	) -> impl Responder {
		let res = OrderEdit::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
