...
#[derive(Deserialize)]
pub struct DiscountRegionsParmns {
	discount_id: String,
	region_id: String,
}
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/discount_regions")
			.route("/", web::get().to(GetDiscountRegions::route))
			.route("/", web::post().to(CreateDiscountRegions::route))
			.route("/list", web::get().to(ListDiscountRegions::route))
			.route("/{id}", web::put().to(UpdateDiscountRegions::route))
			.route("/{id}", web::get().to(DeleteDiscountRegions::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetDiscountRegions {
	pub discount_id: Option<String>,
	pub region_id: Option<String>,
}

impl GetDiscountRegions {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetDiscountRegions>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetDiscountRegions::default(),
		};
		let mut query = DiscountRegions::find();
		if let Some(discount_id) = &model.discount_id {
			query = query.filter(DiscountRegionsColumn::DiscountId.eq(discount_id.clone()));
		}
		if let Some(region_id) = &model.region_id {
			query = query.filter(DiscountRegionsColumn::RegionId.eq(region_id.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListDiscountRegions {
	pub discount_id: Option<String>,
	pub region_id: Option<String>,
}

impl ListDiscountRegions {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListDiscountRegions>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListDiscountRegions::default(),
		};
		let mut query = DiscountRegions::find();
		if let Some(discount_id) = &model.discount_id {
			query = query.filter(DiscountRegionsColumn::DiscountId.eq(discount_id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateDiscountRegions {}

impl CreateDiscountRegions {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateDiscountRegions>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = DiscountRegionsActiveModel {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateDiscountRegions {}

impl UpdateDiscountRegions {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<DiscountRegionsParmns>,
		json_data: Option<web::Json<UpdateDiscountRegions>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		...
		let old_table =
			DiscountRegions::find_by_id((path.discount_id.clone(), path.region_id.clone()))
				.one(db.get_ref())
				.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: DiscountRegionsActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteDiscountRegions {
	pub discount_id: String,
	pub region_id: String,
}

impl DeleteDiscountRegions {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<DiscountRegionsParmns>,
	) -> impl Responder {
		let res = DiscountRegions::delete_by_id((path.discount_id.clone(), path.region_id.clone()))
			.exec(db.get_ref())
			.await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseDiscountRegions {
	pub discount_id: String,
	pub region_id: String,
}
