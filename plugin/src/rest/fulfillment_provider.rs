...
#[derive(Deserialize)]
pub struct FulfillmentProviderParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/fulfillment_provider")
			.route("/", web::get().to(GetFulfillmentProvider::route))
			.route("/", web::post().to(CreateFulfillmentProvider::route))
			.route("/list", web::get().to(ListFulfillmentProvider::route))
			.route("/{id}", web::put().to(UpdateFulfillmentProvider::route))
			.route("/{id}", web::get().to(DeleteFulfillmentProvider::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetFulfillmentProvider {
	pub id: Option<String>,
	pub is_installed: Option<bool>,
}

impl GetFulfillmentProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetFulfillmentProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetFulfillmentProvider::default(),
		};
		let mut query = FulfillmentProvider::find();
		if let Some(id) = &model.id {
			query = query.filter(FulfillmentProviderColumn::Id.eq(id.clone()));
		}
		if let Some(is_installed) = &model.is_installed {
			query = query.filter(FulfillmentProviderColumn::IsInstalled.eq(is_installed.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListFulfillmentProvider {
	pub id: Option<String>,
	pub is_installed: Option<bool>,
}

impl ListFulfillmentProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListFulfillmentProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListFulfillmentProvider::default(),
		};
		let mut query = FulfillmentProvider::find();
		if let Some(id) = &model.id {
			query = query.filter(FulfillmentProviderColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateFulfillmentProvider {
	pub is_installed: bool,
}

impl CreateFulfillmentProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateFulfillmentProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = FulfillmentProviderActiveModel {
			id: Set("id".to_string()),
			is_installed: Set(model.is_installed.clone()),
		};
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateFulfillmentProvider {
	pub is_installed: Option<bool>,
}

impl UpdateFulfillmentProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<FulfillmentProviderParmns>,
		json_data: Option<web::Json<UpdateFulfillmentProvider>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = FulfillmentProvider::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: FulfillmentProviderActiveModel = table.into();
					if let Some(is_installed) = &model.is_installed {
						model_active.is_installed = Set(is_installed.clone());
					}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteFulfillmentProvider {
	pub id: String,
}

impl DeleteFulfillmentProvider {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<FulfillmentProviderParmns>,
	) -> impl Responder {
		let res = FulfillmentProvider::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseFulfillmentProvider {
	pub id: String,
	pub is_installed: bool,
}
