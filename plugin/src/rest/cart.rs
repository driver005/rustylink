...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/cart")
			.route("/", web::get().to(GetCart::route))
			.route("/", web::post().to(CreateCart::route))
			.route("/list", web::get().to(ListCart::route))
			.route("/{id}", web::put().to(UpdateCart::route))
			.route("/{id}", web::get().to(DeleteCart::route)),
	);
}
...
pub struct GetCart {
	...
	pub r#type: Option<CartTypeEnum>,
...
impl GetCart {
	async fn route(
		...
		json_data: Option<web::Json<GetCart>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetCart::default(),
		};
		let mut query = Cart::find();
		if let Some(id) = &model.id {
			query = query.filter(CartColumn::Id.eq(id.clone()));
		}
		if let Some(email) = &model.email {
			query = query.filter(CartColumn::Email.eq(email.clone()));
		}
		if let Some(billing_address_id) = &model.billing_address_id {
			query = query.filter(CartColumn::BillingAddressId.eq(billing_address_id.clone()));
		}
		if let Some(shipping_address_id) = &model.shipping_address_id {
			query = query.filter(CartColumn::ShippingAddressId.eq(shipping_address_id.clone()));
		}
		if let Some(region_id) = &model.region_id {
			query = query.filter(CartColumn::RegionId.eq(region_id.clone()));
		}
		if let Some(customer_id) = &model.customer_id {
			query = query.filter(CartColumn::CustomerId.eq(customer_id.clone()));
		}
		if let Some(payment_id) = &model.payment_id {
			query = query.filter(CartColumn::PaymentId.eq(payment_id.clone()));
		}
		if let Some(r#type) = &model.r#type {
			query = query.filter(CartColumn::Type.eq(r#type.clone()));
		}
		if let Some(completed_at) = &model.completed_at {
			query = query.filter(CartColumn::CompletedAt.eq(completed_at.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(CartColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(CartColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(CartColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(CartColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(idempotency_key) = &model.idempotency_key {
			query = query.filter(CartColumn::IdempotencyKey.eq(idempotency_key.clone()));
		}
		if let Some(context) = &model.context {
			query = query.filter(CartColumn::Context.eq(context.clone()));
		}
		if let Some(payment_authorized_at) = &model.payment_authorized_at {
			query = query.filter(CartColumn::PaymentAuthorizedAt.eq(payment_authorized_at.clone()));
		}
		if let Some(sales_channel_id) = &model.sales_channel_id {
			query = query.filter(CartColumn::SalesChannelId.eq(sales_channel_id.clone()));
		}
...
impl ListCart {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListCart>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListCart::default(),
		};
		let mut query = Cart::find();
		if let Some(id) = &model.id {
			query = query.filter(CartColumn::Id.eq(id.clone()));
...
impl CreateCart {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateCart>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let r#type = sea_orm_active_enums::CartTypeEnum::from(*&model.r#type);
		let new_table = CartActiveModel {
			id: Set("id".to_string()),
			...
			payment_id: Set(model.payment_id.clone()),
			r#type: Set(r#type),
			...
			payment_authorized_at: Set(model.payment_authorized_at.clone()),
...
impl UpdateCart {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<CartParmns>,
		json_data: Option<web::Json<UpdateCart>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Cart::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: CartActiveModel = table.into();
					model_active.email = Set(model.email);
					...
					model_active.customer_id = Set(model.customer_id);
					model_active.payment_id = Set(model.payment_id);
					if let Some(value) = &model.r#type {
						let r#type = Some(sea_orm_active_enums::CartTypeEnum::from(*value));
						model_active.r#type = Set(r#type);
					}
					model_active.completed_at = Set(model.completed_at);
					if let Some(created_at) = &model.created_at {
					...
					model_active.context = Set(model.context);
					model_active.payment_authorized_at = Set(model.payment_authorized_at);
...
impl DeleteCart {
	async fn route(
		...
		let res = Cart::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
