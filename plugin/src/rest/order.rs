...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/order")
			.route("/", web::get().to(GetOrder::route))
			.route("/", web::post().to(CreateOrder::route))
			.route("/list", web::get().to(ListOrder::route))
			.route("/{id}", web::put().to(UpdateOrder::route))
			.route("/{id}", web::get().to(DeleteOrder::route)),
	);
}
...
pub struct GetOrder {
	...
	pub fulfillment_status: Option<OrderFulfillmentStatusEnum>,
	pub payment_status: Option<OrderPaymentStatusEnum>,
...
impl GetOrder {
	async fn route(
		...
		json_data: Option<web::Json<GetOrder>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetOrder::default(),
		};
		let mut query = Order::find();
		if let Some(id) = &model.id {
			query = query.filter(OrderColumn::Id.eq(id.clone()));
		}
		if let Some(status) = &model.status {
			query = query.filter(OrderColumn::Status.eq(status.clone()));
		}
		if let Some(fulfillment_status) = &model.fulfillment_status {
			query = query.filter(OrderColumn::FulfillmentStatus.eq(fulfillment_status.clone()));
		}
		if let Some(payment_status) = &model.payment_status {
			query = query.filter(OrderColumn::PaymentStatus.eq(payment_status.clone()));
		}
		if let Some(display_id) = &model.display_id {
			query = query.filter(OrderColumn::DisplayId.eq(display_id.clone()));
		}
		if let Some(cart_id) = &model.cart_id {
			query = query.filter(OrderColumn::CartId.eq(cart_id.clone()));
		}
		if let Some(customer_id) = &model.customer_id {
			query = query.filter(OrderColumn::CustomerId.eq(customer_id.clone()));
		}
		if let Some(email) = &model.email {
			query = query.filter(OrderColumn::Email.eq(email.clone()));
		}
		if let Some(billing_address_id) = &model.billing_address_id {
			query = query.filter(OrderColumn::BillingAddressId.eq(billing_address_id.clone()));
		}
		if let Some(shipping_address_id) = &model.shipping_address_id {
			query = query.filter(OrderColumn::ShippingAddressId.eq(shipping_address_id.clone()));
		}
		if let Some(region_id) = &model.region_id {
			query = query.filter(OrderColumn::RegionId.eq(region_id.clone()));
		}
		if let Some(currency_code) = &model.currency_code {
			query = query.filter(OrderColumn::CurrencyCode.eq(currency_code.clone()));
		}
		if let Some(tax_rate) = &model.tax_rate {
			query = query.filter(OrderColumn::TaxRate.eq(tax_rate.clone()));
		}
		if let Some(canceled_at) = &model.canceled_at {
			query = query.filter(OrderColumn::CanceledAt.eq(canceled_at.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(OrderColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(OrderColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(OrderColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(idempotency_key) = &model.idempotency_key {
			query = query.filter(OrderColumn::IdempotencyKey.eq(idempotency_key.clone()));
		}
		if let Some(draft_order_id) = &model.draft_order_id {
			query = query.filter(OrderColumn::DraftOrderId.eq(draft_order_id.clone()));
		}
		if let Some(no_notification) = &model.no_notification {
			query = query.filter(OrderColumn::NoNotification.eq(no_notification.clone()));
		}
		if let Some(external_id) = &model.external_id {
			query = query.filter(OrderColumn::ExternalId.eq(external_id.clone()));
		}
		if let Some(sales_channel_id) = &model.sales_channel_id {
			query = query.filter(OrderColumn::SalesChannelId.eq(sales_channel_id.clone()));
		}
...
impl ListOrder {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListOrder>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
...
impl CreateOrder {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateOrder>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let status = sea_orm_active_enums::OrderStatusEnum::from(*&model.status);
		let fulfillment_status =
			sea_orm_active_enums::OrderFulfillmentStatusEnum::from(*&model.fulfillment_status);
		let payment_status =
			sea_orm_active_enums::OrderPaymentStatusEnum::from(*&model.payment_status);
		let new_table = OrderActiveModel {
			id: Set("id".to_string()),
			status: Set(status),
			fulfillment_status: Set(fulfillment_status),
			payment_status: Set(payment_status),
			display_id: Set(model.display_id.clone()),
			cart_id: Set(model.cart_id.clone()),
			customer_id: Set(model.customer_id.clone()),
			email: Set(model.email.clone()),
			billing_address_id: Set(model.billing_address_id.clone()),
			shipping_address_id: Set(model.shipping_address_id.clone()),
			region_id: Set(model.region_id.clone()),
			currency_code: Set(model.currency_code.clone()),
			tax_rate: Set(model.tax_rate.clone()),
			canceled_at: Set(model.canceled_at.clone()),
			created_at: Set(model.created_at.clone()),
			updated_at: Set(model.updated_at.clone()),
			metadata: Set(model.metadata.clone()),
			idempotency_key: Set(model.idempotency_key.clone()),
			draft_order_id: Set(model.draft_order_id.clone()),
			no_notification: Set(model.no_notification.clone()),
			external_id: Set(model.external_id.clone()),
			sales_channel_id: Set(model.sales_channel_id.clone()),
		};
...
impl UpdateOrder {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<OrderParmns>,
		json_data: Option<web::Json<UpdateOrder>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Order::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: OrderActiveModel = table.into();
					if let Some(value) = &model.status {
						let status = Some(sea_orm_active_enums::OrderStatusEnum::from(*value));
						model_active.status = Set(status);
					}
					if let Some(value) = &model.fulfillment_status {
						let fulfillment_status =
							Some(sea_orm_active_enums::OrderFulfillmentStatusEnum::from(*value));
						model_active.fulfillment_status = Set(fulfillment_status);
					}
					if let Some(value) = &model.payment_status {
						let payment_status =
							Some(sea_orm_active_enums::OrderPaymentStatusEnum::from(*value));
						model_active.payment_status = Set(payment_status);
					}
					if let Some(display_id) = &model.display_id {
						model_active.display_id = Set(display_id.clone());
					}
					model_active.cart_id = Set(model.cart_id);
					if let Some(customer_id) = &model.customer_id {
						model_active.customer_id = Set(customer_id.clone());
					}
					if let Some(email) = &model.email {
						model_active.email = Set(email.clone());
					}
					model_active.billing_address_id = Set(model.billing_address_id);
					model_active.shipping_address_id = Set(model.shipping_address_id);
					if let Some(region_id) = &model.region_id {
						model_active.region_id = Set(region_id.clone());
					}
					if let Some(currency_code) = &model.currency_code {
						model_active.currency_code = Set(currency_code.clone());
					}
					model_active.tax_rate = Set(model.tax_rate);
					model_active.canceled_at = Set(model.canceled_at);
					if let Some(created_at) = &model.created_at {
					...
					model_active.idempotency_key = Set(model.idempotency_key);
					model_active.draft_order_id = Set(model.draft_order_id);
					model_active.no_notification = Set(model.no_notification);
					model_active.external_id = Set(model.external_id);
					model_active.sales_channel_id = Set(model.sales_channel_id);
...
impl DeleteOrder {
	async fn route(
		...
		let res = Order::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
