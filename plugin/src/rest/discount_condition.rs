...
#[derive(Deserialize)]
pub struct DiscountConditionParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/discount_condition")
			.route("/", web::get().to(GetDiscountCondition::route))
			.route("/", web::post().to(CreateDiscountCondition::route))
			.route("/list", web::get().to(ListDiscountCondition::route))
			.route("/{id}", web::put().to(UpdateDiscountCondition::route))
			.route("/{id}", web::get().to(DeleteDiscountCondition::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetDiscountCondition {
	pub id: Option<String>,
	pub r#type: Option<DiscountConditionTypeEnum>,
	pub operator: Option<DiscountConditionOperatorEnum>,
...
impl GetDiscountCondition {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetDiscountCondition>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetDiscountCondition::default(),
		};
		let mut query = DiscountCondition::find();
		if let Some(id) = &model.id {
			query = query.filter(DiscountConditionColumn::Id.eq(id.clone()));
		}
		if let Some(r#type) = &model.r#type {
			query = query.filter(DiscountConditionColumn::Type.eq(r#type.clone()));
		}
		if let Some(operator) = &model.operator {
			query = query.filter(DiscountConditionColumn::Operator.eq(operator.clone()));
		}
		if let Some(discount_rule_id) = &model.discount_rule_id {
			query =
				query.filter(DiscountConditionColumn::DiscountRuleId.eq(discount_rule_id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(DiscountConditionColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(DiscountConditionColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(DiscountConditionColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(DiscountConditionColumn::Metadata.eq(metadata.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListDiscountCondition {
	pub id: Option<String>,
	pub r#type: Option<DiscountConditionTypeEnum>,
	pub operator: Option<DiscountConditionOperatorEnum>,
	pub discount_rule_id: Option<String>,
...
impl ListDiscountCondition {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListDiscountCondition>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListDiscountCondition::default(),
		};
		let mut query = DiscountCondition::find();
		if let Some(id) = &model.id {
			query = query.filter(DiscountConditionColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateDiscountCondition {
...
impl CreateDiscountCondition {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateDiscountCondition>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let r#type = sea_orm_active_enums::DiscountConditionTypeEnum::from(*&model.r#type);
		let operator = sea_orm_active_enums::DiscountConditionOperatorEnum::from(*&model.operator);
		let new_table = DiscountConditionActiveModel {
			id: Set("id".to_string()),
			r#type: Set(r#type),
			operator: Set(operator),
			discount_rule_id: Set(model.discount_rule_id.clone()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateDiscountCondition {
	pub r#type: Option<DiscountConditionTypeEnum>,
	pub operator: Option<DiscountConditionOperatorEnum>,
	pub discount_rule_id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
...
impl UpdateDiscountCondition {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<DiscountConditionParmns>,
		json_data: Option<web::Json<UpdateDiscountCondition>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = DiscountCondition::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: DiscountConditionActiveModel = table.into();
					if let Some(value) = &model.r#type {
						let r#type =
							Some(sea_orm_active_enums::DiscountConditionTypeEnum::from(*value));
						model_active.r#type = Set(r#type);
					}
					if let Some(value) = &model.operator {
						let operator =
							Some(sea_orm_active_enums::DiscountConditionOperatorEnum::from(*value));
						model_active.operator = Set(operator);
					}
					if let Some(discount_rule_id) = &model.discount_rule_id {
						model_active.discount_rule_id = Set(discount_rule_id.clone());
					}
					if let Some(created_at) = &model.created_at {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteDiscountCondition {
	pub id: String,
}

impl DeleteDiscountCondition {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<DiscountConditionParmns>,
	) -> impl Responder {
		let res = DiscountCondition::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseDiscountCondition {
	pub id: String,
	pub r#type: DiscountConditionTypeEnum,
	pub operator: DiscountConditionOperatorEnum,
	pub discount_rule_id: String,
