...
#[derive(Deserialize)]
pub struct AnalyticsConfigParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/analytics_config")
			.route("/", web::get().to(GetAnalyticsConfig::route))
			.route("/", web::post().to(CreateAnalyticsConfig::route))
			.route("/list", web::get().to(ListAnalyticsConfig::route))
			.route("/{id}", web::put().to(UpdateAnalyticsConfig::route))
			.route("/{id}", web::get().to(DeleteAnalyticsConfig::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetAnalyticsConfig {
...
impl GetAnalyticsConfig {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetAnalyticsConfig>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetAnalyticsConfig::default(),
		};
		let mut query = AnalyticsConfig::find();
		if let Some(id) = &model.id {
			query = query.filter(AnalyticsConfigColumn::Id.eq(id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(AnalyticsConfigColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(AnalyticsConfigColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(AnalyticsConfigColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(user_id) = &model.user_id {
			query = query.filter(AnalyticsConfigColumn::UserId.eq(user_id.clone()));
		}
		if let Some(opt_out) = &model.opt_out {
			query = query.filter(AnalyticsConfigColumn::OptOut.eq(opt_out.clone()));
		}
		if let Some(anonymize) = &model.anonymize {
			query = query.filter(AnalyticsConfigColumn::Anonymize.eq(anonymize.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListAnalyticsConfig {
	pub id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub deleted_at: Option<DateTimeWithTimeZone>,
...
impl ListAnalyticsConfig {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListAnalyticsConfig>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListAnalyticsConfig::default(),
		};
		let mut query = AnalyticsConfig::find();
		if let Some(id) = &model.id {
			query = query.filter(AnalyticsConfigColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateAnalyticsConfig {
...
impl CreateAnalyticsConfig {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateAnalyticsConfig>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = AnalyticsConfigActiveModel {
			id: Set("id".to_string()),
			...
			opt_out: Set(model.opt_out.clone()),
			anonymize: Set(model.anonymize.clone()),
		};
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateAnalyticsConfig {
	pub created_at: Option<DateTimeWithTimeZone>,
	pub updated_at: Option<DateTimeWithTimeZone>,
	pub deleted_at: Option<DateTimeWithTimeZone>,
	pub user_id: Option<String>,
...
impl UpdateAnalyticsConfig {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<AnalyticsConfigParmns>,
		json_data: Option<web::Json<UpdateAnalyticsConfig>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = AnalyticsConfig::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: AnalyticsConfigActiveModel = table.into();
					if let Some(created_at) = &model.created_at {
					...
					if let Some(user_id) = &model.user_id {
						model_active.user_id = Set(user_id.clone());
					}
					if let Some(opt_out) = &model.opt_out {
						model_active.opt_out = Set(opt_out.clone());
					}
					if let Some(anonymize) = &model.anonymize {
						model_active.anonymize = Set(anonymize.clone());
					}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteAnalyticsConfig {
	pub id: String,
}

impl DeleteAnalyticsConfig {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<AnalyticsConfigParmns>,
	) -> impl Responder {
		let res = AnalyticsConfig::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseAnalyticsConfig {
	pub id: String,
	pub created_at: DateTimeWithTimeZone,
	pub updated_at: DateTimeWithTimeZone,
	pub deleted_at: Option<DateTimeWithTimeZone>,
