...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/store")
			.route("/", web::get().to(GetStore::route))
			.route("/", web::post().to(CreateStore::route))
			.route("/list", web::get().to(ListStore::route))
			.route("/{id}", web::put().to(UpdateStore::route))
			.route("/{id}", web::get().to(DeleteStore::route)),
	);
}
...
impl GetStore {
	async fn route(
		...
		json_data: Option<web::Json<GetStore>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetStore::default(),
		};
		let mut query = Store::find();
		if let Some(id) = &model.id {
			query = query.filter(StoreColumn::Id.eq(id.clone()));
		}
		if let Some(name) = &model.name {
			query = query.filter(StoreColumn::Name.eq(name.clone()));
		}
		if let Some(default_currency_code) = &model.default_currency_code {
			query =
				query.filter(StoreColumn::DefaultCurrencyCode.eq(default_currency_code.clone()));
		}
		if let Some(swap_link_template) = &model.swap_link_template {
			query = query.filter(StoreColumn::SwapLinkTemplate.eq(swap_link_template.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(StoreColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(StoreColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(StoreColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(payment_link_template) = &model.payment_link_template {
			query =
				query.filter(StoreColumn::PaymentLinkTemplate.eq(payment_link_template.clone()));
		}
		if let Some(invite_link_template) = &model.invite_link_template {
			query = query.filter(StoreColumn::InviteLinkTemplate.eq(invite_link_template.clone()));
		}
		if let Some(default_sales_channel_id) = &model.default_sales_channel_id {
			query = query
				.filter(StoreColumn::DefaultSalesChannelId.eq(default_sales_channel_id.clone()));
		}
		if let Some(default_location_id) = &model.default_location_id {
			query = query.filter(StoreColumn::DefaultLocationId.eq(default_location_id.clone()));
		}
...
impl ListStore {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListStore>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
...
impl CreateStore {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateStore>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = StoreActiveModel {
			id: Set("id".to_string()),
			name: Set(model.name.clone()),
			default_currency_code: Set(model.default_currency_code.clone()),
			swap_link_template: Set(model.swap_link_template.clone()),
			created_at: Set(model.created_at.clone()),
			updated_at: Set(model.updated_at.clone()),
			metadata: Set(model.metadata.clone()),
			payment_link_template: Set(model.payment_link_template.clone()),
			invite_link_template: Set(model.invite_link_template.clone()),
			default_sales_channel_id: Set(model.default_sales_channel_id.clone()),
			default_location_id: Set(model.default_location_id.clone()),
		};
...
impl UpdateStore {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<StoreParmns>,
		json_data: Option<web::Json<UpdateStore>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Store::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: StoreActiveModel = table.into();
					if let Some(name) = &model.name {
						model_active.name = Set(name.clone());
					}
					if let Some(default_currency_code) = &model.default_currency_code {
						model_active.default_currency_code = Set(default_currency_code.clone());
					}
					model_active.swap_link_template = Set(model.swap_link_template);
					if let Some(created_at) = &model.created_at {
					...
					model_active.payment_link_template = Set(model.payment_link_template);
					model_active.invite_link_template = Set(model.invite_link_template);
					model_active.default_sales_channel_id = Set(model.default_sales_channel_id);
					model_active.default_location_id = Set(model.default_location_id);
...
impl DeleteStore {
	async fn route(
		...
		let res = Store::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
