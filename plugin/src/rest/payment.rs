...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/payment")
			.route("/", web::get().to(GetPayment::route))
			.route("/", web::post().to(CreatePayment::route))
			.route("/list", web::get().to(ListPayment::route))
			.route("/{id}", web::put().to(UpdatePayment::route))
			.route("/{id}", web::get().to(DeletePayment::route)),
	);
}
...
impl GetPayment {
	async fn route(
		...
		json_data: Option<web::Json<GetPayment>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetPayment::default(),
		};
		let mut query = Payment::find();
		if let Some(id) = &model.id {
			query = query.filter(PaymentColumn::Id.eq(id.clone()));
		}
		if let Some(swap_id) = &model.swap_id {
			query = query.filter(PaymentColumn::SwapId.eq(swap_id.clone()));
		}
		if let Some(cart_id) = &model.cart_id {
			query = query.filter(PaymentColumn::CartId.eq(cart_id.clone()));
		}
		if let Some(order_id) = &model.order_id {
			query = query.filter(PaymentColumn::OrderId.eq(order_id.clone()));
		}
		if let Some(amount) = &model.amount {
			query = query.filter(PaymentColumn::Amount.eq(amount.clone()));
		}
		if let Some(currency_code) = &model.currency_code {
			query = query.filter(PaymentColumn::CurrencyCode.eq(currency_code.clone()));
		}
		if let Some(amount_refunded) = &model.amount_refunded {
			query = query.filter(PaymentColumn::AmountRefunded.eq(amount_refunded.clone()));
		}
		if let Some(provider_id) = &model.provider_id {
			query = query.filter(PaymentColumn::ProviderId.eq(provider_id.clone()));
		}
		if let Some(data) = &model.data {
			query = query.filter(PaymentColumn::Data.eq(data.clone()));
		}
		if let Some(captured_at) = &model.captured_at {
			query = query.filter(PaymentColumn::CapturedAt.eq(captured_at.clone()));
		}
		if let Some(canceled_at) = &model.canceled_at {
			query = query.filter(PaymentColumn::CanceledAt.eq(canceled_at.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(PaymentColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(PaymentColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(PaymentColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(idempotency_key) = &model.idempotency_key {
			query = query.filter(PaymentColumn::IdempotencyKey.eq(idempotency_key.clone()));
		}
...
impl ListPayment {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListPayment>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListPayment::default(),
		};
		let mut query = Payment::find();
		if let Some(id) = &model.id {
			query = query.filter(PaymentColumn::Id.eq(id.clone()));
...
impl CreatePayment {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreatePayment>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = PaymentActiveModel {
			id: Set("id".to_string()),
			...
			amount_refunded: Set(model.amount_refunded.clone()),
			provider_id: Set(model.provider_id.clone()),
			data: Set(model.data.clone()),
			captured_at: Set(model.captured_at.clone()),
...
impl UpdatePayment {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<PaymentParmns>,
		json_data: Option<web::Json<UpdatePayment>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Payment::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: PaymentActiveModel = table.into();
					...
					if let Some(amount_refunded) = &model.amount_refunded {
						model_active.amount_refunded = Set(amount_refunded.clone());
					}
					if let Some(provider_id) = &model.provider_id {
					...
					model_active.captured_at = Set(model.captured_at);
...
impl DeletePayment {
	async fn route(
		...
		let res = Payment::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
