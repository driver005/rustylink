...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/return")
			.route("/", web::get().to(GetReturn::route))
			.route("/", web::post().to(CreateReturn::route))
			.route("/list", web::get().to(ListReturn::route))
			.route("/{id}", web::put().to(UpdateReturn::route))
			.route("/{id}", web::get().to(DeleteReturn::route)),
	);
}
...
impl GetReturn {
	async fn route(
		...
		json_data: Option<web::Json<GetReturn>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetReturn::default(),
		};
		let mut query = Return::find();
		if let Some(id) = &model.id {
			query = query.filter(ReturnColumn::Id.eq(id.clone()));
		}
		if let Some(status) = &model.status {
			query = query.filter(ReturnColumn::Status.eq(status.clone()));
		}
		if let Some(swap_id) = &model.swap_id {
			query = query.filter(ReturnColumn::SwapId.eq(swap_id.clone()));
		}
		if let Some(order_id) = &model.order_id {
			query = query.filter(ReturnColumn::OrderId.eq(order_id.clone()));
		}
		if let Some(shipping_data) = &model.shipping_data {
			query = query.filter(ReturnColumn::ShippingData.eq(shipping_data.clone()));
		}
		if let Some(refund_amount) = &model.refund_amount {
			query = query.filter(ReturnColumn::RefundAmount.eq(refund_amount.clone()));
		}
		if let Some(received_at) = &model.received_at {
			query = query.filter(ReturnColumn::ReceivedAt.eq(received_at.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ReturnColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ReturnColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ReturnColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(idempotency_key) = &model.idempotency_key {
			query = query.filter(ReturnColumn::IdempotencyKey.eq(idempotency_key.clone()));
		}
		if let Some(claim_order_id) = &model.claim_order_id {
			query = query.filter(ReturnColumn::ClaimOrderId.eq(claim_order_id.clone()));
		}
		if let Some(no_notification) = &model.no_notification {
			query = query.filter(ReturnColumn::NoNotification.eq(no_notification.clone()));
		}
		if let Some(location_id) = &model.location_id {
			query = query.filter(ReturnColumn::LocationId.eq(location_id.clone()));
		}
...
impl ListReturn {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListReturn>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListReturn::default(),
		};
		let mut query = Return::find();
		if let Some(id) = &model.id {
			query = query.filter(ReturnColumn::Id.eq(id.clone()));
...
impl CreateReturn {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateReturn>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let status = sea_orm_active_enums::ReturnStatusEnum::from(*&model.status);
		let new_table = ReturnActiveModel {
			id: Set("id".to_string()),
			status: Set(status),
			swap_id: Set(model.swap_id.clone()),
			order_id: Set(model.order_id.clone()),
			shipping_data: Set(model.shipping_data.clone()),
			refund_amount: Set(model.refund_amount.clone()),
			received_at: Set(model.received_at.clone()),
...
impl UpdateReturn {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ReturnParmns>,
		json_data: Option<web::Json<UpdateReturn>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Return::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ReturnActiveModel = table.into();
					if let Some(value) = &model.status {
						let status = Some(sea_orm_active_enums::ReturnStatusEnum::from(*value));
						model_active.status = Set(status);
					}
					model_active.swap_id = Set(model.swap_id);
					model_active.order_id = Set(model.order_id);
					model_active.shipping_data = Set(model.shipping_data);
					if let Some(refund_amount) = &model.refund_amount {
						model_active.refund_amount = Set(refund_amount.clone());
					}
					model_active.received_at = Set(model.received_at);
					if let Some(created_at) = &model.created_at {
...
impl DeleteReturn {
	async fn route(
		...
		let res = Return::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
