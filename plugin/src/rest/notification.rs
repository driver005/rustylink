...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/notification")
			.route("/", web::get().to(GetNotification::route))
			.route("/", web::post().to(CreateNotification::route))
			.route("/list", web::get().to(ListNotification::route))
			.route("/{id}", web::put().to(UpdateNotification::route))
			.route("/{id}", web::get().to(DeleteNotification::route)),
	);
}
...
impl GetNotification {
	async fn route(
		...
		json_data: Option<web::Json<GetNotification>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetNotification::default(),
		};
		let mut query = Notification::find();
		if let Some(id) = &model.id {
			query = query.filter(NotificationColumn::Id.eq(id.clone()));
		}
		if let Some(event_name) = &model.event_name {
			query = query.filter(NotificationColumn::EventName.eq(event_name.clone()));
		}
		if let Some(resource_type) = &model.resource_type {
			query = query.filter(NotificationColumn::ResourceType.eq(resource_type.clone()));
		}
		if let Some(resource_id) = &model.resource_id {
			query = query.filter(NotificationColumn::ResourceId.eq(resource_id.clone()));
		}
		if let Some(customer_id) = &model.customer_id {
			query = query.filter(NotificationColumn::CustomerId.eq(customer_id.clone()));
		}
		if let Some(to) = &model.to {
			query = query.filter(NotificationColumn::To.eq(to.clone()));
		}
		if let Some(data) = &model.data {
			query = query.filter(NotificationColumn::Data.eq(data.clone()));
		}
		if let Some(parent_id) = &model.parent_id {
			query = query.filter(NotificationColumn::ParentId.eq(parent_id.clone()));
		}
		if let Some(provider_id) = &model.provider_id {
			query = query.filter(NotificationColumn::ProviderId.eq(provider_id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(NotificationColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(NotificationColumn::UpdatedAt.eq(updated_at.clone()));
		}
...
impl ListNotification {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListNotification>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListNotification::default(),
		};
		let mut query = Notification::find();
		if let Some(id) = &model.id {
			query = query.filter(NotificationColumn::Id.eq(id.clone()));
...
impl CreateNotification {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateNotification>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = NotificationActiveModel {
			id: Set("id".to_string()),
			event_name: Set(model.event_name.clone()),
			resource_type: Set(model.resource_type.clone()),
			resource_id: Set(model.resource_id.clone()),
			customer_id: Set(model.customer_id.clone()),
			to: Set(model.to.clone()),
...
impl UpdateNotification {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<NotificationParmns>,
		json_data: Option<web::Json<UpdateNotification>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Notification::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: NotificationActiveModel = table.into();
					model_active.event_name = Set(model.event_name);
					if let Some(resource_type) = &model.resource_type {
						model_active.resource_type = Set(resource_type.clone());
					}
					if let Some(resource_id) = &model.resource_id {
						model_active.resource_id = Set(resource_id.clone());
					}
					model_active.customer_id = Set(model.customer_id);
					if let Some(to) = &model.to {
						model_active.to = Set(to.clone());
					}
					if let Some(data) = &model.data {
						model_active.data = Set(data.clone());
					}
					model_active.parent_id = Set(model.parent_id);
...
impl DeleteNotification {
	async fn route(
		...
		let res = Notification::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
