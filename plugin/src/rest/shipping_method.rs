...
#[derive(Deserialize)]
pub struct ShippingMethodParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/shipping_method")
			.route("/", web::get().to(GetShippingMethod::route))
			.route("/", web::post().to(CreateShippingMethod::route))
			.route("/list", web::get().to(ListShippingMethod::route))
			.route("/{id}", web::put().to(UpdateShippingMethod::route))
			.route("/{id}", web::get().to(DeleteShippingMethod::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetShippingMethod {
...
impl GetShippingMethod {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetShippingMethod>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetShippingMethod::default(),
		};
		let mut query = ShippingMethod::find();
		if let Some(id) = &model.id {
			query = query.filter(ShippingMethodColumn::Id.eq(id.clone()));
		}
		if let Some(shipping_option_id) = &model.shipping_option_id {
			query =
				query.filter(ShippingMethodColumn::ShippingOptionId.eq(shipping_option_id.clone()));
		}
		if let Some(order_id) = &model.order_id {
			query = query.filter(ShippingMethodColumn::OrderId.eq(order_id.clone()));
		}
		if let Some(cart_id) = &model.cart_id {
			query = query.filter(ShippingMethodColumn::CartId.eq(cart_id.clone()));
		}
		if let Some(swap_id) = &model.swap_id {
			query = query.filter(ShippingMethodColumn::SwapId.eq(swap_id.clone()));
		}
		if let Some(return_id) = &model.return_id {
			query = query.filter(ShippingMethodColumn::ReturnId.eq(return_id.clone()));
		}
		if let Some(price) = &model.price {
			query = query.filter(ShippingMethodColumn::Price.eq(price.clone()));
		}
		if let Some(data) = &model.data {
			query = query.filter(ShippingMethodColumn::Data.eq(data.clone()));
		}
		if let Some(claim_order_id) = &model.claim_order_id {
			query = query.filter(ShippingMethodColumn::ClaimOrderId.eq(claim_order_id.clone()));
		}
...
impl ListShippingMethod {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListShippingMethod>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListShippingMethod::default(),
		};
		let mut query = ShippingMethod::find();
		if let Some(id) = &model.id {
			query = query.filter(ShippingMethodColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateShippingMethod {
...
impl CreateShippingMethod {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateShippingMethod>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ShippingMethodActiveModel {
			id: Set("id".to_string()),
			shipping_option_id: Set(model.shipping_option_id.clone()),
			order_id: Set(model.order_id.clone()),
			cart_id: Set(model.cart_id.clone()),
			swap_id: Set(model.swap_id.clone()),
			return_id: Set(model.return_id.clone()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateShippingMethod {
	pub shipping_option_id: Option<String>,
	pub order_id: Option<String>,
	pub cart_id: Option<String>,
	pub swap_id: Option<String>,
...
impl UpdateShippingMethod {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ShippingMethodParmns>,
		json_data: Option<web::Json<UpdateShippingMethod>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ShippingMethod::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ShippingMethodActiveModel = table.into();
					if let Some(shipping_option_id) = &model.shipping_option_id {
						model_active.shipping_option_id = Set(shipping_option_id.clone());
					}
					model_active.order_id = Set(model.order_id);
					model_active.cart_id = Set(model.cart_id);
					model_active.swap_id = Set(model.swap_id);
					model_active.return_id = Set(model.return_id);
					if let Some(price) = &model.price {
						model_active.price = Set(price.clone());
					}
					if let Some(data) = &model.data {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteShippingMethod {
	pub id: String,
}

impl DeleteShippingMethod {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ShippingMethodParmns>,
	) -> impl Responder {
		let res = ShippingMethod::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseShippingMethod {
	pub id: String,
	pub shipping_option_id: String,
	pub order_id: Option<String>,
	pub cart_id: Option<String>,
