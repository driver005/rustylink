...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/fulfillment")
			.route("/", web::get().to(GetFulfillment::route))
			.route("/", web::post().to(CreateFulfillment::route))
			.route("/list", web::get().to(ListFulfillment::route))
			.route("/{id}", web::put().to(UpdateFulfillment::route))
			.route("/{id}", web::get().to(DeleteFulfillment::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetFulfillment {
	pub id: Option<String>,
	pub swap_id: Option<String>,
	pub order_id: Option<String>,
	pub tracking_numbers: Option<Json>,
...
impl GetFulfillment {
	async fn route(
		...
		json_data: Option<web::Json<GetFulfillment>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetFulfillment::default(),
		};
		let mut query = Fulfillment::find();
		if let Some(id) = &model.id {
			query = query.filter(FulfillmentColumn::Id.eq(id.clone()));
		}
		if let Some(swap_id) = &model.swap_id {
			query = query.filter(FulfillmentColumn::SwapId.eq(swap_id.clone()));
		}
		if let Some(order_id) = &model.order_id {
			query = query.filter(FulfillmentColumn::OrderId.eq(order_id.clone()));
		}
		if let Some(tracking_numbers) = &model.tracking_numbers {
			query = query.filter(FulfillmentColumn::TrackingNumbers.eq(tracking_numbers.clone()));
		}
		if let Some(data) = &model.data {
			query = query.filter(FulfillmentColumn::Data.eq(data.clone()));
		}
		if let Some(shipped_at) = &model.shipped_at {
			query = query.filter(FulfillmentColumn::ShippedAt.eq(shipped_at.clone()));
		}
		if let Some(canceled_at) = &model.canceled_at {
			query = query.filter(FulfillmentColumn::CanceledAt.eq(canceled_at.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(FulfillmentColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(FulfillmentColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(FulfillmentColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(idempotency_key) = &model.idempotency_key {
			query = query.filter(FulfillmentColumn::IdempotencyKey.eq(idempotency_key.clone()));
		}
		if let Some(provider_id) = &model.provider_id {
			query = query.filter(FulfillmentColumn::ProviderId.eq(provider_id.clone()));
		}
		if let Some(claim_order_id) = &model.claim_order_id {
			query = query.filter(FulfillmentColumn::ClaimOrderId.eq(claim_order_id.clone()));
		}
		if let Some(no_notification) = &model.no_notification {
			query = query.filter(FulfillmentColumn::NoNotification.eq(no_notification.clone()));
		}
		if let Some(location_id) = &model.location_id {
			query = query.filter(FulfillmentColumn::LocationId.eq(location_id.clone()));
		}
...
impl ListFulfillment {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListFulfillment>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListFulfillment::default(),
		};
		let mut query = Fulfillment::find();
		if let Some(id) = &model.id {
			query = query.filter(FulfillmentColumn::Id.eq(id.clone()));
...
impl CreateFulfillment {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateFulfillment>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = FulfillmentActiveModel {
			id: Set("id".to_string()),
			swap_id: Set(model.swap_id.clone()),
			order_id: Set(model.order_id.clone()),
			tracking_numbers: Set(model.tracking_numbers.clone()),
			data: Set(model.data.clone()),
			shipped_at: Set(model.shipped_at.clone()),
			...
			location_id: Set(model.location_id.clone()),
		};
...
impl UpdateFulfillment {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<FulfillmentParmns>,
		json_data: Option<web::Json<UpdateFulfillment>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Fulfillment::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: FulfillmentActiveModel = table.into();
					model_active.swap_id = Set(model.swap_id);
					model_active.order_id = Set(model.order_id);
					if let Some(tracking_numbers) = &model.tracking_numbers {
						model_active.tracking_numbers = Set(tracking_numbers.clone());
					}
					if let Some(data) = &model.data {
						model_active.data = Set(data.clone());
					}
					model_active.shipped_at = Set(model.shipped_at);
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteFulfillment {
	pub id: String,
}

impl DeleteFulfillment {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<FulfillmentParmns>,
	) -> impl Responder {
		let res = Fulfillment::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseFulfillment {
	pub id: String,
	pub swap_id: Option<String>,
	pub order_id: Option<String>,
	pub tracking_numbers: Json,
