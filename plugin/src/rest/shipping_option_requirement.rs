...
#[derive(Deserialize)]
pub struct ShippingOptionRequirementParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/shipping_option_requirement")
			.route("/", web::get().to(GetShippingOptionRequirement::route))
			.route("/", web::post().to(CreateShippingOptionRequirement::route))
			.route("/list", web::get().to(ListShippingOptionRequirement::route))
			.route("/{id}", web::put().to(UpdateShippingOptionRequirement::route))
			.route("/{id}", web::get().to(DeleteShippingOptionRequirement::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetShippingOptionRequirement {
	pub id: Option<String>,
	pub shipping_option_id: Option<String>,
	pub r#type: Option<ShippingOptionRequirementTypeEnum>,
	pub amount: Option<i32>,
	pub deleted_at: Option<DateTimeWithTimeZone>,
}

impl GetShippingOptionRequirement {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetShippingOptionRequirement>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetShippingOptionRequirement::default(),
		};
		let mut query = ShippingOptionRequirement::find();
		if let Some(id) = &model.id {
			query = query.filter(ShippingOptionRequirementColumn::Id.eq(id.clone()));
		}
		if let Some(shipping_option_id) = &model.shipping_option_id {
			query = query.filter(
				ShippingOptionRequirementColumn::ShippingOptionId.eq(shipping_option_id.clone()),
			);
		}
		if let Some(r#type) = &model.r#type {
			query = query.filter(ShippingOptionRequirementColumn::Type.eq(r#type.clone()));
		}
		if let Some(amount) = &model.amount {
			query = query.filter(ShippingOptionRequirementColumn::Amount.eq(amount.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ShippingOptionRequirementColumn::DeletedAt.eq(deleted_at.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListShippingOptionRequirement {
	pub id: Option<String>,
	pub shipping_option_id: Option<String>,
	pub r#type: Option<ShippingOptionRequirementTypeEnum>,
	pub amount: Option<i32>,
...
impl ListShippingOptionRequirement {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListShippingOptionRequirement>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListShippingOptionRequirement::default(),
		};
		let mut query = ShippingOptionRequirement::find();
		if let Some(id) = &model.id {
			query = query.filter(ShippingOptionRequirementColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateShippingOptionRequirement {
...
impl CreateShippingOptionRequirement {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateShippingOptionRequirement>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let r#type = sea_orm_active_enums::ShippingOptionRequirementTypeEnum::from(*&model.r#type);
		let new_table = ShippingOptionRequirementActiveModel {
			id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateShippingOptionRequirement {
	pub shipping_option_id: Option<String>,
	pub r#type: Option<ShippingOptionRequirementTypeEnum>,
	pub amount: Option<i32>,
	pub deleted_at: Option<DateTimeWithTimeZone>,
...
impl UpdateShippingOptionRequirement {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ShippingOptionRequirementParmns>,
		json_data: Option<web::Json<UpdateShippingOptionRequirement>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		...
		let old_table =
			ShippingOptionRequirement::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ShippingOptionRequirementActiveModel = table.into();
					if let Some(shipping_option_id) = &model.shipping_option_id {
						model_active.shipping_option_id = Set(shipping_option_id.clone());
					}
					if let Some(value) = &model.r#type {
						let r#type = Some(
							sea_orm_active_enums::ShippingOptionRequirementTypeEnum::from(*value),
						);
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteShippingOptionRequirement {
	pub id: String,
}

impl DeleteShippingOptionRequirement {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ShippingOptionRequirementParmns>,
	) -> impl Responder {
		let res = ShippingOptionRequirement::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseShippingOptionRequirement {
	pub id: String,
	pub shipping_option_id: String,
	pub r#type: ShippingOptionRequirementTypeEnum,
	pub amount: i32,
