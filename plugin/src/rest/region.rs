...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/region")
			.route("/", web::get().to(GetRegion::route))
			.route("/", web::post().to(CreateRegion::route))
			.route("/list", web::get().to(ListRegion::route))
			.route("/{id}", web::put().to(UpdateRegion::route))
			.route("/{id}", web::get().to(DeleteRegion::route)),
	);
}
...
pub struct GetRegion {
	...
	pub gift_cards_taxable: Option<bool>,
	pub automatic_taxes: Option<bool>,
...
impl GetRegion {
	async fn route(
		...
		json_data: Option<web::Json<GetRegion>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetRegion::default(),
		};
		let mut query = Region::find();
		if let Some(id) = &model.id {
			query = query.filter(RegionColumn::Id.eq(id.clone()));
		}
		if let Some(name) = &model.name {
			query = query.filter(RegionColumn::Name.eq(name.clone()));
		}
		if let Some(currency_code) = &model.currency_code {
			query = query.filter(RegionColumn::CurrencyCode.eq(currency_code.clone()));
		}
		if let Some(tax_rate) = &model.tax_rate {
			query = query.filter(RegionColumn::TaxRate.eq(tax_rate.clone()));
		}
		if let Some(tax_code) = &model.tax_code {
			query = query.filter(RegionColumn::TaxCode.eq(tax_code.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(RegionColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(RegionColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(RegionColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(RegionColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(gift_cards_taxable) = &model.gift_cards_taxable {
			query = query.filter(RegionColumn::GiftCardsTaxable.eq(gift_cards_taxable.clone()));
		}
		if let Some(automatic_taxes) = &model.automatic_taxes {
			query = query.filter(RegionColumn::AutomaticTaxes.eq(automatic_taxes.clone()));
		}
		if let Some(tax_provider_id) = &model.tax_provider_id {
			query = query.filter(RegionColumn::TaxProviderId.eq(tax_provider_id.clone()));
		}
...
impl ListRegion {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListRegion>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListRegion::default(),
		};
		let mut query = Region::find();
		if let Some(id) = &model.id {
			query = query.filter(RegionColumn::Id.eq(id.clone()));
...
impl CreateRegion {
	async fn route(
		...
		let new_table = RegionActiveModel {
			...
			gift_cards_taxable: Set(model.gift_cards_taxable.clone()),
			automatic_taxes: Set(model.automatic_taxes.clone()),
			tax_provider_id: Set(model.tax_provider_id.clone()),
		};
...
impl UpdateRegion {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<RegionParmns>,
		json_data: Option<web::Json<UpdateRegion>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Region::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: RegionActiveModel = table.into();
					if let Some(name) = &model.name {
					...
					if let Some(tax_rate) = &model.tax_rate {
						model_active.tax_rate = Set(tax_rate.clone());
					}
					model_active.tax_code = Set(model.tax_code);
					if let Some(created_at) = &model.created_at {
					...
					if let Some(gift_cards_taxable) = &model.gift_cards_taxable {
						model_active.gift_cards_taxable = Set(gift_cards_taxable.clone());
					}
					if let Some(automatic_taxes) = &model.automatic_taxes {
						model_active.automatic_taxes = Set(automatic_taxes.clone());
					}
					model_active.tax_provider_id = Set(model.tax_provider_id);
...
impl DeleteRegion {
	async fn route(
		...
		let res = Region::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
