...
#[derive(Deserialize)]
pub struct ProductVariantMoneyAmountParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/product_variant_money_amount")
			.route("/", web::get().to(GetProductVariantMoneyAmount::route))
			.route("/", web::post().to(CreateProductVariantMoneyAmount::route))
			.route("/list", web::get().to(ListProductVariantMoneyAmount::route))
			.route("/{id}", web::put().to(UpdateProductVariantMoneyAmount::route))
			.route("/{id}", web::get().to(DeleteProductVariantMoneyAmount::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetProductVariantMoneyAmount {
	pub id: Option<String>,
	pub money_amount_id: Option<String>,
...
impl GetProductVariantMoneyAmount {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetProductVariantMoneyAmount>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetProductVariantMoneyAmount::default(),
		};
		let mut query = ProductVariantMoneyAmount::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductVariantMoneyAmountColumn::Id.eq(id.clone()));
		}
		if let Some(money_amount_id) = &model.money_amount_id {
			query = query
				.filter(ProductVariantMoneyAmountColumn::MoneyAmountId.eq(money_amount_id.clone()));
		}
		if let Some(variant_id) = &model.variant_id {
			query = query.filter(ProductVariantMoneyAmountColumn::VariantId.eq(variant_id.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ProductVariantMoneyAmountColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ProductVariantMoneyAmountColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ProductVariantMoneyAmountColumn::UpdatedAt.eq(updated_at.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListProductVariantMoneyAmount {
	pub id: Option<String>,
	pub money_amount_id: Option<String>,
	pub variant_id: Option<String>,
	pub deleted_at: Option<DateTimeWithTimeZone>,
...
impl ListProductVariantMoneyAmount {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListProductVariantMoneyAmount>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListProductVariantMoneyAmount::default(),
		};
		let mut query = ProductVariantMoneyAmount::find();
		if let Some(id) = &model.id {
			query = query.filter(ProductVariantMoneyAmountColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateProductVariantMoneyAmount {
...
impl CreateProductVariantMoneyAmount {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateProductVariantMoneyAmount>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ProductVariantMoneyAmountActiveModel {
			id: Set("id".to_string()),
			money_amount_id: Set(model.money_amount_id.clone()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateProductVariantMoneyAmount {
	pub money_amount_id: Option<String>,
	pub variant_id: Option<String>,
	pub deleted_at: Option<DateTimeWithTimeZone>,
	pub created_at: Option<DateTimeWithTimeZone>,
...
impl UpdateProductVariantMoneyAmount {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductVariantMoneyAmountParmns>,
		json_data: Option<web::Json<UpdateProductVariantMoneyAmount>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		...
		let old_table =
			ProductVariantMoneyAmount::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ProductVariantMoneyAmountActiveModel = table.into();
					if let Some(money_amount_id) = &model.money_amount_id {
						model_active.money_amount_id = Set(money_amount_id.clone());
					}
					if let Some(variant_id) = &model.variant_id {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteProductVariantMoneyAmount {
	pub id: String,
}

impl DeleteProductVariantMoneyAmount {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ProductVariantMoneyAmountParmns>,
	) -> impl Responder {
		let res = ProductVariantMoneyAmount::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseProductVariantMoneyAmount {
	pub id: String,
	pub money_amount_id: String,
	pub variant_id: String,
	pub deleted_at: Option<DateTimeWithTimeZone>,
