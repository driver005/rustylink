...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/swap")
			.route("/", web::get().to(GetSwap::route))
			.route("/", web::post().to(CreateSwap::route))
			.route("/list", web::get().to(ListSwap::route))
			.route("/{id}", web::put().to(UpdateSwap::route))
			.route("/{id}", web::get().to(DeleteSwap::route)),
	);
}
...
pub struct GetSwap {
	...
	pub fulfillment_status: Option<SwapFulfillmentStatusEnum>,
	pub payment_status: Option<SwapPaymentStatusEnum>,
...
impl GetSwap {
	async fn route(
		...
		json_data: Option<web::Json<GetSwap>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetSwap::default(),
		};
		let mut query = Swap::find();
		if let Some(id) = &model.id {
			query = query.filter(SwapColumn::Id.eq(id.clone()));
		}
		if let Some(fulfillment_status) = &model.fulfillment_status {
			query = query.filter(SwapColumn::FulfillmentStatus.eq(fulfillment_status.clone()));
		}
		if let Some(payment_status) = &model.payment_status {
			query = query.filter(SwapColumn::PaymentStatus.eq(payment_status.clone()));
		}
		if let Some(order_id) = &model.order_id {
			query = query.filter(SwapColumn::OrderId.eq(order_id.clone()));
		}
		if let Some(difference_due) = &model.difference_due {
			query = query.filter(SwapColumn::DifferenceDue.eq(difference_due.clone()));
		}
		if let Some(shipping_address_id) = &model.shipping_address_id {
			query = query.filter(SwapColumn::ShippingAddressId.eq(shipping_address_id.clone()));
		}
		if let Some(cart_id) = &model.cart_id {
			query = query.filter(SwapColumn::CartId.eq(cart_id.clone()));
		}
		if let Some(confirmed_at) = &model.confirmed_at {
			query = query.filter(SwapColumn::ConfirmedAt.eq(confirmed_at.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(SwapColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(SwapColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(SwapColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(SwapColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(idempotency_key) = &model.idempotency_key {
			query = query.filter(SwapColumn::IdempotencyKey.eq(idempotency_key.clone()));
		}
		if let Some(no_notification) = &model.no_notification {
			query = query.filter(SwapColumn::NoNotification.eq(no_notification.clone()));
		}
		if let Some(canceled_at) = &model.canceled_at {
			query = query.filter(SwapColumn::CanceledAt.eq(canceled_at.clone()));
		}
		if let Some(allow_backorder) = &model.allow_backorder {
			query = query.filter(SwapColumn::AllowBackorder.eq(allow_backorder.clone()));
		}
...
impl ListSwap {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListSwap>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListSwap::default(),
		};
		let mut query = Swap::find();
		if let Some(id) = &model.id {
			query = query.filter(SwapColumn::Id.eq(id.clone()));
...
impl CreateSwap {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateSwap>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
		...
		let fulfillment_status =
			sea_orm_active_enums::SwapFulfillmentStatusEnum::from(*&model.fulfillment_status);
		let payment_status =
			sea_orm_active_enums::SwapPaymentStatusEnum::from(*&model.payment_status);
		let new_table = SwapActiveModel {
			id: Set("id".to_string()),
			fulfillment_status: Set(fulfillment_status),
			payment_status: Set(payment_status),
			order_id: Set(model.order_id.clone()),
			difference_due: Set(model.difference_due.clone()),
...
impl UpdateSwap {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<SwapParmns>,
		json_data: Option<web::Json<UpdateSwap>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Swap::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: SwapActiveModel = table.into();
					if let Some(value) = &model.fulfillment_status {
						let fulfillment_status =
							Some(sea_orm_active_enums::SwapFulfillmentStatusEnum::from(*value));
						model_active.fulfillment_status = Set(fulfillment_status);
					}
					if let Some(value) = &model.payment_status {
						let payment_status =
							Some(sea_orm_active_enums::SwapPaymentStatusEnum::from(*value));
						model_active.payment_status = Set(payment_status);
					}
					if let Some(order_id) = &model.order_id {
						model_active.order_id = Set(order_id.clone());
					}
					model_active.difference_due = Set(model.difference_due);
...
impl DeleteSwap {
	async fn route(
		...
		let res = Swap::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
