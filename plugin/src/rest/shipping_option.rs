...
#[derive(Deserialize)]
pub struct ShippingOptionParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/shipping_option")
			.route("/", web::get().to(GetShippingOption::route))
			.route("/", web::post().to(CreateShippingOption::route))
			.route("/list", web::get().to(ListShippingOption::route))
			.route("/{id}", web::put().to(UpdateShippingOption::route))
			.route("/{id}", web::get().to(DeleteShippingOption::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetShippingOption {
	pub id: Option<String>,
	pub name: Option<String>,
	pub region_id: Option<String>,
	pub profile_id: Option<String>,
	pub provider_id: Option<String>,
	pub price_type: Option<ShippingOptionPriceTypeEnum>,
...
impl GetShippingOption {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetShippingOption>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetShippingOption::default(),
		};
		let mut query = ShippingOption::find();
		if let Some(id) = &model.id {
			query = query.filter(ShippingOptionColumn::Id.eq(id.clone()));
		}
		if let Some(name) = &model.name {
			query = query.filter(ShippingOptionColumn::Name.eq(name.clone()));
		}
		if let Some(region_id) = &model.region_id {
			query = query.filter(ShippingOptionColumn::RegionId.eq(region_id.clone()));
		}
		if let Some(profile_id) = &model.profile_id {
			query = query.filter(ShippingOptionColumn::ProfileId.eq(profile_id.clone()));
		}
		if let Some(provider_id) = &model.provider_id {
			query = query.filter(ShippingOptionColumn::ProviderId.eq(provider_id.clone()));
		}
		if let Some(price_type) = &model.price_type {
			query = query.filter(ShippingOptionColumn::PriceType.eq(price_type.clone()));
		}
		if let Some(amount) = &model.amount {
			query = query.filter(ShippingOptionColumn::Amount.eq(amount.clone()));
		}
		if let Some(is_return) = &model.is_return {
			query = query.filter(ShippingOptionColumn::IsReturn.eq(is_return.clone()));
		}
		if let Some(data) = &model.data {
			query = query.filter(ShippingOptionColumn::Data.eq(data.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ShippingOptionColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ShippingOptionColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ShippingOptionColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ShippingOptionColumn::Metadata.eq(metadata.clone()));
		}
		if let Some(admin_only) = &model.admin_only {
			query = query.filter(ShippingOptionColumn::AdminOnly.eq(admin_only.clone()));
		}
...
impl ListShippingOption {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListShippingOption>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListShippingOption::default(),
		};
		let mut query = ShippingOption::find();
		if let Some(id) = &model.id {
			query = query.filter(ShippingOptionColumn::Id.eq(id.clone()));
...
impl CreateShippingOption {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateShippingOption>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
		...
		let price_type =
			sea_orm_active_enums::ShippingOptionPriceTypeEnum::from(*&model.price_type);
		let new_table = ShippingOptionActiveModel {
			id: Set("id".to_string()),
			name: Set(model.name.clone()),
			region_id: Set(model.region_id.clone()),
			profile_id: Set(model.profile_id.clone()),
			...
			admin_only: Set(model.admin_only.clone()),
		};
...
impl UpdateShippingOption {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ShippingOptionParmns>,
		json_data: Option<web::Json<UpdateShippingOption>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ShippingOption::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ShippingOptionActiveModel = table.into();
					if let Some(name) = &model.name {
						model_active.name = Set(name.clone());
					}
					if let Some(region_id) = &model.region_id {
						model_active.region_id = Set(region_id.clone());
					}
					if let Some(profile_id) = &model.profile_id {
						model_active.profile_id = Set(profile_id.clone());
					}
					if let Some(provider_id) = &model.provider_id {
						model_active.provider_id = Set(provider_id.clone());
					}
					if let Some(value) = &model.price_type {
						let price_type =
							Some(sea_orm_active_enums::ShippingOptionPriceTypeEnum::from(*value));
						model_active.price_type = Set(price_type);
					}
					...
					if let Some(admin_only) = &model.admin_only {
						model_active.admin_only = Set(admin_only.clone());
					}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteShippingOption {
	pub id: String,
}

impl DeleteShippingOption {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ShippingOptionParmns>,
	) -> impl Responder {
		let res = ShippingOption::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseShippingOption {
	pub id: String,
	pub name: String,
	pub region_id: String,
	pub profile_id: String,
