...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/claim_tag")
			.route("/", web::get().to(GetClaimTag::route))
			.route("/", web::post().to(CreateClaimTag::route))
			.route("/list", web::get().to(ListClaimTag::route))
			.route("/{id}", web::put().to(UpdateClaimTag::route))
			.route("/{id}", web::get().to(DeleteClaimTag::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetClaimTag {
...
impl GetClaimTag {
	async fn route(
		...
		json_data: Option<web::Json<GetClaimTag>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetClaimTag::default(),
		};
		let mut query = ClaimTag::find();
		if let Some(id) = &model.id {
			query = query.filter(ClaimTagColumn::Id.eq(id.clone()));
		}
		if let Some(value) = &model.value {
			query = query.filter(ClaimTagColumn::Value.eq(value.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(ClaimTagColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(ClaimTagColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(ClaimTagColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(ClaimTagColumn::Metadata.eq(metadata.clone()));
		}
...
impl ListClaimTag {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListClaimTag>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListClaimTag::default(),
		};
		let mut query = ClaimTag::find();
		if let Some(id) = &model.id {
			query = query.filter(ClaimTagColumn::Id.eq(id.clone()));
...
impl CreateClaimTag {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateClaimTag>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = ClaimTagActiveModel {
			id: Set("id".to_string()),
...
impl UpdateClaimTag {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<ClaimTagParmns>,
		json_data: Option<web::Json<UpdateClaimTag>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = ClaimTag::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: ClaimTagActiveModel = table.into();
					if let Some(value) = &model.value {
...
impl DeleteClaimTag {
	async fn route(
		...
		let res = ClaimTag::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
