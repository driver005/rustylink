...
#[derive(Deserialize)]
pub struct CustomShippingOptionParmns {
	id: String,
}

pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/custom_shipping_option")
			.route("/", web::get().to(GetCustomShippingOption::route))
			.route("/", web::post().to(CreateCustomShippingOption::route))
			.route("/list", web::get().to(ListCustomShippingOption::route))
			.route("/{id}", web::put().to(UpdateCustomShippingOption::route))
			.route("/{id}", web::get().to(DeleteCustomShippingOption::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetCustomShippingOption {
...
impl GetCustomShippingOption {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetCustomShippingOption>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetCustomShippingOption::default(),
		};
		let mut query = CustomShippingOption::find();
		if let Some(id) = &model.id {
			query = query.filter(CustomShippingOptionColumn::Id.eq(id.clone()));
		}
		if let Some(price) = &model.price {
			query = query.filter(CustomShippingOptionColumn::Price.eq(price.clone()));
		}
		if let Some(shipping_option_id) = &model.shipping_option_id {
			query = query.filter(
				CustomShippingOptionColumn::ShippingOptionId.eq(shipping_option_id.clone()),
			);
		}
		if let Some(cart_id) = &model.cart_id {
			query = query.filter(CustomShippingOptionColumn::CartId.eq(cart_id.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(CustomShippingOptionColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(CustomShippingOptionColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(CustomShippingOptionColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(CustomShippingOptionColumn::Metadata.eq(metadata.clone()));
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListCustomShippingOption {
	pub id: Option<String>,
	pub price: Option<i32>,
	pub shipping_option_id: Option<String>,
	pub cart_id: Option<String>,
...
impl ListCustomShippingOption {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListCustomShippingOption>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListCustomShippingOption::default(),
		};
		let mut query = CustomShippingOption::find();
		if let Some(id) = &model.id {
			query = query.filter(CustomShippingOptionColumn::Id.eq(id.clone()));
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreateCustomShippingOption {
	pub price: i32,
...
impl CreateCustomShippingOption {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateCustomShippingOption>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = CustomShippingOptionActiveModel {
			id: Set("id".to_string()),
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdateCustomShippingOption {
	pub price: Option<i32>,
	pub shipping_option_id: Option<String>,
	pub cart_id: Option<String>,
	pub created_at: Option<DateTimeWithTimeZone>,
...
impl UpdateCustomShippingOption {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<CustomShippingOptionParmns>,
		json_data: Option<web::Json<UpdateCustomShippingOption>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = CustomShippingOption::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: CustomShippingOptionActiveModel = table.into();
					if let Some(price) = &model.price {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeleteCustomShippingOption {
	pub id: String,
}

impl DeleteCustomShippingOption {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<CustomShippingOptionParmns>,
	) -> impl Responder {
		let res = CustomShippingOption::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponseCustomShippingOption {
	pub id: String,
	pub price: i32,
	pub shipping_option_id: String,
	pub cart_id: Option<String>,
