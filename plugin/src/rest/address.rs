...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/address")
			.route("/", web::get().to(GetAddress::route))
			.route("/", web::post().to(CreateAddress::route))
			.route("/list", web::get().to(ListAddress::route))
			.route("/{id}", web::put().to(UpdateAddress::route))
			.route("/{id}", web::get().to(DeleteAddress::route)),
	);
}
...
impl GetAddress {
	async fn route(
		...
		json_data: Option<web::Json<GetAddress>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetAddress::default(),
		};
		let mut query = Address::find();
		if let Some(id) = &model.id {
			query = query.filter(AddressColumn::Id.eq(id.clone()));
		}
		if let Some(customer_id) = &model.customer_id {
			query = query.filter(AddressColumn::CustomerId.eq(customer_id.clone()));
		}
		if let Some(company) = &model.company {
			query = query.filter(AddressColumn::Company.eq(company.clone()));
		}
		if let Some(first_name) = &model.first_name {
			query = query.filter(AddressColumn::FirstName.eq(first_name.clone()));
		}
		if let Some(last_name) = &model.last_name {
			query = query.filter(AddressColumn::LastName.eq(last_name.clone()));
		}
		if let Some(address_1) = &model.address_1 {
			query = query.filter(AddressColumn::Address1.eq(address_1.clone()));
		}
		if let Some(address_2) = &model.address_2 {
			query = query.filter(AddressColumn::Address2.eq(address_2.clone()));
		}
		if let Some(city) = &model.city {
			query = query.filter(AddressColumn::City.eq(city.clone()));
		}
		if let Some(country_code) = &model.country_code {
			query = query.filter(AddressColumn::CountryCode.eq(country_code.clone()));
		}
		if let Some(province) = &model.province {
			query = query.filter(AddressColumn::Province.eq(province.clone()));
		}
		if let Some(postal_code) = &model.postal_code {
			query = query.filter(AddressColumn::PostalCode.eq(postal_code.clone()));
		}
		if let Some(phone) = &model.phone {
			query = query.filter(AddressColumn::Phone.eq(phone.clone()));
		}
		if let Some(created_at) = &model.created_at {
			query = query.filter(AddressColumn::CreatedAt.eq(created_at.clone()));
		}
		if let Some(updated_at) = &model.updated_at {
			query = query.filter(AddressColumn::UpdatedAt.eq(updated_at.clone()));
		}
		if let Some(deleted_at) = &model.deleted_at {
			query = query.filter(AddressColumn::DeletedAt.eq(deleted_at.clone()));
		}
		if let Some(metadata) = &model.metadata {
			query = query.filter(AddressColumn::Metadata.eq(metadata.clone()));
		}
...
impl ListAddress {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListAddress>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListAddress::default(),
		};
		let mut query = Address::find();
		if let Some(id) = &model.id {
			query = query.filter(AddressColumn::Id.eq(id.clone()));
...
impl CreateAddress {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreateAddress>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = AddressActiveModel {
			id: Set("id".to_string()),
			...
			address_1: Set(model.address_1.clone()),
			address_2: Set(model.address_2.clone()),
...
impl UpdateAddress {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<AddressParmns>,
		json_data: Option<web::Json<UpdateAddress>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = Address::find_by_id(path.id.clone()).one(db.get_ref()).await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: AddressActiveModel = table.into();
					model_active.customer_id = Set(model.customer_id);
					model_active.company = Set(model.company);
					model_active.first_name = Set(model.first_name);
					model_active.last_name = Set(model.last_name);
					model_active.address_1 = Set(model.address_1);
					model_active.address_2 = Set(model.address_2);
					model_active.city = Set(model.city);
					model_active.country_code = Set(model.country_code);
					model_active.province = Set(model.province);
					model_active.postal_code = Set(model.postal_code);
					model_active.phone = Set(model.phone);
					if let Some(created_at) = &model.created_at {
...
impl DeleteAddress {
	async fn route(
		...
		let res = Address::delete_by_id(path.id.clone()).exec(db.get_ref()).await;
		match res {
