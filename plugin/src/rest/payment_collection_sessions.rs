...
#[derive(Deserialize)]
pub struct PaymentCollectionSessionsParmns {
...
pub fn configure(cfg: &mut web::ServiceConfig) {
	cfg.service(
		web::scope("/payment_collection_sessions")
			.route("/", web::get().to(GetPaymentCollectionSessions::route))
			.route("/", web::post().to(CreatePaymentCollectionSessions::route))
			.route("/list", web::get().to(ListPaymentCollectionSessions::route))
			.route("/{id}", web::put().to(UpdatePaymentCollectionSessions::route))
			.route("/{id}", web::get().to(DeletePaymentCollectionSessions::route)),
	);
}

#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GetPaymentCollectionSessions {
	pub payment_collection_id: Option<String>,
	pub payment_session_id: Option<String>,
}

impl GetPaymentCollectionSessions {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<GetPaymentCollectionSessions>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => GetPaymentCollectionSessions::default(),
		};
		let mut query = PaymentCollectionSessions::find();
		if let Some(payment_collection_id) = &model.payment_collection_id {
			query = query.filter(
				PaymentCollectionSessionsColumn::PaymentCollectionId
					.eq(payment_collection_id.clone()),
			);
		}
		if let Some(payment_session_id) = &model.payment_session_id {
			query = query.filter(
				PaymentCollectionSessionsColumn::PaymentSessionId.eq(payment_session_id.clone()),
			);
		}
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ListPaymentCollectionSessions {
	pub payment_collection_id: Option<String>,
	pub payment_session_id: Option<String>,
}

impl ListPaymentCollectionSessions {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<ListPaymentCollectionSessions>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => ListPaymentCollectionSessions::default(),
		};
		let mut query = PaymentCollectionSessions::find();
		if let Some(payment_collection_id) = &model.payment_collection_id {
			query = query.filter(
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct CreatePaymentCollectionSessions {}

impl CreatePaymentCollectionSessions {
	async fn route(
		db: web::Data<DatabaseConnection>,
		json_data: Option<web::Json<CreatePaymentCollectionSessions>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let new_table = PaymentCollectionSessionsActiveModel {
			payment_collection_id: Set("id".to_string()),
			payment_session_id: Set("id".to_string()),
		};
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct UpdatePaymentCollectionSessions {}

impl UpdatePaymentCollectionSessions {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<PaymentCollectionSessionsParmns>,
		json_data: Option<web::Json<UpdatePaymentCollectionSessions>>,
	) -> impl Responder {
		let model = match json_data {
			Some(model) => model.into_inner(),
			None => return HttpResponse::InternalServerError().body("Empty body received"),
		};
		let old_table = PaymentCollectionSessions::find_by_id((
			path.payment_collection_id.clone(),
			path.payment_session_id.clone(),
		))
		.one(db.get_ref())
		.await;
		match old_table {
			Ok(old) => match old {
				Some(table) => {
					let mut model_active: PaymentCollectionSessionsActiveModel = table.into();
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct DeletePaymentCollectionSessions {
	pub payment_collection_id: String,
	pub payment_session_id: String,
}

impl DeletePaymentCollectionSessions {
	async fn route(
		db: web::Data<DatabaseConnection>,
		path: web::Path<PaymentCollectionSessionsParmns>,
	) -> impl Responder {
		let res = PaymentCollectionSessions::delete_by_id((
			path.payment_collection_id.clone(),
			path.payment_session_id.clone(),
		))
		.exec(db.get_ref())
...
#[derive(Default, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ResponsePaymentCollectionSessions {
	pub payment_collection_id: String,
	pub payment_session_id: String,
}
